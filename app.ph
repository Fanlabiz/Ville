...
try:
    ...
    # PHASE 2 : Placement et optimisation des culturels autour des casernes
    ...
    # Optimisation fine des culturels (déplacement pour maximiser culture Guérison)
    st.write("   → Optimisation fine des culturels...")
    strong_cult = [
        b for b in placed
        if b['Type'] == 'Culturel' and b.get('placed')
        and 'row' in b and 'col' in b and 'rotation' in b
    ]
    for building in strong_cult:
        ...
        for try_rot in [False, True]:
            ...
            for nr, nc, _ in cands:
                ...
                if score > best_score + 15:  # seuil plus élevé pour les gros
                    best_score = score
    ...
    # Calcul final (seulement sur Guérison pour cette version)
    ...
    # Stats (focus Guérison)
    stats = defaultdict(float)
    for b in placed:
        p = b.get('Production','')
        if b['Type']=='Producteur' and p and p == 'Guerison':
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        # Bâtiments placés (focus Guérison)
        ...
        # Résumé
        pd.DataFrame({
            'Indicateur': ['Cases libres', 'Bât non placés', 'Cases non placées', 'Guérison /h'],
            'Valeur': [cases_libres, len(non_places), cases_non_place, round(stats.get('Guerison',0),1)]
