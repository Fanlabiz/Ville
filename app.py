...
def optimize(placed, terrain_grid, max_r, max_c, n_passes=2, progress_cb=None):
    ...
    # ── Simulated Annealing : débloquer les optima locaux (mode placement complet) ──
    # Le SA s'active quand le score est inférieur à 60% du maximum théorique
    # des casernes haute priorité. Il effectue des échanges aléatoires de positions
    # entre bâtiments non-neutres, acceptant parfois des échanges défavorables
    # pour sortir des optima locaux.
    ...
    # Activer le SA si score des casernes < 70% du max possible
    if _top_prods_sa and _top_score_sa < 0.70 * _top_score_max_sa and n_passes >= 2:
        _sa_seed = 42
        _random.seed(_sa_seed)
        _T = 600.0; _T_min = 1.0; _alpha = 0.9990
        _time_budget_sa = 5 * n_passes  # ~15s pour 3 passes, ~25s pour 5 passes

        import time as _time_mod
        _t_sa = _time_mod.time()

        _non_neutral_idx = [i for i, b in enumerate(placed) if b["type"] != "Neutre"]
        _sa_score = score_placement(placed)
        _sa_best = [dict(b) for b in placed]
        _sa_best_score = _sa_score

        def _make_occ_sa(pl, excl_ids):
            _occ = [[False]*max_c for _ in range(max_r)]
            for _b in pl:
                if id(_b) in excl_ids: continue
                for _dr in range(_b["rows"]):
                    for _dc in range(_b["cols"]):
                        _rr, _cc = _b["r"]+_dr, _b["c"]+_dc
                        if 0<=_rr<max_r and 0<=_cc<max_c: _occ[_rr][_cc] = True
            return _occ

        _sa_iters = _sa_accepts = _sa_improves = 0
        while _T > _T_min and _time_mod.time() - _t_sa < _time_budget_sa:
            if len(_non_neutral_idx) < 2:
                break
            _i1, _i2 = _random.sample(_non_neutral_idx, 2)
            _b1, _b2 = placed[_i1], placed[_i2]
            _s1 = (_b1["r"], _b1["c"], _b1["rows"], _b1["cols"])
            _s2 = (_b2["r"], _b2["c"], _b2["rows"], _b2["cols"])

            _occ_w = _make_occ_sa(placed, {id(_b1), id(_b2)})
            _swapped = False; _np1 = _np2 = None

            for _r1, _co1 in [(_b1["rows"], _b1["cols"]), (_b1["cols"], _b1["rows"])]:
                if _swapped: break
                if not can_place(_s2[0], _s2[1], _r1, _co1, x_grid, _occ_w, max_r, max_c): continue
                _occ_w2 = [_row[:] for _row in _occ_w]
                for _dr in range(_r1):
                    for _dc in range(_co1):
                        _rr, _cc = _s2[0]+_dr, _s2[1]+_dc
                        if 0<=_rr<max_r and 0<=_cc<max_c: _occ_w2[_rr][_cc] = True
                for _r2, _co2 in [(_b2["rows"], _b2["cols"]), (_b2["cols"], _b2["rows"])]:
                    if not can_place(_s1[0], _s1[1], _r2, _co2, x_grid, _occ_w2, max_r, max_c): continue
            ...
            if not _swapped:
                _T *= _alpha; _sa_iters += 1; continue

            _b1["r"],_b1["c"],_b1["rows"],_b1["cols"] = _np1
            _b2["r"],_b2["c"],_b2["rows"],_b2["cols"] = _np2
            _new_score = score_placement(placed)
            _delta = _new_score - _sa_score

            if _delta > 0 or _random.random() < _math.exp(min(_delta / _T, 0)):
                _sa_score = _new_score; _sa_accepts += 1
                if _new_score > _sa_best_score:
                    _sa_best_score = _new_score
                    _sa_best = [dict(b) for b in placed]
                    _sa_improves += 1
            else:
            ...
            _T *= _alpha; _sa_iters += 1

        # Restaurer le meilleur état SA
        for _i, _b in enumerate(placed):
            if _i < len(_sa_best):
                _b.update(_sa_best[_i])
...
if uploaded:
    ...
    with mode_col2:
        n_passes = st.slider(
            ...
            help="Plus de passes = meilleur résultat mais plus long. 3 passes est un bon compromis. 5+ pour un résultat maximal."
        )
