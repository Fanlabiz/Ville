...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    headers = ["Nom", "Type", "Production", "Placé", "Coord (L,C)", "Orientation",
               "Priorite", "Placement", "Culture recue", "Boost atteint", "Qte/h avec boost", "Score boost"]
    widths  = [30, 12, 22, 8, 12, 12, 10, 12, 14, 13, 18, 12]
    ...
    # Ajouter les bâtiments non placés (optionnels) avec Placé=Non
    ...
    for b_def in buildings_def:
        ...
        is_optional = b_def.get("placement", "Obligatoire").strip().lower() == "optionnel"
        if missing > 0 and is_optional:
            for _ in range(missing):
                fill_np = mfill(C_GRAY)
                placement_val = b_def.get("placement", "Obligatoire")
                vals_np = [nom, b_def["type"], b_def.get("production",""),
                           "Non", "", "",
                           "", placement_val,
                           "", "", "", ""]
...
if uploaded:
    ...
    if missing_list or missing_optional_list:
        ...
        do_place_missing = st.checkbox(
            "Placer automatiquement les bâtiments manquants avant d'optimiser",
            value=True,
            help="Les bâtiments manquants seront placés au mieux sur les cases libres, puis l'optimisation s'exécutera sur l'ensemble."
        )
    else:
    ...
    with mode_col2:
        ...
        # Convertir en nombre de passes approximatif pour l'algo interne
        # ~40s par passe (25s SA + 15s greedy)
...
if st.session_state.result_excel is not None:
    ...
    st.divider()
    st.download_button(
        label="⬇️  Télécharger le fichier résultat Excel",
    ...
    with st.expander("📊 Ouvrir dans Google Sheets"):
        st.markdown("""
**Pour importer le résultat dans Google Sheets :**

1. Téléchargez le fichier Excel ci-dessus
2. Ouvrez [Google Sheets](https://sheets.google.com)
3. **Fichier** → **Importer** → **Importer un fichier**
4. Glissez le fichier `.xlsx` ou cliquez pour le sélectionner
5. Choisissez **Insérer une nouvelle feuille de calcul** → **Importer les données**

Tous les onglets (Liste batiments, Synthèse, Déplacements, Terrain optimisé) seront importés avec la mise en forme.
        """)
