...
class PlacementBatiments:
    def __init__(self, terrain, batiments):
        ...
        self.terrain = terrain.copy()  # 1 = libre, 0 = occupé
        ...
        for i in range(x_min, x_max):
            for j in range(y_min, y_max):
                ...
                if (i < x or i >= x + longueur or j < y or j >= y + largeur):
                    if self.terrain[i, j] == 1 or isinstance(self.terrain[i, j], str):
                        zone.append((i, j))
    ...
    def peut_placer_batiment(self, x, y, longueur, largeur):
        ...
        for i in range(x, x + longueur):
            for j in range(y, y + largeur):
                if self.terrain[i, j] != 1:  # Pas libre
                    return False
        return True
    ...
    def verifier_bande_1(self, x, y, longueur, largeur):
        ...
        if x == 0 or x + longueur == self.hauteur or y == 0 or y + largeur == self.largeur:
            ...
            if x > 0:
                ...
                bande_valide = False
                for j in range(y, y + largeur):
                    if x - 1 >= 0 and (self.terrain[x-1, j] == 0 or j == 0 or j == self.largeur-1):
                        bande_valide = True
                        break
                if not bande_valide:
                    return False
            ...
            if x + longueur < self.hauteur:
                ...
                bande_valide = False
                for j in range(y, y + largeur):
                    if x + longueur < self.hauteur and (self.terrain[x+longueur, j] == 0 or j == 0 or j == self.largeur-1):
                        bande_valide = True
                        break
                if not bande_valide:
                    return False
            ...
            if y > 0:
                ...
                bande_valide = False
                for i in range(x, x + longueur):
                    if y - 1 >= 0 and (self.terrain[i, y-1] == 0 or i == 0 or i == self.hauteur-1):
                        bande_valide = True
                        break
                if not bande_valide:
                    return False
            ...
            if y + largeur < self.largeur:
                ...
                bande_valide = False
                for i in range(x, x + longueur):
                    if y + largeur < self.largeur and (self.terrain[i, y+largeur] == 0 or i == 0 or i == self.hauteur-1):
                        bande_valide = True
                        break
                if not bande_valide:
                    return False
                        ...
                        for ii in range(max(0, i - batiment['rayonnement']), 
                            ...
                            for jj in range(max(0, j - batiment['rayonnement']), 
                                ...
                                if (ii < i or ii >= i + longueur or jj < j or jj >= j + largeur):
                                    if self.terrain[ii, jj] == 1 or isinstance(self.terrain[ii, jj], str):
    ...
    def placer_batiment(self, batiment, x, y, longueur, largeur, culture_recue=0):
        ...
        if batiment['type'] == 'culturel':
            zone = self.calculer_zone_rayonnement(x, y, longueur, largeur, 
...
def lire_fichier_excel(uploaded_file):
    ...
    for _, row in df_batiments.iterrows():
        batiment = {
            ...
            'boost_25': float(row['Boost 25%']) if pd.notna(row['Boost 25%']) else 0,
            'boost_50': float(row['Boost 50%']) if pd.notna(row['Boost 50%']) else 0,
            'boost_100': float(row['Boost 100%']) if pd.notna(row['Boost 100%']) else 0,
            'production': str(row['Production']) if pd.notna(row['Production']) else ''
        }
        batiments.append(batiment)
    
    return terrain, batiments

def generer_fichier_resultat(terrain_original, terrain_place, batiments_places, stats_culture):
    """
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Créer une représentation lisible du terrain avec les noms des bâtiments
        terrain_affichage = []
        for i in range(terrain_place.shape[0]):
            ligne = []
            for j in range(terrain_place.shape[1]):
                if isinstance(terrain_place[i, j], (int, np.integer)):
                    if terrain_place[i, j] == 1:
                        ligne.append('LIBRE')
                    elif terrain_place[i, j] == 0:
                        ligne.append('OCCUPE')
                    else:
                        ligne.append(str(terrain_place[i, j]))
                else:
                    ligne.append(str(terrain_place[i, j]))
