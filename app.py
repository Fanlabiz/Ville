...
def optimize_no_reduction(placed, terrain_grid, max_r, max_c, progress_cb=None):
    ...
    sans en diminuer aucun. Tous les producteurs sont pris en compte
    indépendamment de leur priorité.
    """
    x_grid = make_x_grid(terrain_grid, max_r, max_c)
    placed = [dict(b) for b in placed]
    all_moves = []
    ...
    # Indexer les producteurs par position dans la liste pour comparaison stable
    prod_idx = {id(b): i for i, b in enumerate(producteurs)}

    def get_boosts():
        """Retourne la liste ordonnée des boosts pour tous les producteurs."""
    ...
    def tick():
        op[0] += 1
        if progress_cb:
            progress_cb(min(op[0] / max(n * 10, 1), 0.98))
        ...
        for b in culturels:
            ...
            for r in range(max_r - b["rows"] + 1):
                for c in range(max_c - b["cols"] + 1):
                    ...
                    # Tester le mouvement
                    b["r"], b["c"] = r, c
                    boosts_after = get_boosts()
                    b["r"], b["c"] = old_r, old_c

                    # Vérifier : aucune régression, au moins une progression
                    regression  = any(a < bef for a, bef in zip(boosts_after, boosts_before))
                    progression = any(a > bef for a, bef in zip(boosts_after, boosts_before))
                    if not regression and progression:
                        gain = sum(a - bef for a, bef in zip(boosts_after, boosts_before))
                        if gain > best_gain:
                            best_gain = gain
                            best_pos = (r, c)
