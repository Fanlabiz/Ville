...
def score_total(placed_list, prio_order=None):
    """Score pondéré par type de production, priorité individuelle et gain potentiel."""
    if prio_order is None:
        prio_order = ['Guérison', 'Nourriture', 'Or']

    # Poids par type de production (exponentiel pour garantir la hiérarchie)
    base_weights = {}
    for i, p in enumerate(prio_order):
        base_weights[p] = 10 ** (len(prio_order) - i)

    # Nombre de niveaux de priorité individuelle (1=plus important, 4=moins)
    MAX_PRIO = 4
    ...
    for bat in placed_list:
        ...
        # Poids par priorité individuelle : priorité 1 → facteur MAX_PRIO,
        # priorité 4 → facteur 1, priorité 0 (non définie) → facteur 1
        prio = bat.get('priorite', 0) or 0
        ...
        # Gain potentiel vers le prochain palier
        if b100 and cr < b100:
            gain_potentiel = qte * 2.0 - prod_actuelle
        elif b50 and cr < b50:
        ...
        score += w_prod * w_prio * (prod_actuelle + 0.1 * gain_potentiel)
...
def optimiser(placed_list, inside_mask, grid_shape, prio_order, n_passes=3, progress_cb=None):
    ...
    def culture_recue_bat(prod, state):
        """Culture reçue par un producteur donné depuis tous les culturels de state."""
        r0, c0 = prod['row'], prod['col']
        r1, c1 = r0 + prod['h'] - 1, c0 + prod['w'] - 1
        total = 0.0
        for b in state:
            ...
            ray = b['rayonnement']
            cr0, cc0 = b['row'], b['col']
            cr1, cc1 = cr0 + b['h'] - 1, cc0 + b['w'] - 1
            if r0 <= cr1+ray and r1 >= cr0-ray and c0 <= cc1+ray and c1 >= cc0-ray:
                total += b['culture']
        return total

    def apply_boost(bat):
        cr = bat.get('culture_recue', 0)
        b25, b50, b100 = bat.get('boost25',0) or 0, bat.get('boost50',0) or 0, bat.get('boost100',0) or 0
        if b100 and cr >= b100:
            bat['boost'], bat['boost_factor'] = 100, 2.0
        elif b50 and cr >= b50:
            bat['boost'], bat['boost_factor'] = 50, 1.5
        elif b25 and cr >= b25:
            bat['boost'], bat['boost_factor'] = 25, 1.25
        else:
            bat['boost'], bat['boost_factor'] = 0, 1.0
        bat['prod_boosted'] = bat['quantite'] * bat['boost_factor']

    def recalc_producteurs(state):
        """Recalcule culture_recue/boost uniquement pour les producteurs."""
        for bat in state:
            if bat['type'] == 'Producteur':
                bat['culture_recue'] = culture_recue_bat(bat, state)
                apply_boost(bat)
            else:
                bat['culture_recue'] = 0.0
                bat['boost'] = 0; bat['boost_factor'] = 1.0; bat['prod_boosted'] = 0.0

    def score_incremental(state):
        return score_total(state, prio_order)
    ...
    for pass_num in range(1, n_passes + 1):
        ...
        for i in order:
            ...
            # Orientation courante en premier (amélioration 2)
            ...
            for orientation in orientations:
                ...
                for r, c in free_positions(h, w, occ):
                    if r == bat['row'] and c == bat['col'] and h == bat['h'] and w == bat['w']:
                        continue
                    # Ne tester que les positions vraiment libres
                ...
                for ori_i in ['H', 'V']:
                    ...
                    for ori_j in ['H', 'V']:
                        ...
                        if not fits(hi, wi, rj, cj_col) or not fits(hj, wj, ri, ci_col):
                            continue
                        # Vérifier absence de chevauchement entre les deux nouvelles zones
        ...
        for pi in prod_prio:
            ...
            # Pré-calculer positions touchantes par culturel
            cult_pos = {}
                ...
                for orientation in ['H','V']:
                    ...
                    for r, c in free_positions(h, w, occ_sans):
                        if touche(r, c, h, w, ray):
                            pos.append((r, c, h, w, orientation))
                cult_pos[ci] = pos
            ...
            for ci in cult_indices:
                ...
                for orientation in ['H', 'V']:
                    ...
                    for r, c in free_positions(h, w, occ_sans):
                        if any((r+dr, c+dc) in occ_sans for dr in range(h) for dc in range(w)):
                            continue
                        # Vérifier que cette position touche au moins un producteur cible
                        touche_cible = False
                        for t in targets:
                            r0t, c0t = t['row'], t['col']
                            r1t, c1t = r0t+t['h']-1, c0t+t['w']-1
                            if r0t <= r+h-1+ray and r1t >= r-ray and                                c0t <= c+w-1+ray and c1t >= c-ray:
                                touche_cible = True
                                break
                        if not touche_cible:
                            continue
                        # Distance de cette position au barycentre
                        new_dist = abs(r + h/2.0 - bar_r) + abs(c + w/2.0 - bar_c)
                        if new_dist >= best_dist:
                            continue
                        # Tester si ça améliore le score (ou au moins ne dégrade pas > 5%)
                        old = move_bat(cult, r, c, h, w, orientation)
                        recalc_producteurs(best)
                        s = score_incremental(best)
