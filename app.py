...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    def ffd_pure(nn_ordered, n_ordered):
        ...
        for b in nn_ordered:
            ...
            if is_top:
                # Barycentre mobile : se placer le plus près possible
                # des casernes déjà placées (ou du centre géo si première).
                # Chaque caserne rejoint le groupe existant -> cluster compact.
                if placed_top_prods:
                    _target_r = sum(p["r"]+p["rows"]/2 for p in placed_top_prods)/len(placed_top_prods)
                    _target_c = sum(p["c"]+p["cols"]/2 for p in placed_top_prods)/len(placed_top_prods)
                else:
                    _target_r, _target_c = max_r / 2.0, max_c / 2.0
                chosen = None; _best_d = 9999
                for r in range(max_r):
                    for c in range(max_c):
                        for rows, cols in [(b["rows"], b["cols"]), (b["cols"], b["rows"])]:
                            if can_place(r, c, rows, cols, x_grid, occ, max_r, max_c):
                                d = abs(r+rows/2-_target_r)+abs(c+cols/2-_target_c)
                                if d < _best_d:
