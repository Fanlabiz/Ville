...
if uploaded:
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if optionnels_a_placer:
            ...
            # ── Phase 2 : compaction ─────────────────────────────────────────────
            # Pour chaque passe: tester tous les HdV placés un par un.
            # OCC recalculée à chaque HdV testé (jamais périmée).
            # Dès qu'un gain est trouvé: l'appliquer et relancer depuis le début.
            # S'arrêter quand aucun HdV ne permet de gain.
            for _cp in range(100):
                _placed_c = {}
                ...
                for _h in list(b for b in optimized if b["nom"].strip() in _opt_quota):
                    # OCC recalculée à chaque _h pour tenir compte des gains précédents
                    ...
                    if _bm:
                        ...
                        _s_cur = score_placement(optimized)
                        _found_in_pass = True
                        break  # Relancer depuis le début avec OCC mise à jour
