    ...
    def calculate_culture(self):
        ...
        for pb in self.placed_buildings:
            if pb['info']['Type'] == 'Producteur':
                ...
                # On prend la première valeur non nulle dans l'empreinte (toutes devraient être égales)
                culture_recue = 0
                if footprint.size > 0:
                    # Parcourir toutes les cases de l'empreinte
                    for val in footprint.flatten():
                        if val > 0:
                            culture_recue = val
                            break
        ...
        for stat in prod_stats:
            if stat['Nom'] == 'Caserne de siège':
                ...
                for pb_siege in self.placed_buildings:
                    if pb_siege['info']['Nom'] == 'Caserne de siège':
                        r, c, h, w = pb_siege['r'], pb_siege['c'], pb_siege['h'], pb_siege['w']
