...
def placer_batiments_avec_backtracking(terrain, batiments):
    ...
    pile_placement = []
    index = 0
    max_essais = 500  # Limite pour éviter les boucles infinies
    essai = 0
    ...
    ajouter_log(f"=== BILAN INTERMÉDIAIRE ===")
...
if uploaded_file:
    try:
            ...
            with st.spinner("Optimisation en cours..."):
                try:
                    ...
                    if str(e) == "LIMITE_1000_ATTEINTE":
                        st.warning("⚠️ Limite de 1000 entrées journal atteinte - Affichage des résultats partiels")
                        
                        # Récupérer les résultats partiels
                        if 'terrain_optimise' in locals() and 'non_places' in locals():
                            stats = calculer_stats(terrain_optimise)
        ...
        if st.session_state.solution and st.session_state.optimisation_terminee:
            ...
            with col3:
                st.metric("Cases libres", np.sum(solution['terrain'].grille == 1))
            ...
            # Journal (avertissement si limite atteinte)
...
st.markdown("---")
st.markdown("🎯 Optimiseur de placement v3.1 - Arrêt à 1000 entrées avec résultats partiels")