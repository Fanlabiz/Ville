...
st.title(‘🏙️ Optimiseur de Ville’)
st.caption(‘Maximisation de la production par boost culturel’)
st.divider()
...
nom=nom, L=L, W=W, type=typ, culture=cult, ray=ray,
...
if b100 is not None and culture >= b100:
return 1.0
if b50 is not None and culture >= b50:
return 0.5
if b25 is not None and culture >= b25:
return 0.25
return 0.0
...
totals[p[‘prod’]] = totals.get(p[‘prod’], 0) + p[‘qte’] * (1 + boost)
score = 0
...
boost = compute_boost(c, p[‘b25’], p[‘b50’], p[‘b100’])
qty = p[‘qte’] * (1 + boost)
by_prod[p[‘prod’]] = by_prod.get(p[‘prod’], 0) + qty
...
if progress_cb:
    progress_cb(0.05, 'Phase 1 : echanges par paires...')
...
if progress_cb:
    progress_cb(0.45, 'Phase 2 : relocalisation des batiments culturels...')
...
while improved and pass_num < 30:
    ...
    if progress_cb:
        ...
        progress_cb(pct, 'Phase 2 : passe ' + str(pass_num) + '...')

if progress_cb:
    progress_cb(0.95, 'Calcul de la sequence des operations...')
...
def coord_label(r, c, h, w):
return ’Ligne ’ + str(r + 1) + ‘, Col ’ + str(c + 1) + ’  (’ + str(h) + ’L x ’ + str(w) + ‘C)’
...
moved.append({
‘pid’: pid, ‘nom’: po[‘nom’], ‘type’: po[‘type’],
...
while remaining:
    ...
    for m in list(remaining):
        ...
        if blocker_pid is None or blocker_pid in done:
            ...
            if m['pid'] in in_temp:
                steps.append({
                    'group': group_num, 'action': 'replacer',
                    'bat': m['nom'], 'type': m['type'],
                    'from': 'HORS TERRAIN',
                    'to':   coord_label(m['tr'], m['tc'], m['th'], m['tw']),
                })
                in_temp.discard(m['pid'])
            else:
                steps.append({
                    'group': group_num, 'action': 'placer',
                    'bat': m['nom'], 'type': m['type'],
                    'from': coord_label(m['fr'], m['fc'], m['fh'], m['fw']),
                    'to':   coord_label(m['tr'], m['tc'], m['th'], m['tw']),
                })
            done.add(m['pid'])
            remaining.remove(m)
    ...
    if not progress:
        ...
        group_num += 1
        steps.append({
            'group': group_num, 'action': 'retirer',
            'bat': m['nom'], 'type': m['type'],
            'from': coord_label(m['fr'], m['fc'], m['fh'], m['fw']),
...
def _hdr(cell, text, bg=‘FF1F497D’):
cell.value = text
cell.font  = Font(bold=True, color=‘FFFFFFFF’, name=‘Calibri’, size=9)
cell.fill  = PatternFill(‘solid’, start_color=bg)
cell.alignment = Alignment(horizontal=‘center’, vertical=‘center’, wrap_text=True)
cell.border = _thin_border()

def _dc(cell, value, bold=False, bg=None, align=‘left’, num_fmt=None):
cell.value = value
...
ACT_LBL = {
‘retirer’:  ‘1 - Retirer hors terrain’,
‘placer’:   ‘2 - Placer a destination’,
...
# ---- Onglet 2 : Resume production --------------------------------------
ws2 = wb.create_sheet('2-Resume Production')
ws2.freeze_panes = 'A2'
ws2.row_dimensions[1].height = 28
hdrs2 = ['Production', 'Nb batiments', 'Qte base/h', 'Qte optimisee/h',
         'Nb 0%', 'Nb 25%', 'Nb 50%', 'Nb 100%', 'Culture totale']
for ci, h in enumerate(hdrs2, 1):
    _hdr(ws2.cell(1, ci), h)
for ci, w in enumerate([16, 14, 13, 18, 8, 8, 8, 9, 18], 1):
...
for pid, p in placed_best.items():
    if p['type'] != 'Producteur' or p['prod'] == 'Rien':
        continue
    c    = cult_best.get(pid, 0)
    boost = compute_boost(c, p['b25'], p['b50'], p['b100'])
    pr   = p['prod']
    if pr not in s_best:
        s_best[pr] = dict(count=0, qte_base=0, qte_boost=0,
                          n0=0, n25=0, n50=0, n100=0, cult=0)
    s_best[pr]['count']     += 1
    s_best[pr]['qte_base']  += p['qte']
    s_best[pr]['qte_boost'] += p['qte'] * (1 + boost)
    s_best[pr]['cult']      += c
    s_best[pr][{0.0: 'n0', 0.25: 'n25', 0.5: 'n50', 1.0: 'n100'}[boost]] += 1
...
for pr in PROD_ORDER + [p for p in s_best if p not in PROD_ORDER]:
    if pr not in s_best:
        continue
    s = s_best[pr]
    vals = [pr, s['count'], round(s['qte_base'], 0), round(s['qte_boost'], 0),
            s['n0'], s['n25'], s['n50'], s['n100'], round(s['cult'], 0)]
    for ci, v in enumerate(vals, 1):
        nf = '#,##0' if ci in (3, 4, 9) else None
        _dc(ws2.cell(ri, ci), v, align='right' if ci > 1 else 'left', num_fmt=nf)
    ri += 1
...
hdrs3 = ['Production', 'Avant /h', 'Apres /h', 'Gain /h', 'Gain %']
...
for pr in PROD_ORDER + [p for p in best_prod if p not in PROD_ORDER and p != 'Rien']:
    ...
    vals = [pr, round(o, 0), round(b, 0), round(dlt, 0), round(pct, 1)]
    for ci, v in enumerate(vals, 1):
        nf = '#,##0' if ci in (2, 3, 4) else ('0.0' if ci == 5 else None)
        _dc(ws3.cell(ri, ci), v, bg=bg, align='right' if ci > 1 else 'left', num_fmt=nf)
    ri += 1

# ---- Onglet 4 : Batiments deplaces -------------------------------------
ws4 = wb.create_sheet('4-Batiments Deplaces')
ws4.freeze_panes = 'A2'
ws4.row_dimensions[1].height = 28
hdrs4 = ['Nom', 'Type', 'Avant Ligne', 'Avant Col', 'Avant Orient.',
         'Apres Ligne', 'Apres Col', 'Apres Orient.']
...
for ri, m in enumerate(moved_list, 2):
    ...
    vals = [m['nom'], m['type'],
            m['fr'] + 1, m['fc'] + 1, str(m['fh']) + 'L x ' + str(m['fw']) + 'C',
            m['tr'] + 1, m['tc'] + 1, str(m['th']) + 'L x ' + str(m['tw']) + 'C']
    for ci, v in enumerate(vals, 1):
        _dc(ws4.cell(ri, ci), v, bg=bg, align='right' if ci in (3, 4, 6, 7) else 'left')

ws4.cell(len(moved_list) + 3, 1).value = 'Total : ' + str(len(moved_list)) + ' batiment(s) deplace(s)'
ws4.cell(len(moved_list) + 3, 1).font  = Font(bold=True, name='Calibri', size=9)

# ---- Onglet 5 : Sequence des operations --------------------------------
...
c = ws5['A1']
c.value = ('SEQUENCE DES OPERATIONS -- ' + str(len(moved_list)) +
           ' batiments deplaces, ' + str(len(steps)) + ' etapes')
...
c2 = ws5['A2']
c2.value = ('Pour chaque echange : (1) retirer A hors terrain | '
            '(2) deplacer B a sa destination | (3) replacer A a sa destination.')
c2.font  = Font(italic=True, name='Calibri', size=9)
c2.alignment = Alignment(horizontal='left', vertical='center')

hdrs5 = ['Etape', 'Groupe', 'Action', 'Batiment', 'Type',
         'Position actuelle', 'Position finale', 'Note']
for ci, h in enumerate(hdrs5, 1):
    _hdr(ws5.cell(3, ci), h)
for ci, w in enumerate([7, 13, 26, 30, 12, 34, 34, 34], 1):
...
for si, step in enumerate(steps, 1):
    ...
    if step['action'] == 'retirer':
        note = 'Libere la place pour l echange ' + str(grp)
    elif step['action'] == 'placer':
        note = 'Position definitive'
    else:
        note = 'Position definitive (depuis hors terrain)'

    row_vals = [si, 'Groupe ' + str(grp), ACT_LBL[step['action']],
                step['bat'], step['type'], step['from'], step['to'], note]
    ...
    for ci, v in enumerate(row_vals, 1):
        ...
        if ci in (1, 2):
            ...
            _dc(cell, v,
                bg=TYPE_BG.get(step['type']) if ci == 5 else None,
                align='left')

lr = len(steps) + 5
ws5.cell(lr, 1).value = 'LEGENDE :'
ws5.cell(lr, 1).font  = Font(bold=True, name='Calibri', size=9)
leg = [
    ('1 - Retirer hors terrain',        'FFFEF3C7', 'Mettre de cote temporairement'),
    ('2 - Placer a destination',         'FFD1FAE5', 'Placer a la position definitive'),
    ('3 - Replacer depuis hors terrain', 'FFDBEAFE', 'Reprendre et placer definitivement'),
]
for i, (lbl, bg, desc) in enumerate(leg):
    r = lr + 1 + i
...
for col in range(cols_t):
    ws6.column_dimensions[get_column_letter(col + 2)].width = 6.5

for col in range(cols_t):
    cell = ws6.cell(1, col + 2, value=col + 1)
    ...
    for dr in range(p['h']):
        for dc in range(p['w']):
            ...
            label_grid[(p['r'] + dr, p['c'] + dc)] = {
                'name':  p['nom'] if is_tl else '',
                'type':  p['type'],
                'boost': blbl if (is_tl and p['type'] == 'Producteur'
                                  and p['prod'] != 'Rien') else '',
            }

FILL_DARK   = PatternFill('solid', start_color='FF333333')
FILL_CULT   = PatternFill('solid', start_color='FFFF9933')
FILL_PROD   = PatternFill('solid', start_color='FF66BB44')
FILL_NEUTR  = PatternFill('solid', start_color='FFCCCCCC')
FILL_WHITE  = PatternFill('solid', start_color='FFFFFFFF')

for r in range(rows_t):
    ws6.row_dimensions[r + 2].height = 11
    c0 = ws6.cell(r + 2, 1, value=r + 1)
    c0.font = Font(bold=True, name='Calibri', size=7)
    ...
    for c in range(cols_t):
        ...
        if not terrain[r, c]:
            ...
            info = label_grid[(r, c)]
            typ  = info['type']
            cell.fill = (FILL_CULT  if typ == 'Culturel'
                          else FILL_PROD  if typ == 'Producteur'
                          else FILL_NEUTR)
            txt = info['name']
            if info['boost']:
                txt = txt + '\n' + info['boost']
...
map_lr = rows_t + 4
ws6.cell(map_lr, 1, value='LEGENDE').font = Font(bold=True, name='Calibri', size=8)
for i, (lbl, fill, fc) in enumerate([
    ('Culturel',     FILL_CULT,  'FF000000'),
    ('Producteur',   FILL_PROD,  'FF000000'),
    ('Neutre',       FILL_NEUTR, 'FF000000'),
    ('Hors terrain', FILL_DARK,  'FFFFFFFF'),
]):
    c = ws6.cell(map_lr + 1 + i, 1, value=lbl)
    c.fill   = fill
    c.font   = Font(name='Calibri', size=8, color=fc)
...
with col_upload:
st.markdown(’### Fichier d entree’)
uploaded = st.file_uploader(
‘Depose ton fichier Excel ici’,
type=[‘xlsx’],
...
st.markdown(
‘- **Onglet Terrain** : grille de 1 (libre) et X (bord/occupe)\n’
’- **Onglet Batiments** : Nom, Longueur, Largeur, Nombre, Type, Culture, ’
‘Rayonnement, Boost 25%/50%/100%, Production, Quantite\n’
‘- **Onglet Actuel** : terrain peint avec les noms des batiments places’
)
...
```
    missing = {'Terrain', 'Batiments', 'Actuel'} - set(xl.sheet_names)
    if missing:
        st.error('Onglets manquants : ' + ', '.join(missing))
        st.stop()
    ...
    placed_orig = build_placed(parse_actuel(actuel_df), buildings)
    ...
    c1.metric('Terrain', str(rows_t) + ' x ' + str(cols_t))
    c2.metric('Cases libres', str(n_free))
    c3.metric('Batiments places', str(n_placed))
    c4.metric('Cult / Prod / Neutre',
              str(n_cult) + ' / ' + str(n_prod) + ' / ' + str(n_neutre))

    orig_prod, _, _ = compute_production_detail(placed_orig)
    st.markdown('**Production initiale (reference) :**')
    prod_cols = st.columns(min(len(orig_prod), 5))
    for i, (pr, qty) in enumerate(sorted(orig_prod.items(), key=lambda x: -x[1])):
        prod_cols[i % len(prod_cols)].metric(pr, '{:,.0f} /h'.format(qty))
    ...
    if st.button('Optimiser le placement', type='primary', use_container_width=True):
        ...
        prog_text.text('Optimisation terminee !')

        best_prod, _, _ = compute_production_detail(placed_best)
        steps, moved_list = build_sequence(placed_orig, placed_best, terrain)
        n_moved = len(moved_list)
        orig_score = compute_score(placed_orig)
        score_gain = ((best_score - orig_score) / orig_score * 100) if orig_score > 0 else 0

        st.divider()
        st.markdown('### Resultats de l optimisation')

        mc1, mc2, mc3 = st.columns(3)
        mc1.metric('Batiments deplaces', str(n_moved))
        mc2.metric('Etapes de manipulation', str(len(steps)))
        mc3.metric('Score global', ('+' if score_gain >= 0 else '') + '{:.1f}%'.format(score_gain))

        all_p = sorted(
            set(orig_prod) | set(best_prod),
            key=lambda x: (
                PROD_PRIORITY.index(x) if x in PROD_PRIORITY else 99,
                -best_prod.get(x, 0)
            )
        )
        table_data = []
        for pr in all_p:
            ...
            b   = best_prod.get(pr, 0)
            d   = b - o
            pt  = ('+' if d >= 0 else '') + '{:,.0f}'.format(d)
            pp  = ('+' if d >= 0 else '') + '{:.1f}%'.format(d / o * 100) if o > 0 else '--'
            table_data.append({
                'Production': pr,
                'Avant /h':   '{:,.0f}'.format(o),
                'Apres /h':   '{:,.0f}'.format(b),
        ...
        if n_moved > 0:
            st.markdown('**' + str(n_moved) + ' batiment(s) a deplacer :**')
            mv_data = [
                {
                    'Batiment': m['nom'],
                    'Type':     m['type'],
                    'De':       'L' + str(m['fr'] + 1) + '-C' + str(m['fc'] + 1),
                    'Vers':     'L' + str(m['tr'] + 1) + '-C' + str(m['tc'] + 1),
                }
                for m in moved_list
            ]
        ...
        st.divider()
        st.markdown('### Telecharger le resultat')
        ...
        st.download_button(
            label='Telecharger Ville_optimisee.xlsx',
            data=excel_bytes,
        ...
        st.success(
            'Fichier pret ! 6 onglets : batiments optimises, resume production, '
            'comparatif avant/apres, batiments deplaces, sequence des operations, '
            'carte du terrain.'
        )
...
st.info(‘Depose ton fichier Excel pour commencer l optimisation.’)
