...
class Terrain:
    def __init__(self, grid):
        ...
        self.cultural_zones = np.zeros_like(self.grid, dtype=float)  # Effet culturel cumulé
    ...
    def place_building(self, building, x, y, orientation):
        ...
        # Si c'est un bâtiment culturel, mettre à jour les zones culturelles
        if building.type == "culturel" and building.culture > 0:
            self.update_cultural_zones(building, x, y, orientation, longueur, largeur)
    
    def update_cultural_zones(self, building, x, y, orientation, longueur, largeur):
    ...
    def calculate_cultural_effect(self):
        """Recalcule toutes les zones culturelles"""
    ...
    def get_culture_for_building(self, building, x, y, orientation):
        ...
        # Version simplifiée: on prend la somme de la culture sur toutes les cases
        # C'est une approximation mais elle fonctionne
        total_culture = 0
        for i in range(longueur):
            for j in range(largeur):
...
def main():
    ...
    **Stratégie**:
    ...
    - Calcul précis de la culture reçue
    """)
