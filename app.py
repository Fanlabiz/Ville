...
class BuildingPlacer:
    def __init__(self, terrain, buildings):
        ...
        # Créer des listes d'instances avec multiplicités
            ...
            for row in range(self.terrain.height):
                for col in range(self.terrain.width):
                    if self.terrain.can_place_building(building, row, col, orientation):
                        ...
                        # Bonus pour les producteurs proches des culturels
                        if building.type == "Producteur":
                            ...
                            for cultural in cultural_buildings:
                                ...
                                for cell in cells:
                                    for cult_cell in cultural.get_cells():
                        ...
                        # Bonus pour les culturels avec espace autour
    ...
    def place_all_buildings_complete(self):
        """Place TOUS les bâtiments en priorisant les clusters"""
        ...
        # 2. Trier les culturels par valeur de rayonnement
        self.cultural_instances.sort(key=lambda x: -x.get_rayonnement_value())
        
        # 3. Trier les producteurs par priorité
        ...
        # 4. Placement alterné des culturels et producteurs
        used_cultural = set()
        used_producers = set()
        
        # Liste des producteurs à placer
        ...
        for cultural in self.cultural_instances:
            ...
            if pos:
                ...
                # Culturel non placé, on le garde pour plus tard
                pass
        
        # 5. Placer tous les producteurs restants
        for producer in remaining_producers:
        ...
        # 6. Placer tous les culturels restants
        for cultural in self.cultural_instances:
        ...
        # 8. Vérifier les bâtiments non placés
    ...
    def optimize_boosts(self):
        """Optimise les boosts après placement"""
        cultural_buildings = self.terrain.get_cultural_buildings()
        ...
        # Essayer d'améliorer
        for producer in weak_producers[:30]:
            ...
            for cultural in cultural_buildings:
                for orientation in ["horizontal", "vertical"]:
                    for dr in range(-cultural.building.rayonnement, cultural.building.rayonnement + 1):
                        for dc in range(-cultural.building.rayonnement, cultural.building.rayonnement + 1):
                            for r, c in cultural.get_cells():
                                test_r = r + dr
                                test_c = c + dc
                                if self.terrain.can_place_building(producer.building, test_r, test_c, orientation):
                                    # Tester la culture
                                    temp_producer = PlacedBuilding(producer.building, test_r, test_c, orientation)
                                    temp_culture = 0
                                    ...
                                    if temp_culture > best_culture:
                                        ...
                                        best_position = (test_r, test_c, orientation)
            
            if best_position and best_culture > producer.culture_recue * 1.1:
...
def create_output_excel(terrain, production_stats, unplaced_buildings):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # 4. Feuille des bâtiments non placés (TOUJOURS créée, même si vide)
        unplaced_data = []
...
def main():
    st.set_page_config(page_title="Placeur de Bâtiments - Version Complète", layout="wide")
    
    st.title("🏗️ Placeur de Bâtiments - Version Complète")
    st.markdown("Chargez un fichier Excel pour placer **TOUS** les bâtiments en formant des clusters")
    ...
    if uploaded_file is not None:
        try:
            ...
            with col1:
                optimize = st.checkbox("⚡ Optimiser les boosts", value=True)
            
            if st.button("🚀 Lancer le placement complet", type="primary"):
                with st.spinner("Placement de tous les bâtiments..."):
                    terrain = Terrain(terrain_grid)
                    placer = BuildingPlacer(terrain, buildings)
                    
                    # Placement complet de tous les bâtiments
                    placer.place_all_buildings_complete()
                    
                    # Optimisation des boosts si demandée
                    if optimize and len(placer.terrain.buildings) > 0:
                    ...
                    if len(placer.unplaced) == 0:
                        ...
                        # Afficher les bâtiments non placés
                    ...
                    st.download_button(
                        ...
                        data=output_file,
                        file_name="resultats_placement_complet.xlsx",
