...
if uploaded:
    ...
    # Assess theoretical potential: for each producer, what's the max culture reachable?
    producers_info = []
    culturels_all = [b for b in original_placed if b['type'] == 'Culturel' and b['culture'] > 0]
    for b in original_placed:
        if b['type'] != 'Producteur' or b['boost25'] is None:
            continue
        cur_cr = orig_culture_map.get(id(b), {}).get('culture', 0)
        cur_boost = get_boost(b, cur_cr)
        # Max culture = sum of ALL culturels (if they could all cover this producer)
        max_possible = sum(c['culture'] for c in culturels_all)
        max_boost = get_boost(b, max_possible)
        producers_info.append({
            'name': b['name'], 'prod': b['production'],
            'cur_culture': cur_cr, 'cur_boost': cur_boost,
            'max_boost': max_boost, 'boost25': b['boost25'],
        })

    # Show potential warning if no producer can ever get boosted
    any_boost_possible = any(p['max_boost'] > 0 for p in producers_info)
    if not any_boost_possible and producers_info:
        st.warning(
            "⚠️ **Aucun boost possible avec ce terrain** : même en plaçant tous les bâtiments "
            "culturels au plus près, aucun producteur n'atteint le seuil de culture minimum "
            f"pour un boost (seuil min: {min(p['boost25'] for p in producers_info):,.0f}). "
            f"La culture totale disponible est de {sum(c['culture'] for c in culturels_all):,.0f}. "
            "L'optimisation sera lancée mais ne pourra pas améliorer les boosts."
        )
    elif producers_info:
        cur_boosted = sum(1 for p in producers_info if p['cur_boost'] > 0)
        pot_boosted = sum(1 for p in producers_info if p['max_boost'] > 0)
        if pot_boosted > cur_boosted:
            st.info(
                f"💡 **Potentiel d'optimisation détecté** : actuellement {cur_boosted} producteur(s) "
                f"boosté(s), jusqu'à {pot_boosted} pourraient l'être après optimisation."
            )
    ...
    # ── Boost status table ─────────────────────────────────────────────
    if producers_info:
        with st.expander("📊 État des boosts actuels par producteur"):
            boost_rows = []
            for p in producers_info:
                boost_rows.append({
                    'Bâtiment': p['name'], 'Production': p['prod'],
                    'Culture reçue': p['cur_culture'],
                    'Boost actuel': f"{p['cur_boost']}%",
                    'Boost max théorique': f"{p['max_boost']}%",
                    'Seuil 25%': p['boost25'],
                })
            st.dataframe(pd.DataFrame(boost_rows), use_container_width=True)
    ...
    if st.button("🚀 Lancer l'optimisation", type="primary"):
        ...
        orig_score = score_placement(original_placed)
        new_culture_map = compute_culture_received(best)
        new_prod = compute_total_production(best, new_culture_map)
        gain_pct = (best_score - orig_score) / orig_score * 100 if orig_score > 0 else 0

        # Count moved buildings
        n_moved = sum(
            1 for i, b in enumerate(best)
            if b['row'] != original_placed[i]['row'] or b['col'] != original_placed[i]['col']
        )
        # Count boost improvements
        orig_boosts = {}
        for b in original_placed:
            cr = orig_culture_map.get(id(b), {}).get('culture', 0)
            orig_boosts[id(b)] = get_boost(b, cr)

        n_boost_improved = 0
        n_boost_degraded = 0
        orig_by_name2 = {}
        for b in original_placed:
            orig_by_name2.setdefault(b['name'], []).append(b)
        used2 = {n: 0 for n in orig_by_name2}
        for b in best:
            if b['type'] != 'Producteur': continue
            cr_new = new_culture_map.get(id(b), {}).get('culture', 0)
            boost_new = get_boost(b, cr_new)
            idx = used2.get(b['name'], 0)
            olist = orig_by_name2.get(b['name'], [])
            if idx < len(olist):
                ob = olist[idx]
                used2[b['name']] = idx + 1
                cr_old = orig_culture_map.get(id(ob), {}).get('culture', 0)
                boost_old = get_boost(ob, cr_old)
                if boost_new > boost_old: n_boost_improved += 1
                elif boost_new < boost_old: n_boost_degraded += 1

        if gain_pct > 0.01:
            status.success(f"✅ Optimisation terminée — gain de **{gain_pct:.1f}%** | "
                           f"{n_moved} bâtiment(s) déplacé(s) | "
                           f"{n_boost_improved} boost(s) amélioré(s)")
        elif n_moved > 0:
            status.warning(f"⚠️ Optimisation terminée — {n_moved} déplacement(s) effectué(s) "
                           f"mais aucun gain de production (boosts inchangés).")
        else:
            status.warning(
                "⚠️ **Aucun déplacement effectué** — le placement actuel est déjà optimal "
                "pour ce terrain. Cela peut arriver si les régions jouables sont trop éloignées "
                "des zones de rayonnement culturel, ou si la culture totale disponible "
                f"({sum(c['culture'] for c in culturels_all):,.0f}) est insuffisante pour "
                f"atteindre le seuil de boost minimum des producteurs."
            )
