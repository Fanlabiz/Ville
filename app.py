...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        """
        # Dictionnaire pour suivre les bâtiments déjà traités
        processed = set()
        ...
        for r in range(min(len(actuel_data), self.rows)):
            for c in range(min(len(actuel_data[0]), self.cols)):
                ...
                # Vérifier si cette cellule a déjà été traitée
                cell_key = f"{r},{c}"
                if cell_key in processed:
                    continue
                
                # Chercher les dimensions du bâtiment en explorant autour
                # Par défaut, on suppose 1x1
                w, h = 1, 1
                
                # Explorer vers la droite pour trouver la largeur
                ...
                # Explorer vers le bas pour trouver la hauteur
                ...
                # Marquer toutes les cellules de ce bâtiment comme traitées
                for dr in range(h):
                    for dc in range(w):
                        if r + dr < self.rows and c + dc < self.cols:
                            processed.add(f"{r + dr},{c + dc}")
                
                # Marquer la grille comme occupée
                self.grid[r:r+h, c:c+w] = 0
                
                # Enregistrer le bâtiment temporairement
                self.placed_buildings.append({
                    ...
                    'w': w,
                    'h': h,
                    'info': None  # Sera mis à jour plus tard
                })
