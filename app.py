...
def main():
    ...
    if st.button("🚀 Lancer", type="primary"):
        with st.spinner("Placement..."):
            ...
            # Placement des culturels : on les met près du centre
            for i, b in enumerate(cult):
                pos = terrain.get_all_positions(b)
                if pos:
                    # Pour le premier, au centre
                    if i == 0:
                        cx, cy = terrain.width // 2, terrain.height // 2
                        best = min(pos, key=lambda p: abs(p[0] + b.longueur//2 - cx) + abs(p[1] + b.largeur//2 - cy))
                    else:
                        # Pour les suivants, près du premier
                        if terrain.cultural_buildings:
                            ref = terrain.cultural_buildings[0]
                            best = min(pos, key=lambda p: abs(p[0] + b.longueur//2 - ref['center_x']) + 
                                                   abs(p[1] + b.largeur//2 - ref['center_y']))
                        else:
                            best = pos[0]
                    
                    terrain.place_building(b, best[0], best[1], best[2])
                    st.write(f"✅ {b.nom} à ({best[0]},{best[1]})")
                else:
                    st.write(f"❌ Impossible de placer {b.nom}")
            
            # Calculer la carte des zones
            ...
            st.write("🗺️ Zones de rayonnement (nombre de culturels):")
            st.dataframe(pd.DataFrame(zone_map), use_container_width=True)
            
            # Placer les producteurs
            st.write(f"🏭 Placement de {len(prod)} producteurs")
            ...
            for b in prod:
                ...
                for x, y, o in terrain.get_all_positions(b):
                    ...
                    # Compter le max de culturels touchant ce bâtiment
                    max_count = 0
                    for i in range(l):
                        for j in range(L):
                            px, py = x + i, y + j
                            if 0 <= px < terrain.width and 0 <= py < terrain.height:
                                max_count = max(max_count, zone_map[py, px])
                    
                    # Estimer la culture
                    if max_count > 0:
                        cultures = sorted([cb['culture'] for cb in terrain.cultural_buildings], reverse=True)
                        culture = sum(cultures[:max_count])
                    else:
                        culture = 0
                    
                    score = max_count * 10000 + culture
