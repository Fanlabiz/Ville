...
def create_output_excel(terrain, production_stats, unplaced_buildings):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Définir la hauteur des lignes (en points)
        row_height = 45
        
        # Appliquer la hauteur à toutes les lignes
        for i in range(1, terrain.height + 1):
            sheet.row_dimensions[i].height = row_height
        
        # Appliquer la largeur des colonnes pour qu'elles soient carrées
        # La largeur en Excel est en unités de caractères, approximativement 1/7 de point
        # Pour une hauteur de 45 points, une largeur d'environ 7.14 caractères donne une cellule carrée
        col_width = 7.14
        for j in range(1, terrain.width + 1):
            sheet.column_dimensions[get_column_letter(j)].width = col_width
        
        # Appliquer les couleurs et fusionner les cellules pour chaque bâtiment
        for pb in terrain.buildings:
            cells = pb.get_cells()
            if not cells:
                continue
            
            # Trier les cellules pour obtenir la cellule en haut à gauche
            cells_sorted = sorted(cells, key=lambda x: (x[0], x[1]))
            start_cell = cells_sorted[0]
            
            # Calculer la hauteur et la largeur du bâtiment dans la grille
            rows = [r for r, c in cells]
            cols = [c for r, c in cells]
            height = max(rows) - min(rows) + 1
            width = max(cols) - min(cols) + 1
            
            # Convertir en indices Excel (1-indexed)
            start_row = start_cell[0] + 1
            start_col = start_cell[1] + 1
            end_row = start_row + height - 1
            end_col = start_col + width - 1
            
            # Obtenir les références Excel
            ...
            # Appliquer la couleur et le texte à la cellule fusionnée
        ...
        # Colorier les cases libres et les bordures (uniquement les cellules non fusionnées)
        for i in range(terrain.height):
            for j in range(terrain.width):
...
def main():
    ...
    st.title("🏗️ Placeur de Bâtiments - Cellules Carrées")
    st.markdown("Chargez un fichier Excel pour placer les bâtiments avec des cellules carrées")
            ...
            if st.button("🚀 Lancer le placement", type="primary"):
                with st.spinner("Placement de tous les bâtiments..."):
                    ...
                    st.download_button(
                        ...
                        data=output_file,
                        file_name="resultats_placement_carres.xlsx",
