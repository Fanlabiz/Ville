...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        Version corrigée avec détection robuste des dimensions
        """
        ...
        for r in range(len(actuel_str)):
            for c in range(len(actuel_str[0])):
                ...
                # D'abord, chercher la plus grande zone rectangulaire possible
                # avec le même nom
                max_w = 1
                ...
                # Maintenant, on va tester différentes tailles de rectangle
                # pour trouver la plus grande zone rectangulaire valide
                found_valid = False
                best_w, best_h = 1, 1
                
                # Tester toutes les combinaisons possibles de dimensions
                for test_h in range(1, max_h + 1):
                    for test_w in range(1, max_w + 1):
                        # Vérifier que le rectangle est valide
                        valid = True
                        for dr in range(test_h):
                            for dc in range(test_w):
                        ...
                        if valid:
                            # Ce rectangle est valide, on le garde comme candidat
                            # On prend le plus grand rectangle valide
                            if test_h * test_w > best_h * best_w:
...
- **Détection robuste** des bâtiments existants (évite les dimensions impossibles)
