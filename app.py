...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    if best_n_failed > 0:
        ...
        # Pénalité uniquement pour les bâtiments OBLIGATOIRES manquants.
        # En mode "score maximal" (force_complete=False), les bâtiments Optionnels
        # peuvent être laissés de côté sans pénalité.
        # En mode "placement complet" (force_complete=True), tous sont obligatoires.
        ...
        _score_with_penalty_fb   = _fb_score   - _fb_mandatory_fails   * _penalty_per_fail
        _score_with_penalty_best = _best_score - _best_mandatory_fails  * _penalty_per_fail
...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    if not real_moves:
        ...
        # On construit une liste d'opérations élémentaires en tenant compte des conflits :
        # si la position cible d'un bâtiment est occupée par un autre bâtiment initial,
        # ce dernier doit être mis en réserve temporairement avant d'être replacé.

        # Grille des positions initiales (cellule -> bâtiment initial)
        _init_cells = {}
        for b in original_placed:
            for dr in range(b["rows"]):
                for dc in range(b["cols"]):
        ...
        # État courant des positions libérées au fil des opérations
        # (cellules qui ont été vidées par un déplacement déjà effectué)
        _freed_cells = set()   # cellules libérées
        _reserve = {}          # nom -> (target_r, target_c) des bâtiments en réserve

        # Construire un index "nom → position finale" pour tous les déplacés
        _final_pos = {mv["nom"]: mv for mv in real_moves}
        ...
        def _is_target_free(mv):
            """Vrai si toutes les cellules cibles sont libres (dans l'état courant)."""
            tc = _target_cells(mv)
            # Libre = était vide initialement OU a été libérée par un déplacement précédent
            init_occupied = tc - _freed_cells
            for cell in init_occupied:
                if cell in _init_cells and _init_cells[cell]["nom"] != mv["nom"]:
                    return False
            return True

        def _blockers(mv):
            """Bâtiments initiaux qui occupent la cible et n'ont pas encore été déplacés."""
            tc = _target_cells(mv)
            seen = {}
            for cell in tc - _freed_cells:
                if cell in _init_cells:
                    b = _init_cells[cell]
                    if b["nom"] != mv["nom"] and b["nom"] not in seen:
                        seen[b["nom"]] = b
            return list(seen.values())

        # Générer la séquence d'opérations
        operations = []   # liste de dicts: type, nom, from, to, note
        pending = list(real_moves)  # déplacements encore à faire
        done_noms = set()
        iterations = 0

        while pending and iterations < len(real_moves) * 4:
            iterations += 1
            progress = False
            remaining = []
            ...
            for mv in pending:
                ...
                if not blockers:
                    # Cible libre: on peut déplacer directement
                    old_str = f"{get_column_letter(mv['old_c']+1)}{mv['old_r']+1}"
                    new_str = f"{get_column_letter(mv['new_c']+1)}{mv['new_r']+1}"
                    note = ""
                    if mv["nom"] in _reserve:
                        note = "(retour de réserve)"
                    operations.append({
                        "type": "move",
                        "nom": mv["nom"],
                        "from": old_str if mv["nom"] not in _reserve else "Réserve",
                        "to": new_str,
                        "note": note,
                    })
                    # Libérer les cellules source
                    _freed_cells.update(_source_cells(mv))
                    # Marquer les cellules cibles comme occupées (retirer de freed si elles y étaient)
                    _freed_cells -= _target_cells(mv)
                    done_noms.add(mv["nom"])
                    if mv["nom"] in _reserve:
                        del _reserve[mv["nom"]]
                    progress = True
                else:
                    # Cible bloquée: mettre les bloqueurs en réserve
                    for blocker in blockers:
                        bnom = blocker["nom"]
                        if bnom not in _reserve and bnom not in done_noms:
                            ...
                            # Libérer les cellules du bloqueur
                            for dr in range(blocker["rows"]):
                                for dc in range(blocker["cols"]):
                                    _freed_cells.add((blocker["r"]+dr, blocker["c"]+dc))
                            _reserve[bnom] = blocker
                            # Si le bloqueur doit lui-même être déplacé,
                            # mettre à jour son "old" pour indiquer qu'il vient de la réserve
                            if bnom in _final_pos:
                                _final_pos[bnom] = dict(_final_pos[bnom])
                        progress = True  # on a progressé (mis en réserve)
                    remaining.append(mv)
        ...
        # Bâtiments en réserve qui n'ont pas encore été replacés
        for bnom, blocker in list(_reserve.items()):
            if bnom in _final_pos and bnom not in done_noms:
                mv = _final_pos[bnom]
                new_str = f"{get_column_letter(mv['new_c']+1)}{mv['new_r']+1}"
                operations.append({
                    "type": "move",
        ...
        for si, op in enumerate(operations):
            ...
            if op["type"] == "reserve":
                action = (f"Mettre '{op['nom']}' en réserve (retirer de {op['from']}) "
                          f"{op['note']}")
            else:
                action = (f"Placer '{op['nom']}' en {op['to']}"
...
if uploaded:
    ...
    temps_max_min = st.slider(
        "Temps d'attente maximum (minutes)",
        min_value=1, max_value=10, value=3,
        help=(
            "Le script s'arrête dès que ce temps est atteint.\n\n"
            "• 1 min : résultat rapide\n"
            "• 3 min : bon compromis\n"
            "• 5 min : meilleur score\n"
            "• 10 min : optimal pour les grands terrains"
        )
    )
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        # Placer les bâtiments OPTIONNELS post-optimisation avec push algorithm.
        # Le push déplace des Neutres/petits Culturels si nécessaire pour libérer un bloc.
        if optionnels_a_placer:
            x_grid_post = make_x_grid(terrain_grid, max_r, max_c)

            def _build_occ_push(pl, excl=None):
                excl = excl or set()
                occ = [[False]*max_c for _ in range(max_r)]
                for b in pl:
                    if id(b) in excl: continue
                    for dr in range(b["rows"]):
                        for dc in range(b["cols"]):
                            rr,cc=b["r"]+dr,b["c"]+dc
                            if 0<=rr<max_r and 0<=cc<max_c: occ[rr][cc]=True
                return occ

            def _push_place_opt(result, target_b):
                """Place target_b en poussant Neutres/petits Culturels si besoin."""
                cell_map = {}
                for b in result:
                    for dr in range(b["rows"]):
                        for dc in range(b["cols"]):
                            cell_map[(b["r"]+dr,b["c"]+dc)] = b

                for tr, tc in [(target_b["rows"],target_b["cols"]),
                               (target_b["cols"],target_b["rows"])]:
                    occ = _build_occ_push(result)
                    best_block = None; best_sc = float("inf")
                    for r in range(max_r-tr+1):
                        for c in range(max_c-tc+1):
                            if any(x_grid_post[r+dr][c+dc]
                                   for dr in range(tr) for dc in range(tc)):
                                continue
                            blockers = {}
                            for dr in range(tr):
                                for dc in range(tc):
                                    if occ[r+dr][c+dc]:
                                        b = cell_map.get((r+dr,c+dc))
                                        if b: blockers[id(b)] = b
                            if not blockers:
                                return {**target_b,"r":r,"c":c,"rows":tr,"cols":tc}
                            blist = list(blockers.values())
                            # Autoriser push: Neutres ET Culturels surface<=4
                            if all(b["type"]=="Neutre" or
                                   (b["type"]=="Culturel" and b["rows"]*b["cols"]<=4)
                                   for b in blist):
                                sc = sum(b["rows"]*b["cols"] for b in blist)
                                if sc < best_sc:
                                    best_sc=sc; best_block=(r,c,tr,tc,blist)

                    if not best_block: continue
                    r,c,tr2,tc2,blist = best_block
                    reserve = {(r+dr,c+dc) for dr in range(tr2) for dc in range(tc2)}
                    all_moved = True
                    for blocker in blist:
                        result[:] = [b for b in result if b is not blocker]
                        occ_w = _build_occ_push(result)
                        placed_b = False
                        for rows2,cols2 in [(blocker["rows"],blocker["cols"]),
                                            (blocker["cols"],blocker["rows"])]:
                            if placed_b: break
                            for rr in range(max_r):
                                if placed_b: break
                                for cc in range(max_c):
                                    if not can_place(rr,cc,rows2,cols2,
                                                     x_grid_post,occ_w,max_r,max_c): continue
                                    if {(rr+dr,cc+dc) for dr in range(rows2)
                                        for dc in range(cols2)} & reserve: continue
                                    blocker["r"]=rr; blocker["c"]=cc
                                    blocker["rows"]=rows2; blocker["cols"]=cols2
                    ...
                    if all_moved:
                        return {**target_b,"r":r,"c":c,"rows":tr2,"cols":tc2}
                return None

            for _opt_b in sorted(optionnels_a_placer, key=lambda b: -(b["rows"]*b["cols"])):
                _result_b = _push_place_opt(optimized, _opt_b)
                if _result_b:
                    optimized.append(_result_b)
