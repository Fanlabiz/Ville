...
class CityPlanner:
    def __init__(self, terrain_data):
        ...
        # 1 = Libre, 0 = Occupé, X = Obstacle/Bord
...
def generate_excel(planner, full_queue):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # 5, 6, 7. Stats Finales
        placed_indices = [id(p['info']) for p in planner.placed_buildings]
        not_placed = [b for b in full_queue if id(b) not in placed_indices]
        ...
        summary = [
            ...
            ["Statut final", "LIMITE JOURNAL ATTEINTE" if planner.interrupted else "ALGO TERMINE"]
        ]
