...
VERSION = "v2.2-compaction-double"
...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    _vc = ws2.cell(_ver_row, 1, "✅ Script version: v2.2-compaction-double")
...
if uploaded:
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if optionnels_a_placer:
            ...
            _n_hdv_avant_compact = sum(1 for b in optimized if b["nom"].strip() in _opt_quota)
        ...
        _n_hdv_final = sum(1 for b in optimized if b.get("placement","").strip().lower()=="optionnel")
        status.success(f"Optimisation terminée ! Script v2.1-compaction | {_n_hdv_final} optionnels placés")
