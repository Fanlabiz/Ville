...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    for r in range(max_r):
        for c in range(max_c):
            ...
            if terrain_grid[r][c] == "X":
                ...
                # Bord épais si le bâtiment a été déplacé, fin sinon
                cell.border = thick_border() if id(b) in moved_bat_ids else thin_border()
                if b["r"] == r and b["c"] == c:
                    cult = culture_received(b, culturels) if b["type"] == "Producteur" else 0
                    boost = boost_level(cult, b)
                    label = b["nom"]
    ...
    for i, (label, color) in enumerate([
        ...
        cell.border = thin_border()
        cell.alignment = Alignment(horizontal="center")
    # Note sur les bords épais
    ws4.cell(leg_r+4, 1, "Bord épais = bâtiment déplacé").font = Font(italic=True)
