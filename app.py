...
if uploaded:
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if optionnels_a_placer:
            ...
            # ── Phase 2 : compaction locale ───────────────────────────────
            # Déplacer un optionnel déjà placé de ±DR cases pour libérer
            # un bloc contigu où loger un optionnel non encore placé.
            # Score conservé ou amélioré. Converge en quelques passes (<1s/passe).
            _DR = 8
            for _cp in range(20):
                _placed_c = Counter(b["nom"].strip() for b in optimized)
                ...
                for _h in [b for b in optimized if b["nom"].strip() in _opt_quota]:
                    if _found: break
                    _wo = [b for b in optimized if b is not _h]
                    _ow = make_occ_grid(_wo, max_r, max_c)
                    for _nr in range(max(0,_h["r"]-_DR), min(max_r-_h["rows"]+1,_h["r"]+_DR+1)):
                        if _found: break
                        for _nc in range(max(0,_h["c"]-_DR), min(max_c-_h["cols"]+1,_h["c"]+_DR+1)):
                                ...
                                for _r4 in range(max_r - _rw2 + 1):
                                    ...
                                    for _c4 in range(max_c - _cl2 + 1):
                                        if can_place(_r4,_c4,_rw2,_cl2,_xg_opt,_om,max_r,max_c):
                                            optimized = _mv + [{**_un0,"r":_r4,"c":_c4,
                                                                "rows":_rw2,"cols":_cl2}]
                                            _found = True; break
                if not _found:
                    break
