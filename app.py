...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    Strategie :
    - First-Fit Decreasing (FFD) : les batiments sont tries par taille
      decroissante pour eviter la fragmentation du terrain. Les grands
      batiments occupent d'abord les zones libres, les petits remplissent
      les espaces restants.
    - n_trials essais avec des ordres legerement differents (melange aleatoire
      à l'interieur de chaque groupe de taille identique).
    - On garde le placement qui donne le meilleur score initial, car c'est
      la meilleure base de depart pour l'optimiseur.

    Retourne (new_placed, n_placed, n_failed) :
      new_placed  : liste complete (anciens + nouveaux), meilleur essai
    ...
    def ffd_place_one(ordered):
        """
        FFD raster scan : pour chaque batiment dans l'ordre donne,
        prend la premiere case disponible (balayage haut->bas, gauche->droite).
        """
    ...
    # Trier par taille decroissante (FFD de base)
    ...
    for trial in range(n_trials):
        # Melanger aleatoirement à l'interieur de chaque groupe de taille identique
        ...
        # Preferer un essai qui place plus de batiments, puis meilleur score init
        culturels_trial = [b for b in result if b["type"] == "Culturel"]
        s_init = sum(
            boost_level(culture_received(b, culturels_trial), b) / b["priorite"]
            if b["priorite"] > 0 else 0
            for b in result if b["type"] == "Producteur"
        )

        if (n_fail < best_n_failed or
                (n_fail == best_n_failed and s_init > best_score_init)):
            best_placed = result
            best_score_init = s_init
            best_n_placed = n_ok
