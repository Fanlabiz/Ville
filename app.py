                ...
                for r1, c1_, h1, w1, ori1 in cult_pos.get(ci1, []):
                    ...
                    for r2, c2_, h2, w2, ori2 in cult_pos.get(ci2, []):
                        ...
                        if s > best_score + 1e-6 or (new_boost > old_boost and s >= best_score * 0.90):
                            ...
                            # Recalc complet pour garantir un état cohérent avant de continuer
                            recalc_all_numpy(best)
                            best_score = max(best_score, score_fast(best))
                            accepted_c2 = True
                            break   # on garde c1 ET c2 déplacés, on passe à la paire suivante
                        else:
                            restore_bat(c2_bat, old2)
                            # Après restore c2, state[ci2] == old2 → recalc_two_cults avec
                            # old_pos = position tentée (r2,c2_,h2,w2) pour correctement
                            # remettre à zéro les producteurs de la zone tentée
                            recalc_two_cults(best, ci1, old1[:4], ci2, (r2, c2_, h2, w2))
                    ...
                    if accepted_c2:
                        ...
                        # Après restore c1, state[ci1] == old1 → recalc avec
                        # old_pos = position tentée (r1,c1_,h1,w1)
                        recalc_producteurs(best, moved_cult=ci1, old_pos=(r1, c1_, h1, w1))
