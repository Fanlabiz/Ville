...
if uploaded:
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        # Placer les bâtiments OPTIONNELS dans les espaces libres post-optimisation
        if optionnels_a_placer:
            ...
            for _opt_b in sorted(optionnels_a_placer, key=lambda b: -(b["rows"] * b["cols"])):
                _occ_post = make_occ_grid(optimized, max_r, max_c)
                _ok_opt = False
                for _rows, _cols in [(_opt_b["rows"], _opt_b["cols"]),
                                      (_opt_b["cols"], _opt_b["rows"])]:
                    if _ok_opt: break
                    for _r in range(max_r):
                        if _ok_opt: break
                        for _c in range(max_c):
                            if can_place(_r, _c, _rows, _cols, x_grid_post, _occ_post, max_r, max_c):
                                optimized.append({**_opt_b, "r": _r, "c": _c,
                                                  "rows": _rows, "cols": _cols})
                                _ok_opt = True; break
