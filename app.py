...
class Terrain:
    def __init__(self, grid):
        ...
        self.cultural_zone = np.zeros_like(self.grid, dtype=int)
    ...
    def get_radiation_zone_size_in_terrain(self, building, x, y, orientation):
        ...
        Retourne un score élevé si toute la zone est dans le terrain.
        """
        l, L = building.get_dimensions(orientation)
        rayon = building.rayonnement
        ...
        if (x - rayon >= 0 and x + l + rayon <= self.width and
            ...
            # Zone entièrement dans le terrain - score maximal
            return 1000000  # Score très élevé pour prioriser absolument
    ...
    def get_best_cultural_position(self, building):
        ...
        for x, y, o in positions:
            # 1. Zone de rayonnement dans le terrain (PRIORITÉ ABSOLUE)
            ...
            if not creates_corridors:
                # Séparer les positions avec zone complète
                if zone_score >= 1000000:  # Zone entièrement dans le terrain
                    if zone_score > best_zone_in_terrain:
                        best_zone_in_terrain = zone_score
                        best_zone_pos = (x, y, o)
                else:
                    # Pour les positions sans zone complète, on considère l'impact
        ...
        # STRATÉGIE CORRIGÉE :
        # 1. PRIORITÉ MAXIMALE : zone entièrement dans le terrain SANS couloirs
...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Priorité à la zone dans le terrain")
    ...
    if st.button("🚀 Lancer", type="primary"):
        with st.spinner("Placement..."):
            ...
            st.write("🔄 Algorithme alterné - Priorité à la zone dans le terrain")
            ...
            while cult or prod:
                ...
                if cult:
                    ...
                    if best_pos:
                        ...
                        zone_complete = "✅" if rayon >= 1000000 else f"{rayon} cases"
                        
                        has_corridors = terrain.has_width_1_corridors()
                        corridor_msg = "⚠️ avec couloirs" if has_corridors else "✅ sans couloirs"
                        
                        st.write(f"  ✅ à ({best_pos[0]},{best_pos[1]}) - zone: {zone_complete}, impact: {existing_impact} producteurs {corridor_msg}")
                    else:
                ...
                if prod:
                    ...
                    if places_dans_cycle == 0 and iteration == 1:
