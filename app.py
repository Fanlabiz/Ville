...
def optimize_no_reduction(placed, terrain_grid, max_r, max_c, progress_cb=None):
    ...
    N'accepte que les swaps qui augmentent au moins un boost producteur
    sans en diminuer aucun.

    Les priorités sont prises en compte de deux façons :
    1. Tri des candidats : les swaps sont évalués en prioritisant ceux qui
       concernent les producteurs à haute priorité (non encore à 100%).
    2. Score de gain pondéré : un swap est préféré si le gain total pondéré
       par priorité (Σ Δboost × priorité) est plus élevé.
    La contrainte "sans réduction" reste absolue : aucun boost ne peut baisser.
    """
    x_grid = make_x_grid(terrain_grid, max_r, max_c)
    placed = [dict(b) for b in placed]
    all_moves = []
    ...
    # Vérifie si des priorités sont définies (sinon on reste en mode neutre)
    _has_priorities = any(b.get("priorite", 0) > 0 for b in producteurs)

    def get_boosts():
        """Retourne la liste des boosts actuels pour chaque producteur."""
        return [boost_level(culture_received(p, culturels), p) for p in producteurs]

    def get_weighted_score():
        """Score pondéré par priorité : Σ boost × priorité."""
        return sum(
            boost_level(culture_received(p, culturels), p) * max(p.get("priorite", 1), 1)
            for p in producteurs
        )

    def try_swap(b1, b2, boosts_before):
        """
        Tente un swap b1 ↔ b2 (toutes orientations).
        Retourne (True, gain_pondere) si un swap sans régression améliore au moins
        un boost, ou (False, 0) sinon. Applique le meilleur swap trouvé.
        """
        old1 = (b1["r"], b1["c"], b1["rows"], b1["cols"])
        old2 = (b2["r"], b2["c"], b2["rows"], b2["cols"])
        occ = make_occ_grid(placed, max_r, max_c, exclude_ids={id(b1), id(b2)})

        score_before = get_weighted_score()
        best_gain = 0.0
        best_config = None  # (new1_tuple, new2_tuple)

        # Générer toutes les orientations possibles pour b1 et b2
        ...
        for r1_new, c1_new in b1_orients:
            for r2_new, c2_new in b2_orients:
                # b1 → position de b2, b2 → position de b1
                if not can_place(old2[0], old2[1], r1_new, c1_new, x_grid, occ, max_r, max_c):
                    continue
                if not can_place(old1[0], old1[1], r2_new, c2_new, x_grid, occ, max_r, max_c):
                    continue
                # Vérifier absence de chevauchement mutuel
                _cells1 = {(old2[0]+dr, old2[1]+dc) for dr in range(r1_new) for dc in range(c1_new)}
                _cells2 = {(old1[0]+dr, old1[1]+dc) for dr in range(r2_new) for dc in range(c2_new)}
                if _cells1 & _cells2:
                    continue

                b1["r"], b1["c"], b1["rows"], b1["cols"] = old2[0], old2[1], r1_new, c1_new
                b2["r"], b2["c"], b2["rows"], b2["cols"] = old1[0], old1[1], r2_new, c2_new

                ba = get_boosts()
                regression  = any(a < bef for a, bef in zip(ba, boosts_before))
                progression = any(a > bef for a, bef in zip(ba, boosts_before))

                if not regression and progression:
                    gain = get_weighted_score() - score_before
                    if gain > best_gain:
                        best_gain = gain
                        best_config = (
                            (old2[0], old2[1], r1_new, c1_new),
                            (old1[0], old1[1], r2_new, c2_new),
                        )
    ...
    # Trier les culturels par culture décroissante (les plus puissants en premier)
    culturels_sorted = sorted(culturels, key=lambda c: -c.get("culture", 0))

    # Ordre de priorité pour les producteurs : haute priorité non-saturés d'abord
    def _prod_priority_key(p):
        prio = p.get("priorite", 0) if _has_priorities else 1
        cult = culture_received(p, culturels)
        boost = boost_level(cult, p)
        # Non saturé (< 100%) et haute priorité → en premier
        return (-(prio if boost < 100 else 0), -prio)
    ...
    while improved and n_passes < 20:
        ...
        # Trier les producteurs à chaque passe selon priorité/saturation courante
        producteurs_ordered = sorted(producteurs, key=_prod_priority_key)

        # Swaps culturel ↔ culturel (triés par culture décroissante)
        for i, c1 in enumerate(culturels_sorted):
            for c2 in culturels_sorted[i+1:]:
                tick()
                ok, gain = try_swap(c1, c2, boosts_before)
                if ok:
        ...
        # Swaps culturel ↔ producteur — producteurs haute priorité d'abord
        for prod in producteurs_ordered:
            for cult in culturels_sorted:
                tick()
                ok, gain = try_swap(cult, prod, boosts_before)
                if ok:
...
if 'placed' in dir() and 'original_placed' in dir():
    ...
    _mode_optim = st.radio(
        ...
        horizontal=True,
        help=(
            "**Avec réduction possible** : déplace librement les bâtiments culturels pour "
            "maximiser le score pondéré par priorité (casernes, fermes, maisons…). "
            "Certains producteurs peuvent temporairement perdre du boost si le gain global est supérieur.\n\n"
            "**Sans réduction** : n'accepte que les échanges qui améliorent au moins un boost "
            "sans en réduire aucun. Les priorités guident l'ordre de recherche : "
            "les producteurs à haute priorité non saturés sont traités en premier, "
            "et les swaps les plus bénéfiques pour eux sont préférés.\n\n"
            "**Pas d'optimisation** : génère l'Excel sans déplacer aucun bâtiment."
        )
    )
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        # En mode sans réduction : calculer les boosts avant/après avec info priorités
        ...
        if _mode_optim == "Sans réduction":
            ...
            def _count_boosts_by_prio(pl, label):
                """Retourne {prio: {boost_level: count}} pour les producteurs à priorité > 0."""
                cults = [b for b in pl if b["type"] == "Culturel"]
                result = {}
                for p in pl:
                    if p["type"] != "Producteur" or p.get("priorite", 0) <= 0:
                        continue
                    prio = p["priorite"]
                    bl = boost_level(culture_received(p, cults), p)
                    if prio not in result:
                        result[prio] = {0: 0, 25: 0, 50: 0, 100: 0}
                    result[prio][bl] += 1
                return result

            bc_before = _count_boosts(placed_for_optim)
            bc_after  = _count_boosts(optimized)
            bp_before = _count_boosts_by_prio(placed_for_optim, "before")
            bp_after  = _count_boosts_by_prio(optimized, "after")
            boost_delta_info = (bc_before, bc_after, bp_before, bp_after)
...
if st.session_state.result_excel is not None:
    ...
    if bd is not None:
        bc_before, bc_after = bd[0], bd[1]
        bp_before, bp_after = (bd[2], bd[3]) if len(bd) == 4 else ({}, {})
        gains = []
        ...
        # Tableau détaillé par priorité (si des priorités existent)
        if bp_before or bp_after:
            all_prios = sorted(set(list(bp_before.keys()) + list(bp_after.keys())), reverse=True)
            if all_prios:
                with st.expander("📊 Détail des gains par priorité"):
                    for prio in all_prios:
                        av = bp_before.get(prio, {0: 0, 25: 0, 50: 0, 100: 0})
                        ap = bp_after.get(prio,  {0: 0, 25: 0, 50: 0, 100: 0})
                        gains_p = []
                        for seuil in [25, 50, 100]:
                            diff = ap[seuil] - av[seuil]
                            if diff > 0:
                                gains_p.append(f"+{diff} à {seuil}%")
                            elif diff < 0:
                                gains_p.append(f"{diff} à {seuil}%")
                        gain_str = " · ".join(gains_p) if gains_p else "aucun changement"
                        st.write(f"**Priorité {int(prio)}** — {gain_str}")
    st.divider()
    st.download_button(
