...
class Terrain:
    ...
    def calculate_cultural_effect(self):
        """Calcule l'effet culturel de tous les bâtiments avec la bonne formule"""
        self.cultural_zones = np.zeros_like(self.grid, dtype=float)
        
        st.write("🔍 Calcul de l'effet culturel...")
        ...
        for building, x, y, orientation, longueur, largeur in self.buildings:
            if building.type == "culturel" and building.culture > 0:
                ...
                st.write(f"  - {building.nom}: centre=({center_x},{center_y}), rayon={building.rayonnement}, culture={building.culture}")
        ...
        # Afficher un aperçu de la zone culturelle
        st.write("Aperçu de la zone culturelle (premières lignes):")
        preview = pd.DataFrame(self.cultural_zones[:10, :10])
        st.dataframe(preview)
        
        return self.cultural_zones
    
    def get_culture_at_position(self, x, y, longueur, largeur):
        """Calcule la culture totale sur une position"""
        total = 0
        for i in range(longueur):
            for j in range(largeur):
                if 0 <= y+j < self.height and 0 <= x+i < self.width:
                    total += self.cultural_zones[y + j, x + i]
        return total
    ...
    def get_production_boosts(self):
        ...
        st.write("📊 Calcul des boosts de production:")
        ...
        for building, x, y, orientation, longueur, largeur in self.buildings:
            if building.type == "producteur" and building.production:
                ...
                # Calculer la culture reçue (somme, pas moyenne)
                total_culture = self.get_culture_at_position(x, y, longueur, largeur)
                
                st.write(f"  - {building.nom}: culture reçue = {total_culture:.2f}")
                st.write(f"    Seuils: 25%={building.boost_25}, 50%={building.boost_50}, 100%={building.boost_100}")
...
class BuildingPlacer:
    ...
    def find_optimal_cluster_for_building(self, building):
        """Trouve un cluster de cases libres assez grand pour le bâtiment"""
        best_score = -1
        best_position = None
        
        # Pour les grands bâtiments, chercher dans les zones libres
    ...
    def evaluate_position(self, building, x, y, orientation):
        ...
        if building.type == "producteur" and building.production:
            # Pour les producteurs, maximiser la culture reçue
            total_culture = self.terrain.get_culture_at_position(x, y, longueur, largeur)
    ...
    def place_all(self):
        ...
        st.write(f"  - Culturels: {len(cultural_buildains)} types")
        ...
        for building in cultural_buildings:
            ...
            for i in range(building.quantite):
                ...
                # Pour les culturels, chercher des positions qui maximisent la couverture future
                best_pos = self.find_optimal_cluster_for_building(building)
        ...
        for building in producer_buildings:
            ...
            for i in range(building.quantite):
                ...
                if best_pos:
                    ...
                    culture = self.terrain.get_culture_at_position(x, y, 
...
def main():
    ...
    **Stratégie**:
    ...
    - Calcul précis de la culture (somme, pas moyenne)
    """)
    ...
    if uploaded_file:
        ...
        with st.expander("ℹ️ Comment ça marche"):
            ...
            **Stratégie de placement**:
            ...
            3. **Regroupement** : L'algorithme cherche à garder les cases libres groupées
