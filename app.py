...
def create_buildings(df):
    ...
    for _, row in df.iterrows():
        try:
            q = 1
            if quantite := find(col_map['quantite']):
                val = row[quantite]
...
def main():
    ...
    with col2:
        ...
        for b in buildings:
            data.append({
                "Nom": b.nom, "Type": b.type, "Production": b.production,
                "Dimensions": f"{b.longueur}x{b.largeur}", "Quantité": b.quantite,
    ...
    if st.button("🚀 Lancer l'optimisation", type="primary"):
        with st.spinner("Placement en cours..."):
            ...
            culturels = [b for b in buildings if b.type == "culturel"]
            producteurs = [b for b in buildings if b.type == "producteur" and b.production]
            
            # Créer les exemplaires
            tous = []
            for b in culturels:
                for i in range(b.quantite):
                    new = Building(b.nom, b.longueur, b.largeur, 1, b.type, b.culture,
                                  b.rayonnement, b.boost_25, b.boost_50, b.boost_100, b.production)
                    new.id = f"{b.nom}_{i}"
                    tous.append(new)
            ...
            # Trier: culturels d'abord (par rayon), puis producteurs (par priorité)
            culturels_liste = [b for b in tous if b.type == "culturel"]
            producteurs_liste = [b for b in tous if b.type == "producteur"]
            ...
            culturels_liste.sort(key=lambda b: (-b.rayonnement, -b.culture))
            producteurs_liste.sort(key=priorite)
            
            ordre = culturels_liste + producteurs_liste
            ...
            for i, b in enumerate(ordre):
                ...
                if meilleure_pos:
                    ...
                    if b.type == "producteur":
                        ...
                        st.write(f"✅ {b.nom} à ({x},{y}) → {c:.0f} culture")
                    else:
            ...
            with col1:
                st.metric("Placés", f"{sum(b.placed for b in buildings)}/{sum(b.quantite for b in buildings)}")
            with col2:
            ...
            for b, x, y, o, l, L in terrain_final.buildings:
                if b.nom not in symbols:
                    symbols[b.nom] = '🔵' if b.type == 'culturel' else '🔴'
                for i in range(l):
                    for j in range(L):
