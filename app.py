    ...
    def update_culture(self, cultural_buildings):
        ...
        for cultural in cultural_buildings:
            if cultural != self and cultural.building.type == "Culturel":
                # Vérifier si le batiment producteur est dans la zone de rayonnement
...
def read_input_excel(file):
    ...
    try:
        current_df = pd.read_excel(xls, sheet_name="Actuel", header=None)
        # On peut fusionner avec le terrain initial si nécessaire
    except:
        pass
...
def create_output_excel(terrain, production_stats, unplaced_buildings):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        extra_data = pd.DataFrame({
            'Statistiques': ['Cases libres restantes', 'Cases des batiments non places'],
            'Valeur': [terrain.get_free_cells_count(), sum(b.get_area() for b in unplaced_buildings)]
        })
...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("🚀 Lancer le placement des bâtiments"):
                with st.spinner("Placement des bâtiments en cours..."):
                    ...
                    for prod_type, stats in production_stats.items():
                        with st.expander(f"{prod_type}"):
                            st.write(f"Culture totale: {stats['total_culture']:.0f}")
                            st.write(f"Boost moyen: {stats['avg_boost']:.1f}%")
                            st.write(f"Nombre de bâtiments: {stats['count']}")
                            st.write(f"Production/heure: {stats['total_production']:.2f}")
