...
Version avancée avec optimisation globale
"""
    ...
    def calculate_culture(self):
        ...
        for producteur in self.building_instances:
            if producteur.building.type == 'Producteur':
                ...
                producteur.culture_recue = total_culture
                producteur.boost_percentage = producteur.get_boost_score() * 25
        ...
        for building in self.building_instances:
            if building.building.type == 'Producteur':
                ...
                if prod_type and prod_type != '':
                    production = building.building.quantite * (1 + building.boost_percentage / 100)
...
class AdvancedOptimizer:
    ...
    def evaluate_cultural_position(self, cultural_instance, producteurs, terrain_size):
        """
        Évalue une position pour un bâtiment culturel
        Calcule le nombre de producteurs potentiellement boostés
        """
        ...
        for producteur in producteurs:
            producteur_cells = set(producteur.get_occupied_cells())
            if producteur_cells & rayonnement_cells:
                # Plus de points pour les producteurs à haute priorité
                priority_map = {'Guérison': 100, 'Nourriture': 10, 'Or': 1}
                priority = priority_map.get(producteur.building.production, 0)
                score += (1 + priority / 10)
    ...
    def evaluate_position_score(self, instance, cultural_instances, terrain_size):
        ...
        if instance.building.type == 'Producteur':
            ...
            return boost_score * 100 + priority + instance.building.priorite
    ...
    def optimize(self):
        ...
        # Créer une grille d'importance pour le placement des culturels
        importance_grid = np.zeros((self.terrain.height, self.terrain.width))
        
        # Préparer tous les producteurs à placer
        all_producteurs = []
        for building in producteur_buildings:
            for _ in range(building.nombre):
                all_producteurs.append(building)
        ...
        for building in cultural_buildings:
            for idx in range(building.nombre):
                ...
                for orientation, x, y in positions:
                    ...
                    # Évaluer la position en fonction des producteurs à placer
                    temp_producteurs = []
                    for prod_building in all_producteurs[:min(100, len(all_producteurs))]:  # Échantillon pour performance
                        # Créer une instance temporaire pour évaluation
                        temp_prod = BuildingInstance(prod_building, 0, 0, 'h')
                        temp_producteurs.append(temp_prod)
                    
                    score = self.evaluate_cultural_position(instance, temp_producteurs, (self.terrain.height, self.terrain.width))
                    
                    # Bonus pour être au centre (meilleure couverture)
                    center_x = self.terrain.height / 2
                    center_y = self.terrain.width / 2
                    distance_to_center = abs(x + building.longueur/2 - center_x) + abs(y + building.largeur/2 - center_y)
                    center_bonus = (self.terrain.height + self.terrain.width) / distance_to_center if distance_to_center > 0 else 1000
                    
                    score += center_bonus / 10
        ...
        for building in producteur_buildings:
            for _ in range(building.nombre):
                ...
                for orientation, x, y in positions:
                    instance = BuildingInstance(building, x, y, orientation)
                    score = self.evaluate_position_score(instance, cultural_instances, (self.terrain.height, self.terrain.width))
        ...
        while improved and iterations < max_iterations:
            ...
            # Essayer d'améliorer chaque producteur
            for idx, instance in enumerate(self.terrain.building_instances):
                ...
                old_score = self.evaluate_position_score(instance, cultural_instances, (self.terrain.height, self.terrain.width))
                
                # Enlever temporairement le bâtiment
                ...
                for orientation, x, y in positions:
                    ...
                    score = self.evaluate_position_score(test_instance, cultural_instances, (self.terrain.height, self.terrain.width))
                ...
                if best_position and best_score > old_score:
                    ...
                    if self.terrain.place_building(instance):
                        self.moves.append({
                            ...
                            'apres_x': x,
                            'apres_y': y,
                            'apres_orientation': orientation
                        })
                        improved = True
                else:
                    # Remettre le bâtiment
...
class ExcelExporter:
    ...
    def export_results(terrain, optimizer):
        ...
        for instance in terrain.building_instances:
            buildings_data.append({
                ...
                'Production/heure': f"{instance.building.quantite * (1 + instance.boost_percentage / 100):.2f}"
            })
    ...
    def _create_terrain_sheet(ws: Worksheet, terrain: Terrain):
        ...
        # Créer un dictionnaire des bâtiments
        buildings_by_cell = {}
        for instance in terrain.building_instances:
            cells = instance.get_occupied_cells()
            for cell in cells:
            ...
            for j in range(terrain.width):
                ...
                if (i, j) in buildings_by_cell:
                    ...
                    if (i, j) == first_cell:
                        ...
                        # Cellule non fusionnée mais occupée
                        cell.value = ""
                else:
                    cell.value = ""
        ...
        try:
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    st.warning(f"Ligne {idx + 2}: Erreur - {str(e)}")
...
def main():
    ...
    **Améliorations apportées :**
    - 🎯 **Placement optimal des culturels** : Les bâtiments culturels sont placés au centre pour maximiser la couverture
    - 🔄 **Amélioration itérative** : Ajustement continu des positions pour obtenir les meilleurs boosts
    - 📊 **Priorisation des productions** : Guérison > Nourriture > Or
    - 💡 **Calcul précis des rayonnements** : Prise en compte des limites du terrain
        ...
        try:
            ...
            with st.spinner("Optimisation avancée en cours..."):
                ...
                if stats:
                    ...
                    with col2:
                        st.subheader("📈 Pourcentage d'augmentation")
                        gains_df = pd.DataFrame([
                            {
                                'Type': k,
                                'Augmentation': f"{((v['total'] - v['quantite']) / v['quantite'] * 100):.1f}%" if v['quantite'] > 0 else "N/A",
                                'Nb bâtiments': sum([1 for b in terrain.building_instances if b.building.type == 'Producteur' and b.building.production == k])
                            }
                            for k, v in stats.items()
                        ])
                        st.dataframe(gains_df)
                
                with st.expander("🏠 Visualisation détaillée des bâtiments"):
                    buildings_placed = []
                    for instance in terrain.building_instances:
                        buildings_placed.append({
                            ...
                            'Culture recue': instance.culture_recue,
                            'Boost': f"{instance.boost_percentage}%",
                            'Production/h': f"{instance.building.quantite * (1 + instance.boost_percentage / 100):.2f}"
                        })
                ...
                st.download_button(
                    ...
                    data=excel_data,
                    file_name="resultats_optimisation_avancee.xlsx",
                ...
                3. Consultez l'onglet **Terrain_final** pour voir le placement optimisé
                4. Utilisez l'onglet **Sequence_operations** pour les déplacements
                """)
                
        except Exception as e:
            st.error(f"❌ Erreur: {str(e)}")
            st.info("Vérifiez le format de votre fichier Excel")
