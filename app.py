...
try:
    ...
    def compute_culture_map(g, placed_list):
        ...
        for b in placed_list:
            if b.get('Type') != 'Culturel' or not b.get('placed'):
                continue
    ...
    def guerison_culture_score(placed_list, culture_map):
        total = 0.0
        for b in placed_list:
            if b.get('Production') != 'Guerison' or not b.get('placed'):
                continue
            if 'row' not in b or 'col' not in b or 'rotation' not in b:
                continue
    ...
    strong_cult = [
        b for b in placed
        if b['Type'] == 'Culturel' and b.get('placed') and 'row' in b and 'col' in b
    ]
    for building in strong_cult:
        ...
        for try_rot in [False, True]:
            ...
            for nr, nc, _ in cands:
                if not can_place(grid_work, nr, nc, hh, ww, False):
                    continue
    ...
    for b in placed:
        if b['Type'] != 'Producteur' or b.get('Production') != 'Guerison':
            continue
        if 'row' not in b:
            continue
        r,c,rot = b['row'], b['col'], b['rotation']
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # 4. Terrain visuel – fusion corrigée
        wb = writer.book
        ...
        orange_fill  = PatternFill("solid", start_color="FF9900")
        green_fill   = PatternFill("solid", start_color="00CC44")
        gray_fill    = PatternFill("solid", start_color="AAAAAA")
        ...
        for b in placed:
            ...
            fill_color = orange_fill if b['Type']=='Culturel' else green_fill if b['Type']=='Producteur' else gray_fill

            txt = str(b['Nom'])[:15]
            if b['Type']=='Producteur' and 'boost' in b:
                txt += f"\n{int(b['boost']*100)}%"

            tl_col = openpyxl.utils.get_column_letter(c + 1)
            tl_row = r + 1
            tl = f"{tl_col}{tl_row}"
            ...
            # ÉCRITURE UNIQUEMENT sur la cellule en haut à gauche
            top_left_cell = ws[tl]
            top_left_cell.value = txt
            top_left_cell.fill = fill_color
            top_left_cell.alignment = center_align
            top_left_cell.font = bold_font

            # Style (couleur) sur toutes les cellules de la zone fusionnée
            for ri in range(r, r + h):
                for ci in range(c, c + w):
                    ws.cell(row=ri + 1, column=ci + 1).fill = fill_color

        # Ajustement colonnes / lignes
