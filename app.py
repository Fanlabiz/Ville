...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    # Ordre : Producteurs -> Culturels -> Neutres, taille decroissante dans chaque groupe
    ...
    for trial in range(n_trials):
        ...
        # A l'interieur de chaque groupe (meme type + meme taille), melange aleatoire
        groups = []
        for _, g in groupby(sorted_base,
                             key=lambda b: (priority.get(b["type"], 9), b["rows"] * b["cols"])):
            grp = list(g)
            random.shuffle(grp)
            groups.append(grp)
        ...
        result, n_ok, n_fail = smart_place_one(ordered, seed=trial * 17 + 3)
