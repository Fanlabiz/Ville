...
class ExcelExporter:
    ...
    def _create_terrain_sheet(ws: Worksheet, terrain: Terrain):
        ...
        # Premier passage: Créer toutes les cellules avec les X
        ...
        # Calculer les positions réelles des bâtiments
        building_cells = {}
        for instance in terrain.building_instances:
            cells = instance.get_occupied_cells()
            if cells:
                ...
                for (x, y) in cells:
                    ...
                    if 0 <= real_i < total_height and 0 <= real_j < total_width:
                        building_cells[(real_i, real_j)] = instance
        
        # Trouver les cellules de départ (min pour chaque bâtiment)
        building_start_cells = {}
        processed_instances = set()
        for (i, j), instance in building_cells.items():
        ...
        # Appliquer les fusions et le contenu
        for (i, j), instance in building_start_cells.items():
            ...
            if (i > 0 and j > 0 and 
                i + height - 1 < total_height - 1 and 
                j + width - 1 < total_width - 1):
                ...
                # Définir la couleur et le contenu
