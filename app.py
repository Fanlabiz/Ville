...
def _csv_building_category(nom_complet):
    ...
    if "BARRACKS" in n:
        return "Barracks"
    if "FARM" in n or "CAMELFARMED" in n or "MERCHANT" in n \
            or "IRRIGATION" in n or "CAMEL" in n:
        return "Farm"
    if "HOME" in n:
        return "Home"
    if "WORKSHOP" in n or "SMITHY" in n or "FORGE" in n \
...
def read_from_csv(file_obj, city_name=None,
                  prio_barracks=0, prio_farm=0, prio_home=0, prio_workshop=0):
    """
    ...
    prio_barracks  : priorité appliquée aux bâtiments Producteurs de type Barracks
    prio_farm      : priorité appliquée aux bâtiments Producteurs de type Farm
    prio_home      : priorité appliquée aux bâtiments Producteurs de type Home
    prio_workshop  : priorité appliquée aux bâtiments Producteurs de type Workshop
    ...
    for _, row in df_real.iterrows():
        ...
        if key not in catalog_rows:
            ...
            # Priorité selon la catégorie fonctionnelle (uniquement pour les Producteurs)
            if btype == "Producteur":
                _cat = _csv_building_category(key)
                if _cat == "Barracks":
                    _prio = float(prio_barracks)
                elif _cat == "Farm":
                    _prio = float(prio_farm)
                elif _cat == "Home":
                    _prio = float(prio_home)
                elif _cat == "Workshop":
                    _prio = float(prio_workshop)
                else:
                    _prio = 0.0
            else:
                _prio = 0.0
...
def optimize_no_reduction(placed, terrain_grid, max_r, max_c, progress_cb=None):
    """
    Optimisation 'sans réduction' — trois mécanismes combinés :

    1. Déplacements vers cases libres (free moves)
       Un culturel peut se déplacer vers n'importe quelle case libre du terrain,
       pas seulement échanger sa position avec un autre bâtiment.
       Condition : au moins un boost augmente, aucun ne diminue.

    2. Swaps bilatéraux (culturel ↔ culturel, culturel ↔ producteur)
       Échange de positions entre deux bâtiments.
       Même condition stricte sans réduction.

    3. Chaînes de 3 (multi-swap)
       Si un culturel A veut aller à une position P mais qu'un bâtiment B
       bloque le mouvement (B partirait en régression), on tente d'abord de
       déplacer B vers une case libre compatible, puis de déplacer A vers P.
       On n'accepte la chaîne que si le résultat final est sans régression.

    4. Filtrage par zone de rayonnement (accélération)
       Seuls les culturels dont la zone de rayonnement peut atteindre un
       producteur sous-optimal (pas encore à 100%) sont traités activement.
       Les autres (qui ne couvrent que des producteurs déjà saturés) sont
       ignorés dans les passes coûteuses.

    Les priorités guident l'ordre : les producteurs haute priorité non saturés
    sont traités en premier. Le gain pondéré par priorité sert à choisir le
    meilleur parmi plusieurs mouvements valides.
    """
    x_grid = make_x_grid(terrain_grid, max_r, max_c)
    placed = [dict(b) for b in placed]
    all_moves = []
    ...
    _has_priorities = any(b.get("priorite", 0) > 0 for b in producteurs)
    ...
    def is_no_regression(boosts_after, boosts_before):
        return not any(a < b for a, b in zip(boosts_after, boosts_before))

    def has_progression(boosts_after, boosts_before):
        return any(a > b for a, b in zip(boosts_after, boosts_before))

    def weighted_gain(boosts_after, boosts_before):
        """Gain total pondéré par priorité."""
        return sum(
            (ba - bb) * max(p.get("priorite", 1), 1)
            for ba, bb, p in zip(boosts_after, boosts_before, producteurs)
        )

    # ── Point 4 : filtrage par zone ──────────────────────────────────────────
    def _undersaturated_cells():
        """Ensemble des cases appartenant à des producteurs non encore à 100%."""
        cells = set()
        for p in producteurs:
            if boost_level(culture_received(p, culturels), p) < 100:
                cells |= cells_of(p)
        return cells

    def _cult_can_reach_target(cult, target_cells):
        """
        Un culturel 'peut atteindre' une cible s'il existe au moins une position
        (y compris sa position actuelle) d'où son rayonnement couvre au moins
        une case de target_cells.
        On estime cela en vérifiant si la distance Manhattan entre le centre du
        culturel et le centre de target_cells est ≤ rayonnement + demi-diagonale
        du bâtiment + rayon de la cible.  C'est une borne supérieure large —
        on préfère sur-inclure que rater des cas.
        """
        ray = cult.get("rayonnement", 1)
        cr = cult["r"] + cult["rows"] / 2.0
        cc = cult["c"] + cult["cols"] / 2.0
        for (tr, tc) in target_cells:
            if abs(cr - tr) <= ray + cult["rows"] + 2 and \
               abs(cc - tc) <= ray + cult["cols"] + 2:
                return True
        return False

    # ── Point 1 : déplacement vers case libre ────────────────────────────────
    def try_free_move(cult, boosts_before):
        """
        Tente de déplacer 'cult' vers la meilleure case libre.
        Retourne True et applique le mouvement si un gain sans régression existe.
        Stratégie : évaluer toutes les cases libres, choisir celle dont le gain
        pondéré est maximal (et > 0 avec au moins une progression).
        """
        old = (cult["r"], cult["c"], cult["rows"], cult["cols"])
        occ = make_occ_grid(placed, max_r, max_c, exclude_ids={id(cult)})
        ...
        for rows, cols in {(cult["rows"], cult["cols"]), (cult["cols"], cult["rows"])}:
            for r in range(max_r - rows + 1):
                for c in range(max_c - cols + 1):
                    if (r, c, rows, cols) == old:
                        continue
                    if not can_place(r, c, rows, cols, x_grid, occ, max_r, max_c):
                        continue
                    cult["r"], cult["c"], cult["rows"], cult["cols"] = r, c, rows, cols
                    ba = get_boosts()
                    cult["r"], cult["c"], cult["rows"], cult["cols"] = old
                    if is_no_regression(ba, boosts_before) and has_progression(ba, boosts_before):
                        g = weighted_gain(ba, boosts_before)
                        if g > best_gain:
                            best_gain = g
                            best_pos  = (r, c, rows, cols)
        ...
        if best_pos:
            all_moves.append({
                "nom":   cult.get("nom_fr") or cult["nom"],
                "old_r": old[0], "old_c": old[1],
                "new_r": best_pos[0], "new_c": best_pos[1],
            })
            cult["r"], cult["c"], cult["rows"], cult["cols"] = best_pos
            return True
        return False

    # ── Point 2 : swap bilatéral ─────────────────────────────────────────────
    def try_swap(b1, b2, boosts_before):
        """
        Tente toutes les orientations de swap b1 ↔ b2.
        Retourne True et applique le meilleur swap sans régression.
        """
        old1 = (b1["r"], b1["c"], b1["rows"], b1["cols"])
        old2 = (b2["r"], b2["c"], b2["rows"], b2["cols"])
        occ  = make_occ_grid(placed, max_r, max_c, exclude_ids={id(b1), id(b2)})

        b1_orients = [(b1["rows"], b1["cols"])]
        if b1["rows"] != b1["cols"]:
            b1_orients.append((b1["cols"], b1["rows"]))
        b2_orients = [(b2["rows"], b2["cols"])]
        if b2["rows"] != b2["cols"]:
            b2_orients.append((b2["cols"], b2["rows"]))

        best_gain   = 0.0
        best_config = None

        for r1n, c1n in b1_orients:
            for r2n, c2n in b2_orients:
                if not can_place(old2[0], old2[1], r1n, c1n, x_grid, occ, max_r, max_c):
                    continue
                if not can_place(old1[0], old1[1], r2n, c2n, x_grid, occ, max_r, max_c):
                    continue
                _cells1 = {(old2[0]+dr, old2[1]+dc) for dr in range(r1n) for dc in range(c1n)}
                _cells2 = {(old1[0]+dr, old1[1]+dc) for dr in range(r2n) for dc in range(c2n)}
                if _cells1 & _cells2:
                    continue
                b1["r"], b1["c"], b1["rows"], b1["cols"] = old2[0], old2[1], r1n, c1n
                b2["r"], b2["c"], b2["rows"], b2["cols"] = old1[0], old1[1], r2n, c2n
                ba = get_boosts()
                ...
                if is_no_regression(ba, boosts_before) and has_progression(ba, boosts_before):
                    ...
                    if g > best_gain:
                        best_gain   = g
                        best_config = (
                            (old2[0], old2[1], r1n, c1n),
                            (old1[0], old1[1], r2n, c2n),
                        )

        if best_config:
            b1["r"], b1["c"], b1["rows"], b1["cols"] = best_config[0]
            b2["r"], b2["c"], b2["rows"], b2["cols"] = best_config[1]
            return True
        return False

    # ── Point 3 : chaîne de 3 ────────────────────────────────────────────────
    def try_chain3(cult_a, boosts_before):
        """
        Tente : déplacer cult_a vers une position P occupée par un bâtiment B,
        après avoir d'abord déplacé B vers une case libre compatible.
        N'accepte la chaîne que si le résultat final est sans régression
        et avec au moins une progression.

        On teste uniquement les positions P dont la zone de rayonnement de
        cult_a couvrirait un producteur non saturé (filtrage point 4).
        """
        old_a   = (cult_a["r"], cult_a["c"], cult_a["rows"], cult_a["cols"])
        target_cells = _undersaturated_cells()

        # Liste des bâtiments candidats à "pousser" (culturels et neutres seulement —
        # on évite de pousser un producteur car cela changerait ses boosts reçus)
        pushables = [b for b in placed
                     if b is not cult_a
                     and b["type"] in ("Culturel", "Neutre")]

        best_gain   = 0.0
        best_result = None   # (new_pos_a, pushed_b, new_pos_b)

        for b in pushables:
            old_b = (b["r"], b["c"], b["rows"], b["cols"])

            # Est-ce que la zone de cult_a depuis la position de b couvrirait
            # un producteur sous-optimal ? (filtrage rapide)
            cult_a["r"], cult_a["c"] = old_b[0], old_b[1]
            zone_candidate = radiation_zone(cult_a)
            cult_a["r"], cult_a["c"] = old_a[0], old_a[1]
            if not (zone_candidate & target_cells):
                continue

            # Vérifier les orientations de cult_a à la position de b
            for r_an, c_an in {(cult_a["rows"], cult_a["cols"]),
                                (cult_a["cols"], cult_a["rows"])}:
                # Peut-on placer cult_a à la position de b ?
                occ_ab = make_occ_grid(placed, max_r, max_c,
                                       exclude_ids={id(cult_a), id(b)})
                if not can_place(old_b[0], old_b[1], r_an, c_an,
                                 x_grid, occ_ab, max_r, max_c):
                    continue

                # Chercher une case libre pour b (après avoir retiré cult_a et b)
                for r_bn, c_bn in {(b["rows"], b["cols"]), (b["cols"], b["rows"])}:
                    for rb in range(max_r - r_bn + 1):
                        found_b = False
                        for cb in range(max_c - c_bn + 1):
                            # La nouvelle position de b ne doit pas être
                            # la position actuelle de b ni celle de cult_a,
                            # ni les cases que cult_a va occuper
                            if (rb, cb) == (old_b[0], old_b[1]):
                                continue
                            if not can_place(rb, cb, r_bn, c_bn,
                                             x_grid, occ_ab, max_r, max_c):
                                continue
                            # Pas de chevauchement entre nouvelle pos b et nouvelle pos cult_a
                            _ca_cells = {(old_b[0]+dr, old_b[1]+dc)
                                         for dr in range(r_an) for dc in range(c_an)}
                            _b_cells  = {(rb+dr, cb+dc)
                                         for dr in range(r_bn) for dc in range(c_bn)}
                            if _ca_cells & _b_cells:
                                continue

                            # Simuler : appliquer les deux mouvements et vérifier
                            cult_a["r"], cult_a["c"], cult_a["rows"], cult_a["cols"] = \
                                old_b[0], old_b[1], r_an, c_an
                            b["r"], b["c"], b["rows"], b["cols"] = rb, cb, r_bn, c_bn

                            ba = get_boosts()

                            cult_a["r"], cult_a["c"], cult_a["rows"], cult_a["cols"] = old_a
                            b["r"], b["c"], b["rows"], b["cols"] = old_b

                            if (is_no_regression(ba, boosts_before) and
                                    has_progression(ba, boosts_before)):
                                g = weighted_gain(ba, boosts_before)
                                if g > best_gain:
                                    best_gain   = g
                                    best_result = (
                                ...
                                found_b = True
                                break  # on a trouvé une position pour b, inutile d'en chercher d'autres
                        if found_b:
                            break

        if best_result:
            new_pos_a, pushed_b, new_pos_b = best_result
            old_b_saved = (pushed_b["r"], pushed_b["c"],
                           pushed_b["rows"], pushed_b["cols"])
            # Appliquer : d'abord pousser b, puis déplacer cult_a
            pushed_b["r"],   pushed_b["c"]   = new_pos_b[0], new_pos_b[1]
            pushed_b["rows"],pushed_b["cols"] = new_pos_b[2], new_pos_b[3]
            cult_a["r"],  cult_a["c"]  = new_pos_a[0], new_pos_a[1]
            cult_a["rows"],cult_a["cols"] = new_pos_a[2], new_pos_a[3]
            all_moves.append({
                "nom":   pushed_b.get("nom_fr") or pushed_b["nom"],
                "old_r": old_b_saved[0], "old_c": old_b_saved[1],
                "new_r": new_pos_b[0],   "new_c": new_pos_b[1],
            })
            all_moves.append({
                "nom":   cult_a.get("nom_fr") or cult_a["nom"],
                "old_r": old_a[0], "old_c": old_a[1],
                "new_r": new_pos_a[0], "new_c": new_pos_a[1],
            })
            return True
        return False

    # ── Progression et barre de progression ──────────────────────────────────
    n = len(culturels)
    op = [0]
    # Estimation : free_moves + swaps + chain3 par passe
    total_ops = 20 * n * (1 + n + len(producteurs) + n)

    def tick(k=1):
        op[0] += k
        if progress_cb:
            progress_cb(min(op[0] / max(total_ops, 1), 0.97))

    # ── Tri initial des culturels par culture décroissante ────────────────────
    culturels_sorted = sorted(culturels, key=lambda c: -c.get("culture", 0))

    def _prod_order():
        """Producteurs non saturés haute-priorité en premier."""
        return sorted(producteurs, key=lambda p: (
            -(p.get("priorite", 0) if boost_level(culture_received(p, culturels), p) < 100 else 0),
            -p.get("priorite", 0)
        ))
    ...
    while improved and n_passes < max_passes:
        ...
        # Recalculer les cibles sous-optimales pour le filtrage (point 4)
        target_cells = _undersaturated_cells()

        # ── Phase A : free moves (point 1) ────────────────────────────────
        for cult in culturels_sorted:
            tick()
            # Filtrage point 4 : ignorer les culturels qui ne peuvent pas
            # atteindre géométriquement un producteur sous-optimal
            if target_cells and not _cult_can_reach_target(cult, target_cells):
                continue
            if try_free_move(cult, boosts_before):
                boosts_before = get_boosts()
                target_cells  = _undersaturated_cells()
                improved = True

        # ── Phase B : swaps bilatéraux (point 2) ─────────────────────────
        # culturel ↔ culturel
        for i, c1 in enumerate(culturels_sorted):
            for c2 in culturels_sorted[i+1:]:
                tick()
        ...
        # culturel ↔ producteur (ordre priorité)
        ...
        # ── Phase C : chaînes de 3 (point 3) — uniquement si nécessaire ──
        # On ne lance la phase C que si des producteurs restent sous-optimaux
        # et que les phases A et B n'ont rien trouvé dans cette passe.
        if target_cells:
            for cult in culturels_sorted:
                tick()
                if not _cult_can_reach_target(cult, target_cells):
                    continue
                if try_chain3(cult, boosts_before):
                    boosts_before = get_boosts()
                    target_cells  = _undersaturated_cells()
                    improved = True
        ...
        for c in range(max_c):
            ...
            if terrain_grid[r][c] == "X":
                ...
                if b["r"] == r and b["c"] == c:
                    ...
                    merge_col_start = t_col - b["rows"] + 1
                    merge_row_start = t_row - b["cols"] + 1
                    if b["rows"] > 1 or b["cols"] > 1:
                        try:
                            ws4.merge_cells(
                                start_row=merge_row_start,
...
if _input_mode == "📁 Fichier Excel (.xlsx)":
    ...
    if uploaded_csv is not None:
        ...
        with _col_p1:
            _prio_barracks = st.number_input(
                "🏰 Casernes (Barracks)",
                min_value=0, max_value=10, value=8, step=1,
                help="Bâtiments contenant 'Barracks' dans le nom (infanterie, archers, cavalerie…)"
            )
        with _col_p2:
            _prio_farm = st.number_input(
                "🌾 Fermes (Farm)",
                min_value=0, max_value=10, value=3, step=1,
                help="Bâtiments contenant 'Farm', 'Merchant', 'Irrigation' ou 'Camel'"
            )
        with _col_p3:
            _prio_home = st.number_input(
                "🏠 Maisons (Home)",
                min_value=0, max_value=10, value=2, step=1,
                help="Bâtiments contenant 'Home' dans le nom"
            )
        with _col_p4:
            _prio_workshop = st.number_input(
                "⚒️ Ateliers (Workshop)",
                min_value=0, max_value=10, value=0, step=1,
                help="Bâtiments contenant 'Workshop', 'Smithy', 'Forge', 'Carpenter', 'Pottery'…"
            )
    else:
        # Valeurs par défaut si pas encore de fichier
        _prio_barracks, _prio_farm, _prio_home, _prio_workshop = 8, 3, 2, 0
...
if uploaded_csv is not None and _csv_data_ready:
    ...
    try:
        ...
        terrain_grid, max_r, max_c, placed, buildings_def, _ = read_from_csv(
            ...
            prio_barracks=int(_prio_barracks),
            prio_farm=int(_prio_farm),
            prio_home=int(_prio_home),
            prio_workshop=int(_prio_workshop),
        )
...
if 'placed' in dir() and 'original_placed' in dir():
    ...
    _mode_optim = st.radio(
        "Mode d'optimisation",
        options=["Par priorité", "Sans réduction", "Aucune"],
        index=0,
        horizontal=True,
        help=(
            ...
            "**Sans réduction** : n'accepte que les mouvements qui améliorent au moins "
            "un boost sans en réduire aucun.\n\n"
            "**Aucune** : génère l'Excel sans déplacer aucun bâtiment."
        )
    )
    if _mode_optim == "Par priorité":
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if _mode_optim == "Par priorité":
            ...
            optimized, moves = optimize_no_reduction(
                placed_for_optim, terrain_grid, max_r, max_c,
