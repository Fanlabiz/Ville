import pandas as pd
import numpy as np
from openpyxl import load_workbook
from openpyxl.styles import PatternFill, Font, Alignment
import streamlit as st
import io
from collections import defaultdict

# ====================== CLASSE BATIMENT ======================
class Batiment:
    def __init__(self, row):
        self.nom = str(row['Nom']).strip() if pd.notna(row.get('Nom')) else "Inconnu"
        self.long = int(row.get('Longueur', 1))
        self.larg = int(row.get('Largeur', 1))
        self.nombre = int(row.get('Nombre', 1))
        self.typ = str(row.get('Type', 'Neutre')).strip()
        self.culture = int(row.get('Culture', 0))
        self.rayon = int(row.get('Rayonnement', 0))
        
        # Boost robuste
        def get_boost(cols):
            for c in cols:
                val = row.get(c)
                if pd.notna(val):
                    try: return int(val)
                    except: return 0
            return 0
        self.boost25 = get_boost(['Boost 25%', 'Boost25', 'Boost 25 %'])
        self.boost50 = get_boost(['Boost 50%', 'Boost50', 'Boost 50 %'])
        self.boost100 = get_boost(['Boost 100%', 'Boost100', 'Boost 100 %'])
        
        self.prod_type = str(row.get('Production', 'Rien')).strip()
        self.quantite = int(row.get('Quantite', 0))
        self.instances = []  # (r, c, orient, culture_recue, boost_pct)

    def surface(self):
        return self.long * self.larg

# ====================== LECTURE ======================
def lire_excel(uploaded_file):
    df_terrain = pd.read_excel(uploaded_file, sheet_name='Terrain', header=None)
    terrain = df_terrain.to_numpy()

    df_bats = pd.read_excel(uploaded_file, sheet_name='Batiments')
    batiments = [Batiment(row) for _, row in df_bats.iterrows() if pd.notna(row.get('Nom')) and str(row.get('Nom')).strip()]

    if 'Actuel' in load_workbook(uploaded_file, data_only=True).sheetnames:
        df_actuel = pd.read_excel(uploaded_file, sheet_name='Actuel', header=None)
        actuel = df_actuel.to_numpy()
        terrain = np.where((actuel.astype(str) == 'X') | (actuel == 0), 0, terrain)

    return terrain, batiments

# ====================== PLACEMENT (amélioré avec Nombre) ======================
def peut_placer(terrain, r, c, h, w):
    if r < 0 or c < 0 or r + h > terrain.shape[0] or c + w > terrain.shape[1]:
        return False
    return np.all(terrain[r:r+h, c:c+w] == 1)

def placer(terrain, r, c, h, w):
    terrain[r:r+h, c:c+w] = 0

def score_position(terrain, r, c, h, w, bat, is_neutre=False, is_culturel=False):
    if not peut_placer(terrain, r, c, h, w):
        return -999999
    score = 1000 - (abs(r + h/2 - terrain.shape[0]/2) + abs(c + w/2 - terrain.shape[1]/2)) * 8
    if is_neutre and (r <= 2 or r + h >= terrain.shape[0]-2 or c <= 2 or c + w >= terrain.shape[1]-2):
        score += 15000
    if is_culturel:
        free = sum(1 for i in range(max(0,r-bat.rayon), min(terrain.shape[0],r+h+bat.rayon))
                   for j in range(max(0,c-bat.rayon), min(terrain.shape[1],c+w+bat.rayon)) if terrain[i,j] == 1)
        score += free * 15
    return score

def trouver_meilleure_position(terrain, bat, is_neutre=False, is_culturel=False, priorite_guerrison=False):
    best_score, best_pos, best_orient = -999999, None, 'H'
    for orient in ['H', 'V']:
        h = bat.larg if orient == 'V' else bat.long
        w = bat.long if orient == 'V' else bat.larg
        for i in range(terrain.shape[0] - h + 1):
            for j in range(terrain.shape[1] - w + 1):
                score = score_position(terrain, i, j, h, w, bat, is_neutre, is_culturel)
                if priorite_guerrison and bat.prod_type == "Guerison":
                    score += 30000
                if score > best_score:
                    best_score = score
                    best_pos = (i, j)
                    best_orient = orient
    return best_pos, best_orient

def optimiser_placement(terrain_orig, batiments):
    terrain = terrain_orig.copy()
    neutres = [b for b in batiments if b.typ == "Neutre"]
    culturels = [b for b in batiments if b.typ == "Culturel"]
    producteurs = [b for b in batiments if b.typ == "Producteur"]

    for grp in [neutres, culturels, producteurs]:
        grp.sort(key=lambda b: b.surface(), reverse=True)

    placed_list = []
    non_places = []

    # 1. Neutres sur bords
    for bat in neutres:
        for _ in range(bat.nombre):
            pos, orient = trouver_meilleure_position(terrain, bat, is_neutre=True)
            if pos:
                h = bat.larg if orient == 'V' else bat.long
                w = bat.long if orient == 'V' else bat.larg
                placer(terrain, *pos, h, w)
                bat.instances.append((*pos, orient, 0, 0))
                placed_list.append((bat, *pos, orient, h, w))
            else:
                non_places.append(bat)
                break

    # 2. Culturels + Producteurs alternés (priorité Guérison)
    all_remaining = sorted(culturels + producteurs, key=lambda b: (b.typ != "Culturel", -b.surface() if b.prod_type != "Guerison" else 10000))
    for bat in all_remaining:
        for _ in range(bat.nombre):
            is_cult = bat.typ == "Culturel"
            pos, orient = trouver_meilleure_position(terrain, bat, is_culturel=is_cult, 
                                                     priorite_guerrison=(bat.prod_type == "Guerison"))
            if pos:
                h = bat.larg if orient == 'V' else bat.long
                w = bat.long if orient == 'V' else bat.larg
                placer(terrain, *pos, h, w)
                bat.instances.append((*pos, orient, 0, 0))
                placed_list.append((bat, *pos, orient, h, w))
            else:
                non_places.append(bat)
                break

    return terrain, placed_list, non_places

# ====================== CALCUL CULTURE & BOOST ======================
def calcul_culture_boost(placed_list, batiments, terrain):
    culturels = [p for p in placed_list if p[0].typ == "Culturel"]
    for bat in batiments:
        if bat.typ != "Producteur": continue
        for idx, (r, c, orient, _, _) in enumerate(bat.instances):
            h = bat.larg if orient == 'V' else bat.long
            w = bat.long if orient == 'V' else bat.larg
            total_culture = 0
            seen = set()
            for cult_bat, cr, cc, corient, ch, cw in culturels:
                if id(cult_bat) in seen: continue
                for i in range(r, r+h):
                    for j in range(c, c+w):
                        for ci in range(cr - cult_bat.rayon, cr + ch + cult_bat.rayon):
                            for cj in range(cc - cult_bat.rayon, cc + cw + cult_bat.rayon):
                                if 0 <= ci < terrain.shape[0] and 0 <= cj < terrain.shape[1]:
                                    if max(abs(ci-i), abs(cj-j)) <= cult_bat.rayon:  # Chebyshev
                                        total_culture += cult_bat.culture
                                        seen.add(id(cult_bat))
                                        break
            boost = 100 if total_culture >= bat.boost100 else 50 if total_culture >= bat.boost50 else 25 if total_culture >= bat.boost25 else 0
            bat.instances[idx] = (r, c, orient, total_culture, boost)

# ====================== EXPORT ======================
def creer_excel_resultat(batiments, non_places, terrain_final, placed_list):
    output = io.BytesIO()
    writer = pd.ExcelWriter(output, engine='openpyxl')

    # Batiments places
    rows = []
    for bat in batiments:
        for inst in bat.instances:
            r, c, orient, cult, boost = inst
            h = bat.larg if orient == 'V' else bat.long
            w = bat.long if orient == 'V' else bat.larg
            rows.append({
                "Nom": bat.nom, "Type": bat.typ, "Production": bat.prod_type,
                "Ligne": r+1, "Colonne": c+1, "Hauteur": h, "Largeur": w,
                "Culture recue": cult, "Boost (%)": boost,
                "Quantite/h": bat.quantite, "Prod totale/h": round(bat.quantite * (1 + boost/100), 1),
                "Origine": "Placé"
            })
    pd.DataFrame(rows).to_excel(writer, sheet_name="Batiments places", index=False)

    # Synthese corrigée
    prod_data = defaultdict(lambda: {"cult": 0, "nb": 0, "prod": 0.0})
    for bat in batiments:
        if bat.typ != "Producteur": continue
        for _, _, _, cult, boost in bat.instances:
            prod_data[bat.prod_type]["cult"] += cult
            prod_data[bat.prod_type]["nb"] += 1
            prod_data[bat.prod_type]["prod"] += bat.quantite * (1 + boost / 100.0)

    synthese = [["Synthese par type de production","","","",""],
                ["Production","Culture totale","Boost moyen (%)","Nb batiments","Production/h"]]
    for ptype, d in prod_data.items():
        boost_moy = round(d["prod"] / (d["nb"] * max(1, list(batiments)[0].quantite if any(b.quantite>0 for b in batiments) else 1)) * 100, 1) if d["nb"] > 0 else 0
        synthese.append([ptype, d["cult"], boost_moy, d["nb"], round(d["prod"], 1)])
    synthese += [["","","","",""], 
                 ["Cases libres restantes", int(np.sum(terrain_final == 1)), "","",""],
                 ["Cases des batiments non places", sum(b.surface()*b.nombre for b in non_places), "","",""]]
    pd.DataFrame(synthese).to_excel(writer, sheet_name="Synthese", index=False, header=False)

    # Terrain + Non places
    pd.DataFrame(terrain_final).to_excel(writer, sheet_name="Terrain", index=False, header=False)
    non_data = [{"Nom": b.nom, "Type": b.typ, "Production": b.prod_type, "Longueur": b.long, "Largeur": b.larg, "Cases": b.surface()*b.nombre} for b in non_places]
    pd.DataFrame(non_data).to_excel(writer, sheet_name="Non places", index=False)

    writer.close()
    output.seek(0)

    # Coloration + noms
    wb = load_workbook(output)
    ws = wb["Terrain"]
    fills = {"Neutre": PatternFill("solid", "CCCCCC"), "Culturel": PatternFill("solid", "FFCC99"), "Producteur": PatternFill("solid", "99FF99")}
    bold = Font(bold=True)

    for bat, r, c, orient, h, w in placed_list:
        fill = fills.get(bat.typ, fills["Neutre"])
        for i in range(r, r+h):
            for j in range(c, c+w):
                ws.cell(i+1, j+1).fill = fill
        cell = ws.cell(r+1, c+1)
        cell.value = f"{bat.nom[:12]} {bat.instances[0][4]}%" if bat.instances else bat.nom[:12]
        cell.font = bold
        cell.alignment = Alignment(horizontal="center", vertical="center")

    final = io.BytesIO()
    wb.save(final)
    final.seek(0)
    return final

# ====================== STREAMLIT ======================
def main():
    st.set_page_config(page_title="Optimiseur Ville", layout="wide")
    st.title("🚀 Optimiseur de Placement de Bâtiments - Version corrigée")
    st.caption("Nombre d'occurrences respecté + Boost moyen corrigé + Placement amélioré")

    uploaded = st.file_uploader("Fichier Excel (Ville.xlsx)", type=["xlsx"])

    if uploaded:
        terrain, batiments = lire_excel(uploaded)
        st.success(f"Terrain chargé ({terrain.shape[0]}×{terrain.shape[1]}) | {len(batiments)} types de bâtiments")

        if st.button("🚀 Lancer l'optimisation complète", type="primary"):
            with st.spinner("Placement de tous les exemplaires + calcul..."):
                terrain_final, placed_list, non_places = optimiser_placement(terrain, batiments)
                calcul_culture_boost(placed_list, batiments, terrain_final)
                excel_bytes = creer_excel_resultat(batiments, non_places, terrain_final, placed_list)

                st.success("✅ Terminé ! Tous les bâtiments ont été traités selon leur quantité.")
                st.download_button("📥 Télécharger resultats.xlsx", excel_bytes, "resultats.xlsx",
                                   "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

                st.subheader("Résumé")
                st.write(f"Types de bâtiments traités : {len(batiments)}")
                st.write(f"Non placés (impossible) : {len(non_places)}")
                st.write(f"Cases libres restantes : {np.sum(terrain_final == 1)}")

if __name__ == "__main__":
    main()
