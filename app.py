...
if uploaded_csv is not None and _csv_data_ready:
    ...
    try:
        ...
        # Le nombre de chaque bâtiment à placer est recalculé depuis buildings_def.
        if _terrain_vide:
            # Mettre à jour le nombre dans le catalogue : tous les exemplaires
            # présents dans placed sont considérés comme non placés.
            from collections import Counter as _Counter
            _placed_cnt = _Counter(b["nom"].strip() for b in placed)
            for _bdef in buildings_def:
                _bdef["nombre"] = _placed_cnt.get(_bdef["nom"].strip(), _bdef["nombre"])
            placed = []
