...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        # D'abord, on va peupler possible_dimensions avec les données de l'onglet Batiments
        # Cette méthode est appelée après set_building_dimensions, donc possible_dimensions est déjà rempli
...
- **Détection individuelle** des bâtiments (prend la plus petite dimension possible)
- **Gère les orientations différentes** des bâtiments
- **Gère les bâtiments multiples** (détecte chaque bâtiment individuellement, même en groupe)
