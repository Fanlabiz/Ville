...
def optimize_no_reduction(placed, terrain_grid, max_r, max_c, progress_cb=None):
    """
    Optimisation 'sans réduction' :
    N'accepte que les mouvements qui augmentent au moins un boost
    sans en diminuer aucun. Objectif : maximiser le nombre de
    producteurs en boost 25%, 50% ou 100%, sans jamais rétrograder
    un producteur déjà boosté.
    """
    x_grid = make_x_grid(terrain_grid, max_r, max_c)
    placed = [dict(b) for b in placed]
    all_moves = []
    ...
    def tick():
        op[0] += 1
        if progress_cb:
            progress_cb(min(op[0] / max(n * 5, 1), 0.98))

    def boost_vector(placed_list):
        """Retourne {id(b): boost_level} pour tous les producteurs sans distinction de priorité."""
        culturels = [b for b in placed_list if b["type"] == "Culturel"]
        return {
            id(b): boost_level(culture_received(b, culturels), b)
            for b in placed_list if b["type"] == "Producteur"
        }

    def is_improvement(bv_before, bv_after):
        """Vrai si aucun producteur ne régresse et au moins un progresse."""
        regression = any(bv_after.get(k, 0) < v for k, v in bv_before.items())
        progression = any(bv_after.get(k, 0) > v for k, v in bv_before.items())
        return not regression and progression

    improved = True
    n_passes = 0
    while improved and n_passes < 10:
        improved = False
        n_passes += 1
        culturels = [b for b in placed if b["type"] == "Culturel"]
        for b in culturels:
            tick()
            bv_before = boost_vector(placed)
            old_r, old_c = b["r"], b["c"]

            # Essayer toutes les positions valides
            best_pos = None
            best_gain = 0
            occ = make_occ_grid(placed, max_r, max_c, exclude_ids={id(b)})
            for r in range(max_r - b["rows"] + 1):
                for c in range(max_c - b["cols"] + 1):
                    if r == old_r and c == old_c:
                        continue
                    if not can_place(r, c, b["rows"], b["cols"], x_grid, occ, max_r, max_c):
                        continue
                    b["r"], b["c"] = r, c
                    bv_after = boost_vector(placed)
                    b["r"], b["c"] = old_r, old_c
                    if is_improvement(bv_before, bv_after):
                        gain = sum(bv_after.get(k,0) - bv_before.get(k,0)
                                   for k in set(bv_before)|set(bv_after))
                        if gain > best_gain:
                            best_gain = gain
                            best_pos = (r, c)
            ...
            if best_pos:
                all_moves.append({
                    "nom": b.get("nom_fr") or b["nom"],
                    "old_r": old_r, "old_c": old_c,
