...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Rayonnement en bande autour du bâtiment")
        ...
        with st.spinner("Placement en cours..."):
            ...
            for i, b in enumerate(culturels):
                ...
                if meilleure_pos:
                    ...
                    # Afficher la zone de rayonnement
                    l, L = b.get_dimensions(o)
                    st.write(f"✅ {b.nom} à ({x},{y}) - taille {l}x{L}, rayon {b.rayonnement}")
                else:
                    st.write(f"❌ Impossible de placer {b.nom}")
                
                progress.progress((i + 1) / len(tous))
            
            # Afficher les zones de rayonnement
            st.subheader("🗺️ Zones de rayonnement (bandes autour des culturels)")
            
            # Créer une visualisation des zones
            rayon_viz = np.full((terrain_final.height, terrain_final.width), '⬜', dtype=str)
            for y in range(terrain_final.height):
                for x in range(terrain_final.width):
                    if terrain_final.occupied[y, x]:
                        rayon_viz[y, x] = '⬛'
            
            # Marquer les zones de rayonnement
            colors = ['🟦', '🟩', '🟨', '🟧', '🟪']
            for idx, cb in enumerate(terrain_final.cultural_buildings):
                color = colors[idx % len(colors)]
                for y in range(terrain_final.height):
                    for x in range(terrain_final.width):
                        if terrain_final.is_in_radiation_zone(cb, x, y):
                            if rayon_viz[y, x] == '⬜':  # Si case libre
                                rayon_viz[y, x] = color
            
            st.dataframe(pd.DataFrame(rayon_viz), use_container_width=True)
            
            # Légende des zones
            st.write("Légende des zones de rayonnement:")
            for idx, cb in enumerate(terrain_final.cultural_buildings):
                color = colors[idx % len(colors)]
                st.write(f"{color} {cb['building'].nom} (rayon {cb['rayonnement']})")
            ...
            for i, b in enumerate(producteurs):
                ...
                if meilleure_pos:
                    ...
                    # Recalculer les culturels touchants
                    culturels_touchants = []
                    for cb in terrain_final.cultural_buildings:
                        for i2 in range(l):
                            for j2 in range(L):
                    ...
                    # Calculer la culture
                    culture = 0
                    for nom in culturels_touchants:
                        for cb in terrain_final.cultural_buildings:
                            if cb['building'].nom == nom:
                                culture += cb['culture']
                                break
                    ...
                    st.write(f"✅ {b.nom} à ({x},{y}) - {len(culturels_touchants)} culturels: {', '.join(culturels_touchants)} → {culture:.0f} culture {boost_text}")
                else:
                    st.write(f"❌ Impossible de placer {b.nom}")
