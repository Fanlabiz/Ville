...
def load_buildings(wb):
    ...
    for i, row in enumerate(ws.iter_rows(values_only=True)):
        ...
        prod_raw = str(d.get("Production") or "").strip()
        b = {
            ...
            "production":  prod_raw if prod_raw.lower() not in ("n/a", "none", "") else None,
        }
        buildings.append(b)
    return buildings
...
class FastPlacement:
    ...
    def total_score(self):
        """Score pondéré par priorité : Caserne(x3) > Nourriture(x2) > Or(x1)."""
        score = 0.0
        for inst in self.instances:
            t = inst["template"]
            if t["boost_25"] <= 0 and t["boost_50"] <= 0 and t["boost_100"] <= 0:
                continue
            b = self.boost_for(t, inst["row"], inst["col"], inst["h"], inst["w"])
            prio = max(1, boost_priority(t))  # min 1 pour compter tous les boostables
            score += b * prio
        return score
...
# Priorité de boost par type de production (plus élevé = plus important)
PRODUCTION_PRIORITY = {
    "Caserne":    3,
    "Nourriture": 2,
    "Or":         1,
...
def boost_priority(template):
    """Retourne la priorité de boost d'un bâtiment (3=caserne, 2=nourriture, 1=or, 0=autre)."""
    prod = template.get("production")
    if prod is None:
        return 0
    for key in PRODUCTION_PRIORITY:
        if key.lower() in prod.lower():
            return PRODUCTION_PRIORITY[key]
    return 0
...
def run_placement(terrain, templates, shuffle=False, compact_weight=1.0, boost_weight=10.0):
    ...
    if shuffle:
        ...
        random.shuffle(emetteurs)
        random.shuffle(neutres)
    else:
        # Du plus grand au plus petit dans chaque groupe
        boostables.sort(key=lambda b: -(b["largeur"] * b["longueur"]))
        emetteurs.sort(key=lambda b: -(b["largeur"] * b["longueur"]))
        neutres.sort(key=lambda b: -(b["largeur"] * b["longueur"]))

    p = FastPlacement(terrain)
    unplaced = []

    # Étape 1 : grands émetteurs (rayon >= 3) — définissent les zones de culture
    grands = [e for e in emetteurs if e["rayonnement"] >= 3]
    petits = [e for e in emetteurs if e["rayonnement"] < 3]
    for tmpl in grands:
    ...
    # Étape 2 : intercaler boostables et neutres (du plus grand au plus petit)
    # Cela évite que les neutres finissent avec des trous trop fragmentés
    boostables_sorted = sorted(boostables, key=lambda b: (-boost_priority(b), -(b["largeur"] * b["longueur"])))
    neutres_sorted    = sorted(neutres,    key=lambda b: -(b["largeur"] * b["longueur"]))
    merged = []
    bi, ni = 0, 0
    while bi < len(boostables_sorted) or ni < len(neutres_sorted):
        if bi < len(boostables_sorted):
            merged.append(('boost', boostables_sorted[bi])); bi += 1
        if ni < len(neutres_sorted):
            merged.append(('neutre', neutres_sorted[ni])); ni += 1

    for kind, tmpl in merged:
        if kind == 'boost':
            result = p.best_position_and_orientation(tmpl, sample=150,
                                                      compact_weight=compact_weight,
                                                      boost_weight=boost_weight)
        else:
            result = p.best_position_and_orientation(tmpl, sample=200,
                                                      compact_weight=compact_weight * 2,
                                                      boost_weight=0.0)
        if result is None:
            unplaced.append(tmpl["nom"])
        else:
    ...
    # Étape 3 : petits émetteurs — se placent pour débloquer les paliers des boostables
    for tmpl in petits:
...
def local_improve(placement, terrain, n_iter=300):
    """
    Amélioration locale avec 3 types de mouvements :
    - Type 0 : déplacer un petit émetteur
    - Type 1 : déplacer un boostable sous-boosted
    - Type 2 : compacter (déplacer n'importe quel bâtiment)
    Critère d'acceptation : (nb_placés DESC, score DESC)
    """
    best_score = placement.total_score()
    best_placed = len(placement.instances)

    for iteration in range(n_iter):
        move_type = iteration % 3

        if move_type == 0:
            cands = [idx for idx, inst in enumerate(placement.instances)
                     if inst["template"]["culture"] > 0 and inst["template"]["rayonnement"] < 3]
            if not cands:
                continue
            idx = random.choice(cands)
        elif move_type == 1:
            under = [
                (idx, placement.boost_for(inst["template"], inst["row"], inst["col"], inst["h"], inst["w"]))
            ...
            under = [(i, b) for i, b in under if b < 100]
            if not under:
                continue
            under.sort(key=lambda x: x[1])
            pool = under[:max(1, len(under) // 2)]
            idx, _ = random.choice(pool)
        else:
            idx = random.randrange(len(placement.instances))
        ...
        if t["culture"] > 0 and t["rayonnement"] < 3:
            result = new_p.best_emitter_position(t, sample=200, compact_weight=0.3)
        elif t["boost_25"] > 0 or t["boost_50"] > 0 or t["boost_100"] > 0:
            result = new_p.best_position_and_orientation(t, sample=200, compact_weight=0.5, boost_weight=15.0)
        else:
            result = new_p.best_position_and_orientation(t, sample=200, compact_weight=2.0, boost_weight=0.0)
        ...
        new_placed = len(new_p.instances)

        if (new_placed > best_placed) or (new_placed == best_placed and new_score >= best_score):
            placement = new_p
...
def build_excel(placement, templates):
    ...
    categories  = ["Caserne", "Nourriture", "Or"]
    cat_colors  = {"Caserne": "C0392B", "Nourriture": "27AE60", "Or": "D4AC0D", "Autre": "7F8C8D"}
    cat_fills   = {"Caserne": "FADBD8", "Nourriture": "D5F5E3", "Or": "FDEBD0", "Autre": "F2F3F4"}

    def get_cat(prod):
        if prod is None:
            return "Autre"
        for c in categories:
            if c.lower() in str(prod).lower():
                return c
        return "Autre"

    # Statistiques globales
    ws_ana.cell(row=1, column=1, value="ANALYSE DES BOOSTS").font = Font(bold=True, size=13)
    global_data = [
        ("Batiments boost-sensibles", total_b),
        ("Score global obtenu",       score_obtenu),
        ("Score global maximum",      score_max),
        ("Efficacite globale",        f"{100 * score_obtenu // (score_max or 1)}%"),
        ("", ""),
    ...
    fills_boost = {"100% boost": "A9DFBF", " 50% boost": "FAD7A0", " 25% boost": "FDEBD0", "  0% boost": "FADBD8"}
    row_cur = 3
    for k, v in global_data:
        ws_ana.cell(row=row_cur, column=1, value=k).font = Font(bold=True)
        ws_ana.cell(row=row_cur, column=2, value=v)
        if k in fills_boost:
            for ci in [1, 2]:
                ws_ana.cell(row=row_cur, column=ci).fill = PatternFill("solid", fgColor=fills_boost[k])
        row_cur += 1

    row_cur += 1  # ligne vide

    # Statistiques par catégorie de production
    for cat in categories + ["Autre"]:
        insts_cat = [i for i in boost_insts if get_cat(i["template"].get("production")) == cat]
        if not insts_cat:
            continue
        at_c = {0: 0, 25: 0, 50: 0, 100: 0}
        score_c = 0
        for inst_c in insts_cat:
            t_c = inst_c["template"]
            b_c = placement.boost_for(t_c, inst_c["row"], inst_c["col"], inst_c["h"], inst_c["w"])
            at_c[b_c] += 1
            score_c += b_c
        score_m = len(insts_cat) * 100
        prio = PRODUCTION_PRIORITY.get(cat, 0)
        prio_label = f" — Priorite {prio}" if prio > 0 else ""
        fill_cat = cat_fills.get(cat, "F2F3F4")
        color_cat = cat_colors.get(cat, "000000")

        header = ws_ana.cell(row=row_cur, column=1,
                              value=f"=== {cat.upper()}{prio_label} ===")
        header.font = Font(bold=True, size=11, color=color_cat)
        row_cur += 1

        cat_data = [
            ("Batiments",    len(insts_cat)),
            ("Score obtenu", score_c),
            ("Score maximum", score_m),
            ("Efficacite",   f"{100 * score_c // (score_m or 1)}%"),
            ("100% boost",   at_c[100]),
            (" 50% boost",   at_c[50]),
            (" 25% boost",   at_c[25]),
            ("  0% boost",   at_c[0]),
        ...
        for k, v in cat_data:
            ...
            ws_ana.cell(row=row_cur, column=2, value=v)
            for ci in [1, 2]:
                ws_ana.cell(row=row_cur, column=ci).fill = PatternFill("solid", fgColor=fill_cat)
            row_cur += 1
        row_cur += 1  # ligne vide entre catégories

    # Liste des bâtiments à 0%
    if at[0]:
        ws_ana.cell(row=row_cur, column=1,
                    value="Batiments a 0% :").font = Font(bold=True, color="C0392B")
        row_cur += 1
        for inst in at[0]:
            t = inst["template"]
            received = placement.culture_received(inst["row"], inst["col"], inst["h"], inst["w"], boostable=True)
            manque   = t["boost_25"] - received
            cat      = get_cat(t.get("production"))
            ws_ana.cell(row=row_cur, column=1, value=f"[{cat}] {t['nom']}")
            ws_ana.cell(row=row_cur, column=2,
                        value=f"recu={received:.0f}  seuil={t['boost_25']:.0f}  manque={manque:.0f}")
            for ci in [1, 2]:
                ws_ana.cell(row=row_cur, column=ci).fill = PatternFill("solid", fgColor="FADBD8")
            row_cur += 1

    ws_ana.column_dimensions["A"].width = 38
    ws_ana.column_dimensions["B"].width = 3835
