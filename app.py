...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    buf = io.BytesIO()
    # ── Marqueur de version ──────────────────────────────────────────
    # Ajouté à la fin de l'onglet Synthèse pour prouver la version du script.
    _ver_row = ws2.max_row + 2
    _vc = ws2.cell(_ver_row, 1, "✅ Script version: v2.1-compaction")
    _vc.font = Font(bold=True, color="FFFF0000", size=11)
    _vc.fill = PatternFill("solid", fgColor="FFFFFF00")
    ws2.cell(_ver_row, 2, "Compaction sans limite de distance — si ce message est absent, vous utilisez une ancienne version")
    ws2.cell(_ver_row, 2).font = Font(italic=True, color="FFFF0000")
...
if uploaded:
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if optionnels_a_placer:
            ...
            # ── Phase 2 : compaction – tester TOUS les optionnels placés ──────────
            # À chaque passe: parcourir tous les optionnels placés.
            # Pour chacun, chercher TOUTES les positions (sans limite de distance)
            # qui permettent de placer un optionnel supplémentaire.
            # Effectuer tous les gains trouvés avant de relancer une nouvelle passe.
            for _cp in range(30):
                _placed_c = {}
                ...
                for _h in list(b for b in optimized if b["nom"].strip() in _opt_quota):
                                ...
                                for _r4 in range(max_r - _rw2 + 1):
                                    for _c4 in range(max_c - _cl2 + 1):
                                        if can_place(_r4,_c4,_rw2,_cl2,_xg_opt,_om,max_r,max_c):
                                            _best_move = (_nr,_nc,_r4,_c4,_rw2,_cl2)
                                            break
                                    if _best_move: break
                                if _best_move: break
                            if _best_move: break
                    ...
                    if _best_move:
                        _nr,_nc,_r4,_c4,_rw2,_cl2 = _best_move
                        _wo2 = [b for b in optimized if b is not _h]
                        _mv2 = _wo2 + [{**_h,"r":_nr,"c":_nc}]
                        optimized = _mv2 + [{**_un0,"r":_r4,"c":_c4,"rows":_rw2,"cols":_cl2}]
                        _n_added += 1
                        _s_cur = score_placement(optimized)
                        _unplaced = _unplaced[1:]
                        if _unplaced:
                            _un0 = _unplaced[0]; _ur,_uc = _un0["rows"],_un0["cols"]
