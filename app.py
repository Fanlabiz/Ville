...
class Terrain:
    ...
    def get_space_preservation_score(self, x, y, longueur, largeur, remaining_buildings):
        """
        Calcule un score de préservation d'espace.
        Plus le score est élevé, mieux c'est pour les futurs placements.
        """
        ...
        # Vérifier pour chaque bâtiment restant s'il peut encore être placé
        total_remaining = len(remaining_buildings)
        if total_remaining == 0:
            return 10000  # Plus rien à placer, score maximal
        
        # Compter combien de bâtiments restants peuvent encore être placés
        can_place_count = 0
        for b in remaining_buildings:
            if self.can_place_building_anywhere(b, temp_occupied):
                can_place_count += 1
        
        # Score = pourcentage de bâtiments restants qui peuvent encore être placés
        preservation_score = (can_place_count / total_remaining) * 10000
        ...
        with st.spinner("Placement en cours..."):
            ...
            for i, b in enumerate(producteurs):
                ...
                for x, y, o in positions:
                    ...
                    # Score de préservation d'espace
                    space_score = terrain_final.get_space_preservation_score(x, y, l, L, remaining)
                    score += space_score * 10  # Pondération importante
