...
def load_buildings(wb):
    ...
    for i, row in enumerate(ws.iter_rows(values_only=True)):
        ...
        b = {
            ...
            "quantite":    int(d.get("Quantité") or d.get("Quantite") or 1),
...
class FastPlacement:
    ...
    def culture_received(self, row, col, h, w):
        return float(self.culture_map[row:row+h, col:col+w].sum())

    def boost_for(self, template, row, col, h, w):
        if template["boost_25"] <= 0 and template["boost_50"] <= 0 and template["boost_100"] <= 0:
            return 0
        received = self.culture_received(row, col, h, w)
    ...
    def best_position_and_orientation(self, template, sample=150, compact_weight=1.0, boost_weight=10.0):
        ...
        for (label, h, w) in orientations:
            ...
            for (r, c) in candidates:
                ...
                culture = self.culture_received(r, c, h, w) if needs_boost else 0
...
def run_placement(terrain, templates, shuffle=False, compact_weight=1.0, boost_weight=10.0):
    ...
    if not shuffle:
        instances.sort(key=lambda b: -(b["largeur"] * b["longueur"]))
    else:
        instances.sort(key=lambda b: -(b["largeur"] * b["longueur"]) + random.uniform(-2, 2))
...
def build_excel(placement, templates):
    ...
    for inst in placement.instances:
        ...
        arrow  = "↕" if inst["orientation"] == "V" else "↔"
        label  = t["nom"][:5] + arrow
        for dr in range(h):
            for dc in range(w):
    ...
    hdrs = ["Nom", "Orientation", "Ligne", "Colonne", "Taille", "Culture reçue", "Boost (%)"]
    ...
    for ri, inst in enumerate(placement.instances, 2):
        ...
        r0, c0   = inst["row"], inst["col"]
        h, w     = inst["h"], inst["w"]
        received = placement.culture_received(r0, c0, h, w)
        boost    = placement.boost_for(t, r0, c0, h, w)
        orient   = "Vertical" if inst["orientation"] == "V" else "Horizontal"
        for ci, val in enumerate([t["nom"], orient, r0+1, c0+1, f"{h}×{w}", round(received,1), boost], 1):
    ...
    data = [
        ("Bâtiments boost-sensibles", total_b),
        ("Score obtenu", score_obtenu),
        ("Score maximum possible", score_max),
        ("Efficacité", f"{100*score_obtenu//(score_max or 1)}%"),
        ("", ""),
        ("100% boost", f"{len(at[100])} bâtiments ({100*len(at[100])//(total_b or 1)}%)"),
        (" 50% boost", f"{len(at[50])} bâtiments ({100*len(at[50])//(total_b or 1)}%)"),
        (" 25% boost", f"{len(at[25])} bâtiments ({100*len(at[25])//(total_b or 1)}%)"),
        ("  0% boost", f"{len(at[0])} bâtiments ({100*len(at[0])//(total_b or 1)}%)"),
    ]
    ...
    if at[0]:
        ws_ana.cell(row=14, column=1, value="Bâtiments à 0% :").font = Font(bold=True, color="C0392B")
        for ri, inst in enumerate(at[0], 15):
            ...
            ws_ana.cell(row=ri, column=2, value=f"reçu={received:.0f}  seuil={t['boost_25']:.0f}  manque={manque:.0f}")
            for ci in [1,2]:
    ...
    ws_unp = wb.create_sheet("Non placés")
    red = PatternFill("solid", fgColor="C0392B")
    for ci, h in enumerate(["Bâtiment","Demandés","Placés","Manquants"], 1):
    ...
    for t in templates:
        ...
        if missing > 0:
            ...
            ri += 1
    if ri == 2:
        ws_unp.cell(row=2, column=1, value="✓ Tous les bâtiments ont été placés !")

    # Légende
    ws_leg = wb.create_sheet("Légende")
    for ci, h in enumerate(["Couleur","Bâtiment","Taille orig.","Culture","Rayonnement"], 1):
    ...
    for ri2, t in enumerate(templates, 2):
        ...
        ws_leg.cell(row=ri2, column=3, value=f"{t['largeur']}×{t['longueur']}")
...
if uploaded:
    try:
        ...
        with c1:
            st.markdown(f'<div class="stat-card"><div class="value">{terrain.shape[0]}×{terrain.shape[1]}</div><div class="label">Taille du terrain</div></div>', unsafe_allow_html=True)
        with c2:
        ...
        if surface > free:
            st.markdown(f'<div class="warning-box">⚠️ Surface des bâtiments ({surface}) supérieure aux cases libres ({free}). Certains bâtiments ne pourront pas être placés.</div>', unsafe_allow_html=True)
        ...
        if st.button("🚀 Optimiser le placement", use_container_width=True, type="primary"):
            ...
            if placed_count == total:
                st.markdown('<div class="success-box">✅ Tous les bâtiments ont été placés avec succès !</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="warning-box">⚠️ {placed_count}/{total} bâtiments placés — {len(unplaced)} non placés</div>', unsafe_allow_html=True)
