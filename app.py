...
class Terrain:
    ...
    def get_radiation_zone_size(self, cultural):
        """
        Calcule le nombre de cases dans la zone de rayonnement du bâtiment culturel.
        """
        cx, cy = cultural['x'], cultural['y']
        cl, cL = cultural['longueur'], cultural['largeur']
        ...
        count = 0
        for x in range(max(0, cx - rayon), min(self.width, cx + cl + rayon)):
            for y in range(max(0, cy - rayon), min(self.height, cy + cL + rayon)):
                if self.is_in_radiation_zone(cultural, x, y):
                    count += 1
        return count
...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Optimisation du rayonnement")
    ...
    if st.button("🚀 Lancer l'optimisation", type="primary"):
        with st.spinner("Placement en cours..."):
            ...
            # Étape 2: Placer les producteurs
