...
class Terrain:
    ...
    def has_width_1_corridors(self, temp_occupied=None):
        ...
        Retourne True si des couloirs de largeur 1 sont détectés.
        """
        ...
        for y in range(self.height):
            for x in range(self.width):
                if free[y, x]:
                    # Vérifier si c'est un couloir horizontal (largeur 1)
                    # La case est libre, mais ses voisins verticaux sont occupés
                    top_occupied = y == 0 or not free[y-1, x]
                    bottom_occupied = y == self.height-1 or not free[y+1, x]
                    
                    if top_occupied and bottom_occupied:
                        # C'est une ligne horizontale isolée
                        # Vérifier si elle fait partie d'un rectangle plus large
                        left_free = x > 0 and free[y, x-1]
                        right_free = x < self.width-1 and free[y, x+1]
                        
                        if not (left_free and right_free):
                            # Ce n'est pas une ligne continue, c'est un couloir
                            return True
        ...
        for x in range(self.width):
            for y in range(self.height):
                if free[y, x]:
                    # Vérifier si c'est un couloir vertical (largeur 1)
                    left_occupied = x == 0 or not free[y, x-1]
                    right_occupied = x == self.width-1 or not free[y, x+1]
                    
                    if left_occupied and right_occupied:
                        # C'est une colonne verticale isolée
                        top_free = y > 0 and free[y-1, x]
                        bottom_free = y < self.height-1 and free[y+1, x]
                        
                        if not (top_free and bottom_free):
                            # Ce n'est pas une colonne continue, c'est un couloir
                            return True
    ...
    def get_positions_without_corridors(self, building):
        """Retourne les positions qui ne créent pas de couloirs de largeur 1"""
        good_positions = []
        ...
        for o in ['H', 'V']:
            ...
            for y in range(self.height - L + 1):
                for x in range(self.width - l + 1):
                    ...
                    # Vérifier si ça crée des couloirs
                    if not self.has_width_1_corridors(temp_occupied):
                        good_positions.append((x, y, o))
...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Anti-couloirs")
    ...
    if st.button("🚀 Lancer", type="primary"):
        with st.spinner("Placement..."):
            ...
            st.write("🔄 Algorithme alterné anti-couloirs")
            ...
            while cult or prod:
                ...
                if cult:
                    ...
                    # Chercher les positions qui ne créent pas de couloirs
                    positions = terrain.get_positions_without_corridors(b)
                    ...
                    if best_pos:
                        ...
                        st.write(f"  ❌ Aucune position valide sans créer de couloirs")
                ...
                if prod:
                    ...
                    for idx, b in enumerate(prod):
                        # Chercher des positions dans les zones de rayonnement sans créer de couloirs
                        ...
                        for x, y, o in positions_in_zone:
                            ...
                            for i in range(l):
                                for j in range(L):
                                    if 0 <= y+j < terrain.height and 0 <= x+i < terrain.width:
                                        temp_occupied[y+j, x+i] = True
                            
                            if not terrain.has_width_1_corridors(temp_occupied):
                                good_positions.append((x, y, o))
