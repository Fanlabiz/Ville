...
try:
    ...
    def guerison_culture_score(placed_list, culture_map):
        ...
        for b in placed_list:
            ...
            if 'row' not in b or 'col' not in b or 'rotation' not in b: continue
    ...
    strong_cult = [
        b for b in culturels
        if (b.get('Culture',0) >= 1000 or b.get('Rayonnement',0) >= 2)
        and b.get('placed')
        and 'row' in b and 'col' in b and 'rotation' in b
    ]
    ...
    output.seek(0)
    st.success("Traitement terminé – v4")
    st.download_button(
        "Télécharger résultat v4.xlsx",
        output,
        "Ville_optimisee_v4.xlsx",
