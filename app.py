...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    if not real_moves:
        ...
        # Index rapide : _uid -> mv pour savoir si un bloqueur a un déplacement prévu
        ...
        while pending and iterations < len(real_moves) * 6:
            ...
            for mv in pending:
                ...
                if not blockers:
                    # Cible libre: déplacement direct
                    ...
                    # Cible bloquée : distinguer les bloqueurs selon qu'ils ont
                    # un déplacement prévu ou non.
                    # - Bloqueur AVEC déplacement prévu → attendre qu'il se déplace
                    #   de lui-même (évite la réserve inutile).
                    # - Bloqueur SANS déplacement prévu → mise en réserve obligatoire.
                    needs_reserve = []
                    for blocker in blockers:
                        buid = blocker["_uid"]
                        if buid in _done_uids or buid in _in_reserve:
                            continue
                        if buid not in _uid_to_mv:
                            needs_reserve.append(blocker)
                        # Sinon: bloqueur avec déplacement prévu → on attend

                    if needs_reserve:
                        for blocker in needs_reserve:
                            buid = blocker["_uid"]
