...
def read_from_csv(file_obj, city_name=None,
    ...
    for _, row in df_real.iterrows():
        ...
        # Tenir compte de la rotation : si Rotation=1, inverser rows et cols
        if int(row.get("Rotation", 0)) == 1:
            rows_b, cols_b = cols_b, rows_b
        placed.append({
            **bdef,
            "nom":     key,
            "r":       r_grid,
...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    # ─────────────────────────────────────
    def write_liste_batiments(ws, placed_list, culturels_list, titre):
        headers = ["Nom", "Type", "Placé", "Coord (L,C)", "Orientation",
                   "Priorité", "Placement", "Culture produite",
                   "Boost 25%", "Boost 50%", "Boost 100%",
                   "Culture reçue", "Boost atteint", "Score boost"]
        widths  = [32, 12, 8, 12, 12, 10, 12, 16, 12, 12, 12, 14, 13, 12]
        ...
        for b in sorted(placed_list, key=lambda x: (x["type"], x["nom"])):
            ...
            # Culture produite = culture émise par le bâtiment lui-même
    ...
    # ─────────────────────────────────────
    # ONGLET 1 : Liste bâtiments initiale
    # ─────────────────────────────────────
    ws1 = wb.active
    ws1.title = "Liste batiments initiale"
    orig_culturels = [b for b in original_placed if b["type"] == "Culturel"]
    write_liste_batiments(ws1, original_placed, orig_culturels, "initiale")

    # ─────────────────────────────────────
    # ONGLET 2 : Liste bâtiments optimisée
    # ─────────────────────────────────────
    ws1b = wb.create_sheet("Liste batiments optimisee")
    write_liste_batiments(ws1b, optimized, culturels, "optimisee")
