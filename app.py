...
class Terrain:
    ...
    def count_existing_producers_in_zone(self, building, x, y, orientation):
        ...
        # Parcourir tous les bâtiments déjà placés
        for b, bx, by, bo, bl, bL in self.buildings:
            if b.type == "producteur" and b.production:
                # Vérifier si ce producteur est dans la zone
                for i in range(bl):
                    for j in range(bL):
                        px, py = bx + i, by + j
                        if self.is_in_radiation_zone_building(building, x, y, orientation, px, py):
                            count += 1
                            break
                    if count > 0 and (b.id in [x['id'] for x in self.cultural_buildings]):
                        break
    ...
    def get_best_cultural_position(self, building):
        ...
        for x, y, o in positions:
            ...
            # Score combiné : zone + impact sur producteurs existants (pondération forte)
            combined_score = rayon_score + existing_impact * 1000
        ...
        # Stratégie : 
        # 1. Si possible, éviter les couloirs ET maximiser l'impact sur les producteurs existants
        if best_existing_pos is not None:
            return best_existing_pos
        # 2. Sinon, éviter les couloirs avec la meilleure zone
        if best_without_corridors is not None:
            return best_without_corridors
        # 3. En dernier recours, la meilleure position même avec couloirs
        return best_pos
...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Culturels optimisés pour producteurs existants")
    ...
    if st.button("🚀 Lancer", type="primary"):
        with st.spinner("Placement..."):
            ...
            st.write("🔄 Algorithme alterné - Culturels optimisés pour producteurs existants")
            ...
            while cult or prod:
                ...
                if cult:
                    ...
                    if best_pos:
                        # Calculer l'impact avant placement
                        ...
                        st.write(f"  ✅ à ({best_pos[0]},{best_pos[1]}) - zone: {rayon} cases, impact sur producteurs existants: {existing_impact} {corridor_msg}")
                    else:
