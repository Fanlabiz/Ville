...
# Chargée une seule fois depuis le JSON embarqué
...
_ERA_CODES = ['StoneAge','BronzeAge','MinoanEra','ClassicGreece','EarlyRome','RomanEmpire',
              'ByzantineEra','AgeOfTheFranks','FeudalAge','IberianEra','KingdomOfSicily',
              'HighMiddleAges','EarlyGothicEra','LateGothicEra']

def _lookup_evolving(nom_complet, era, level):
    """Retourne (culture, rayonnement) pour un bâtiment évolutif."""
    # Find matching key in EVOLVING_DATA
    for motif, data in _EVOLVING_DATA.items():
        if motif in nom_complet:
            era_key = era if era else 'LateGothicEra'
            pts = data.get(era_key) or data.get('LateGothicEra')
            if not pts:
                return None
            # pts = [fromLevel, culture, ray, fromLevel, culture, ray, ...]
            # JSON stores as list of lists [[fromLevel, culture, ray], ...]
            # but was stored as nested dict {era: [[l,c,r],...]}
            cult, ray = pts[0][1], pts[0][2] if isinstance(pts[0], list) else (pts[1], pts[2])
            if isinstance(pts[0], list):
                # Format [[l,c,r], [l,c,r], ...]
                for entry in pts:
                    if entry[0] <= level:
                        cult, ray = entry[1], entry[2]
                    else:
                        break
            else:
                # Format [l,c,r,l,c,r,...] flat
                for i in range(0, len(pts)-2, 3):
                    if pts[i] <= level:
                        cult, ray = pts[i+1], pts[i+2]
                    else:
                        break
            return (cult, ray)
    return None
