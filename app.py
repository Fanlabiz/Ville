    ...
    def calculate_culture(self):
        ...
        for pb in self.placed_buildings:
            if pb['info']['Type'] == 'Producteur':
                # Récupérer la zone de l'empreinte du bâtiment
                footprint = culture_map[pb['r']:pb['r']+pb['h'], pb['c']:pb['c']+pb['w']]
                
                # CORRECTION: Un bâtiment producteur reçoit la SOMME de toutes les cultures
                # des bâtiments culturels dont le rayonnement l'atteint
                # Si une seule case du bâtiment est dans la zone, il reçoit la culture
                # Mais comme plusieurs bâtiments culturels peuvent l'affecter, on prend la valeur
                # sur une case quelconque de son empreinte (toutes les cases devraient avoir la même somme)
                culture_recue = np.max(footprint) if footprint.size > 0 else 0
                
                # Déterminer le boost
                boost = 0
                # Vérifier d'abord si les seuils existent (ne sont pas vides)
        ...
        for stat in prod_stats:
            if stat['Nom'] == 'Caserne de siège':
                ...
                # Afficher aussi les cases autour pour vérifier
                for pb in self.placed_buildings:
                    if pb['info']['Nom'] == 'Caserne de siège':
                        r, c, h, w = pb['r'], pb['c'], pb['h'], pb['w']
                        footprint = culture_map[r:r+h, c:c+w]
                        self.log(f"  DEBUG - Valeurs dans l'empreinte: {footprint}")
                        break
...
def generate_excel(planner, full_queue):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for col in range(1, planner.cols + 1):
            ...
            ws.column_dimensions[column_letter].width = 25  # Un peu plus large pour les noms longs
