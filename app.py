...
class PlacementBatiments:
    ...
    def trouver_tous_placements_possibles(self, longueur, largeur, ignorer_bande=False):
        """
        Trouve tous les placements possibles pour une taille donnée
        """
        placements = []
        ...
        for i in range(self.hauteur - longueur + 1):
            for j in range(self.largeur - largeur + 1):
                if self.peut_placer_batiment(i, j, longueur, largeur):
                    bande_ok = self.verifier_bande_1(i, j, longueur, largeur, strict=not ignorer_bande)
                    if bande_ok:
                        placements.append((i, j, longueur, largeur))
    ...
    def trouver_meilleur_placement_culturel(self, batiment):
        ...
        # Phase 1: Recherche avec contrainte de bande stricte
        ...
        # Phase 2: Si aucun placement trouvé, réessayer sans contrainte de bande
        if meilleur_placement is None:
            for longueur, largeur in orientations:
                for i in range(self.hauteur - longueur + 1):
                    for j in range(self.largeur - largeur + 1):
                        if self.peut_placer_batiment(i, j, longueur, largeur):
                            # On ignore la contrainte de bande
    ...
    def trouver_placement_force(self, batiment):
        ...
        # Mélanger un peu pour éviter les placements systématiques aux mêmes endroits
        positions = []
        for longueur, largeur in orientations:
            for i in range(self.hauteur - longueur + 1):
                for j in range(self.largeur - largeur + 1):
                    if self.peut_placer_batiment(i, j, longueur, largeur):
                        positions.append((i, j, longueur, largeur))
        
        if positions:
            # Retourner la première position trouvée (ou une aléatoire pour plus de variété)
            return positions[0]  # ou random.choice(positions) si vous voulez varier
    ...
    def executer_placement(self):
        """
        Exécute l'algorithme de placement avec plusieurs passes
        """
        ...
        # Phase 1: Placement optimal avec contraintes
        st.write("### Phase 1: Placement optimal")
        index_producteur = 0
        iteration = 0
        max_iterations = 200
        ...
        # Phase 2: Placement forcé pour les bâtiments restants
        producteurs_restants = producteurs[index_producteur:]
        culturels_restants = culturels
        
        if producteurs_restants or culturels_restants:
            st.write("### Phase 2: Placement forcé pour les bâtiments restants")
            st.write(f"Bâtiments restants: {len(producteurs_restants)} producteurs, {len(culturels_restants)} culturels")
            
            # Placer d'abord les culturels restants
            for batiment in culturels_restants:
                ...
                if placement:
                    ...
                    st.warning(f"Impossible de placer {batiment['nom']} même en mode forcé!")
            
            # Puis les producteurs restants
            for batiment in producteurs_restants:
                placement = self.trouver_placement_force(batiment)
                if placement:
                    x, y, longueur, largeur = placement
                    # Calculer la culture reçue pour ce placement forcé
        ...
        st.write(f"Tentatives totales: {self.stats_placement['tentatives']}")
...
def generer_fichier_resultat(terrain_original, terrain_place, batiments_places, stats_culture, tous_les_batiments):
    ...
    for row in dataframe_to_rows(df_stats, index=False, header=True):
        ws5.append(row)
...
# Titre
st.title("🏗️ Optimiseur de Placement de Bâtiments (Version Améliorée)")
st.markdown("---")
        ...
        if 'terrain_place' in st.session_state:
            ...
            for bat in st.session_state['batiments_complets']:
                ...
                if quantite_placee < quantite_demandee:
                    non_places.append({
                        'Nom': nom,
                        'Reste à placer': quantite_demandee - quantite_placee
                    })
...
# Footer
st.markdown("---")
st.markdown("🚀 Application développée pour l'optimisation de placement de bâtiments - Version améliorée avec placement forcé")