...
##############################
# CLASSE DE PLACEMENT AVEC STRATÉGIE MER
##############################
class PlacementBatiments:
    ...
    def verifier_bande_1(self, x, y, longueur, largeur, strict=True):
        ...
        Si strict=False, on accepte les bandes de largeur 1 si nécessaire
        """
        if not strict:
            return True  # On ignore cette contrainte en mode non-strict
    ...
    def trouver_maximum_empty_rectangles(self):
        """
        Implémente l'algorithme des Maximum Empty Rectangles (MER)
        Retourne la liste des plus grands rectangles vides sur le terrain
        """
        mer_list = []
        
        # Pour chaque case libre, on cherche à étendre le rectangle
        ...
        # Filtrer pour ne garder que les rectangles maximaux (non inclus dans d'autres)
        mer_list_filtree = []
        for mer in mer_list:
            est_maximal = True
    ...
    def trouver_placement_dans_mer(self, batiment, mer_list, avec_bande=True):
        """
        Trouve le meilleur placement pour un bâtiment dans les MER disponibles
        """
        meilleur_score = -1
        ...
        for mer in mer_list:
            for longueur, largeur in orientations:
                # Vérifier si le bâtiment peut tenir dans ce MER
                if longueur <= mer['hauteur'] and largeur <= mer['largeur']:
                    # Essayer toutes les positions dans ce MER
                    for di in range(mer['hauteur'] - longueur + 1):
                        for dj in range(mer['largeur'] - largeur + 1):
                            x = mer['x'] + di
                            y = mer['y'] + dj
                            
                            if self.peut_placer_batiment(x, y, longueur, largeur):
                                if avec_bande and not self.verifier_bande_1(x, y, longueur, largeur, strict=True):
                                    continue
                                
                                # Calculer le score (culture reçue + préservation de l'espace)
                                culture = self.calculer_culture_pour_position(x, y, longueur, largeur)
                                
                                # Plus le MER est petit, moins on veut l'utiliser pour un grand bâtiment
                                score_mer = mer['surface'] / (longueur * largeur)
                                
                                # Score total
                                score = culture * 1000 + score_mer * 100
                                
                                if score > meilleur_score:
                                    meilleur_score = score
                                    meilleur_placement = (x, y, longueur, largeur, culture)
    ...
    def trouver_meilleur_placement_culturel(self, batiment):
        ...
        """
        # Obtenir les MER actuels
        mer_list = self.trouver_maximum_empty_rectangles()
        
        # Essayer d'abord avec contrainte de bande
        placement = self.trouver_placement_dans_mer(batiment, mer_list, avec_bande=True)
        
        # Si aucun placement trouvé, essayer sans contrainte de bande
        if placement is None:
            self.stats_placement['contraintes_bande_ignorees'] += 1
            placement = self.trouver_placement_dans_mer(batiment, mer_list, avec_bande=False)
    ...
    def trouver_placement_force(self, batiment):
        """
        Trouve n'importe quel placement possible pour un bâtiment (dernier recours)
        Utilise les MER pour une recherche plus intelligente
        """
        mer_list = self.trouver_maximum_empty_rectangles()
        ...
        for mer in mer_list:
            ...
            for longueur, largeur in orientations:
                if longueur <= mer['hauteur'] and largeur <= mer['largeur']:
                    # Prendre la première position disponible dans ce MER
                    for di in range(mer['hauteur'] - longueur + 1):
                        for dj in range(mer['largeur'] - largeur + 1):
                            x = mer['x'] + di
                            y = mer['y'] + dj
                            ...
                            if self.peut_placer_batiment(x, y, longueur, largeur):
                                culture = self.calculer_culture_pour_position(x, y, longueur, largeur)
                                return (x, y, longueur, largeur, culture)
    ...
    def executer_placement(self):
        """
        Exécute l'algorithme de placement avec la stratégie MER
        """
        ...
        st.write("### Phase 1: Placement avec stratégie MER")
        index_producteur = 0
        iteration = 0
        max_iterations = 200
        ...
        while (culturels or index_producteur < len(producteurs)) and iteration < max_iterations:
            ...
            if culturels:
                ...
                if placement:
                    ...
                    # Si on ne peut pas placer, on le remet dans la liste pour plus tard
            ...
            while (index_producteur < len(producteurs) and 
                ...
                if placement:
                    ...
                    # On passe au suivant temporairement
                    index_producteur += 1
                    tentative_producteurs += 1
        
        # Phase 2: Placement forcé avec MER pour les bâtiments restants
        producteurs_restants = producteurs[index_producteur:]
        culturels_restants = culturels
        tous_restants = culturels_restants + producteurs_restants
        
        if tous_restants:
            st.write("### Phase 2: Placement forcé avec MER")
            st.write(f"Bâtiments restants: {len(tous_restants)}")
            
            # Trier par taille décroissante
            tous_restants.sort(key=lambda x: -(x['longueur'] * x['largeur']))
            
            for batiment in tous_restants:
                placement = self.trouver_placement_force(batiment)
                if placement:
                    x, y, longueur, largeur, culture = placement
                    self.placer_batiment(batiment, x, y, longueur, largeur, culture)
                    self.stats_placement['placements_reussis'] += 1
                    st.write(f"✅ {batiment['nom']} placé en ({x},{y})")
                else:
                    st.warning(f"❌ Impossible de placer {batiment['nom']}")
        ...
        # Afficher les MER finaux
        mer_finaux = self.trouver_maximum_empty_rectangles()
        st.write(f"Espaces libres restants: {len(mer_finaux)} rectangles vides")
        if mer_finaux:
            st.write("Plus grands espaces libres:")
            for mer in mer_finaux[:5]:
                st.write(f"  - Position ({mer['x']},{mer['y']}) : {mer['hauteur']}x{mer['largeur']} ({mer['surface']} cases)")
...
# Titre
st.title("🏗️ Optimiseur de Placement de Bâtiments (Stratégie MER)")
st.markdown("---")
...
# Footer
st.markdown("---")
st.markdown("🚀 Application développée avec la stratégie des Maximum Empty Rectangles (MER)")