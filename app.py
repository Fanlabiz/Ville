...
def _short_building_name(nom_complet):
    """
    Extrait le nom court lisible depuis le nom technique RoC.
    Structure : Building_[Era]_[Categorie]_[SousType]_[Niveau]
    → garde uniquement [SousType] (index 3), converti CamelCase → mots.
    Harbor (4 parties) : garde [Nom] (index 2).
    """
    import re as _re2
    if nom_complet == "PURCHASED_BLOCK":
        return "PURCHASED_BLOCK"
    parts = nom_complet.split("_")
    raw = parts[3] if len(parts) >= 5 else parts[2] if len(parts) >= 3 else nom_complet
    return _re2.sub(r'(?<=[a-z])(?=[A-Z])', ' ', raw)


def _detect_type_csv(row):
    """Producteur si Seuil100>0, Culturel si Culture>0, sinon Neutre."""
    if row["Seuil100"] > 0:
        return "Producteur"
    if row["Culture"] > 0:
        return "Culturel"
...
def _building_category(nom_complet):
    """
    Retourne la catégorie fonctionnelle du bâtiment pour les priorités :
    'Barracks', 'Farm', 'Home', ou None.
    Détecté depuis le nom technique (parties 3 ou 4).
    """
    n = nom_complet.upper()
    if "BARRACKS" in n:
        return "Barracks"
    if "FARM" in n or "CAMEL" in n or "IRRIGATION" in n or "WORKSHOP" in n or "MERCHANT" in n:
        return "Farm"
    if "HOME" in n:
        return "Home"
    return None
...
def read_from_csv(file_obj, city_name=None,
                  prio_barracks=10, prio_farm=0, prio_home=0):
    """
    Lit un CSV exporté par le userscript RoC et construit :
      terrain_grid, max_r, max_c, placed, buildings_def, available_cities

    - PURCHASED_BLOCK : zone vide achetée → contribue au terrain, exclue de placed.
    - Ligne ≥ 100000  : bâtiment en réserve → ignoré.
    - prio_barracks / prio_farm / prio_home : priorités appliquées aux Producteurs
      selon leur catégorie fonctionnelle.
    """
    ...
    if df.empty:
        raise ValueError(f"Aucun bâtiment valide pour '{city_name}'.")

    df_purchased = df[df["Nom_complet"] == "PURCHASED_BLOCK"].copy()
    df_real      = df[df["Nom_complet"] != "PURCHASED_BLOCK"].copy()
    if df_real.empty:
        raise ValueError(f"Aucun bâtiment réel pour '{city_name}'.")

    # ── Terrain : cases valides = bâtiments réels + PURCHASED_BLOCK ──
    valid_interior = set()
    for _, row in _pd.concat([df_real, df_purchased]).iterrows():
        r0, c0 = int(row["Ligne"]), int(row["Colonne"])
        for dr in range(int(row["Hauteur"])):
            for dc in range(int(row["Largeur"])):
    ...
    for (r_abs, c_abs) in valid_interior:
        rg, cg = r_abs - grid_lig_min, c_abs - grid_col_min
        if 0 <= rg < max_r and 0 <= cg < max_c:
            terrain_grid[rg][cg] = None

    # ── Catalogue (buildings_def) ──
    catalog_rows = {}
    for _, row in df_real.iterrows():
        key = str(row["Nom_complet"]).strip()
        if key not in catalog_rows:
            btype = _detect_type_csv(row)
            cat   = _building_category(key)
            if btype == "Producteur":
                if cat == "Barracks":
                    prio = float(prio_barracks)
                elif cat == "Farm":
                    prio = float(prio_farm)
                elif cat == "Home":
                    prio = float(prio_home)
                else:
                    prio = 0.0
            else:
                prio = 0.0
            catalog_rows[key] = {
                "nom":         _short_building_name(key),
    ...
    for _, row in df_real.iterrows():
        ...
        placed.append({
            **bdef,
            "nom":  bdef["nom"],
            "r":    int(row["Ligne"])   - grid_lig_min,
            "c":    int(row["Colonne"]) - grid_col_min,
            "rows": int(row["Hauteur"]),
            "cols": int(row["Largeur"]),
        })
...
if _input_mode == "📁 Fichier Excel (.xlsx)":
    ...
    uploaded_csv = st.file_uploader(
        ...
        type=["csv"],
        help="Fichier généré par le userscript 'Rise of Cultures - Export Ville'."
    )
    if uploaded_csv is not None:
        # Sélection de la ville si plusieurs présentes
        import pandas as _pd_cities
        try:
            _df_cities = _pd_cities.read_csv(uploaded_csv)
            uploaded_csv.seek(0)
            _csv_cities = sorted(_df_cities["Ville"].unique().tolist())
        except Exception:
            _csv_cities = []
        if len(_csv_cities) > 1:
            _selected_city = st.selectbox(
                "Plusieurs villes détectées — choisissez celle à importer :",
                _csv_cities
            )
        elif len(_csv_cities) == 1:
            _selected_city = _csv_cities[0]
            st.info(f"Ville détectée : **{_selected_city}**")
        else:
            _selected_city = None

        # ── Priorités des bâtiments ──
        st.subheader("Priorités des bâtiments producteurs")
        st.caption(
        ...
        with _pc1:
            _prio_barracks = st.number_input(
                "🏰 Barracks (casernes)", min_value=0, max_value=10, value=8, step=1,
                help="Bâtiments dont le nom contient 'Barracks'"
            )
        with _pc2:
            _prio_farm = st.number_input(
                "🌾 Farm (fermes)", min_value=0, max_value=10, value=3, step=1,
                help="Bâtiments dont le nom contient 'Farm'"
            )
        with _pc3:
            _prio_home = st.number_input(
                "🏠 Home (maisons)", min_value=0, max_value=10, value=2, step=1,
                help="Bâtiments dont le nom contient 'Home'"
            )
...
# ── Chargement CSV ──
if uploaded_csv is not None and '_selected_city' in dir() and _selected_city:
    _active_name = uploaded_csv.name + "_" + _selected_city
    ...
    try:
        ...
        st.error(f"Impossible de lire le CSV : {_e_csv}")
        st.stop()

# Si un nouveau fichier Excel est charge, on efface les resultats precedents
if uploaded:
