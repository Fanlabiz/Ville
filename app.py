...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    def ffd_pure(nn_ordered, n_ordered):
        ...
        # ── Phase 2 : Placement des top_prods et de leurs culturels ──
        # Stratégie pour les boost100_required (max) :
        #   - Placer chaque max en cherchant la position la plus proche du groupe déjà formé
        #   - Après chaque max, intercaler immédiatement les petits culturels (surf <= 2)
        #     dans les interstices libres du groupe (zone voisinage)
        #   - Une fois tous les max placés, placer les grands culturels autour du groupe
        #     en cherchant la position qui maximise le gain sur tous les max non encore à 100%
        #   - Enfin, culturels pour les autres top_prods (logique d'origine)
        ...
        # Séparer culturels petits (surf <= 2) et grands (surf > 2) du pool
        _cults_petits = sorted(
            [c for c in cults_pool if c["rows"] * c["cols"] <= 2],
            key=lambda c: -(c["culture"] * max(c.get("rayonnement", 1), 1))
        )
        _cults_grands = sorted(
            [c for c in cults_pool if c["rows"] * c["cols"] > 2],
            key=lambda c: -(c["culture"] * max(c.get("rayonnement", 1), 1))
        )

        def _zone_voisinage(placed_refs, rayon=1):
            """Cases libres dans le voisinage immédiat (rayon cases) des bâtiments placés."""
            groupe_cells = set()
            for b in placed_refs:
                for dr in range(b["rows"]):
                    for dc in range(b["cols"]):
                        groupe_cells.add((b["r"]+dr, b["c"]+dc))
            voisins = set()
            for (rr, cc) in groupe_cells:
                for dr in range(-rayon, rayon+1):
                    for dc in range(-rayon, rayon+1):
                        cell = (rr+dr, cc+dc)
                        if (0 <= cell[0] < max_r and 0 <= cell[1] < max_c
                                and cell not in groupe_cells):
                            voisins.add(cell)
            return voisins

        def _placer_petits_culturels_interstices(placed_max_refs):
            """Intercale les petits culturels dans les interstices du groupe max."""
            nonlocal n_ok
            zone = _zone_voisinage(placed_max_refs, rayon=1)
            for cult in list(_cults_petits):
                if id(cult) in cults_placed_ids:
                    continue
                occ = build_occ(result)
                ray = cult.get("rayonnement", 1)
                best_pos = None; best_gain = -1
                for rows, cols in [(cult["rows"], cult["cols"]), (cult["cols"], cult["rows"])]:
                    for (cr, cc) in zone:
                        r, c = cr, cc
                        if not can_place(r, c, rows, cols, x_grid, occ, max_r, max_c):
                            continue
                        # Calculer le gain sur tous les max
                        ...
                        gain = sum(cult["culture"] for p in placed_max_refs
                                   if cells_of(p) & cult_zone)
                        if gain > best_gain:
                            best_gain = gain; best_pos = (r, c, rows, cols)
                if best_pos and best_gain > 0:
                    r, c, rows, cols = best_pos
                    result.append({**cult, "r": r, "c": c, "rows": rows, "cols": cols})
                    cults_placed_ids.add(id(cult)); n_ok += 1
                    # Mettre à jour la zone avec ce nouveau culturel
                    zone |= _zone_voisinage([result[-1]], rayon=1) - _zone_voisinage(placed_max_refs, rayon=0)

        # ── Phase 2a : placer les max un par un en alternant avec les petits culturels ──
        _placed_max_prods = []
        for prod in _max_prods:
            occ = build_occ(result)
            # Position cible : plus proche du groupe déjà formé (centre de gravité des max)
            if _placed_max_prods:
                _tr = sum(b["r"]+b["rows"]/2 for b in _placed_max_prods) / len(_placed_max_prods)
                _tc = sum(b["c"]+b["cols"]/2 for b in _placed_max_prods) / len(_placed_max_prods)
            else:
            ...
            if chosen:
                ...
                # Intercaler immédiatement les petits culturels dans les interstices
                _placer_petits_culturels_interstices(_placed_max_prods)
            else:
                n_fail += 1

        # ── Phase 2b : grands culturels pour les max (gain maximal simultané) ──
        if _placed_max_prods:
            _avail_grands = [c for c in _cults_grands if id(c) not in cults_placed_ids]
            while True:
                _cults_act = [b for b in result if b["type"] == "Culturel"]
                _max_not_done = [p for p in _placed_max_prods
                                 if culture_received(p, _cults_act) < p.get("boost100", 9999)]
                if not _max_not_done:
                    break
                _avail = [c for c in _avail_grands if id(c) not in cults_placed_ids]
                if not _avail:
                    # Essayer aussi les petits culturels restants si les grands ne suffisent plus
                    _avail = [c for c in cults_pool if id(c) not in cults_placed_ids]
                    if not _avail:
                        break
                occ = build_occ(result)
                        ...
                        for r in range(max_r):
                            for c in range(max_c):
                                ...
                                zone = {(rz, cz)
                                        ...
                                        if not (r0<=rz<=r1 and c0<=cz<=c1)
                                        and 0<=rz<max_r and 0<=cz<max_c}
                                gain = sum(cult["culture"] for p in _max_not_done
                                           if cells_of(p) & zone)
                                if gain > best_gain:
                                    best_gain = gain; best_cult = cult
                                    best_pos = (r, c, rows, cols)
        ...
        # ── Phase 2c : placer les autres top_prods au centre ──
        ...
        # ── Phase 2d : culturels pour les autres top_prods (logique d'origine) ──
