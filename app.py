        ...
        for w, h in dims:
            for r in range(self.rows - h + 1):
                for c in range(self.cols - w + 1):
                    if b['Type'] == 'Neutre' and not self.is_adjacent_to_X(r, c, w, h): continue
                    if self.can_place(r, c, w, h, buildings[1:]):
                        if self.try_placement(b, r, c, w, h, buildings): return True
...
def generate_excel(planner, full_queue):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        # 1. CARTE DE CULTURE (SOMME CUMULÉE)
        ...
        for pb in planner.placed_buildings:
            if pb['info']['Type'] == 'Producteur':
                # On récupère la culture cumulée sur la zone du bâtiment
                c_recue = culture_map[pb['r'], pb['c']]
                
                boost = 0
                b100 = pb['info'].get('Boost 100%')
                b50 = pb['info'].get('Boost 50%')
                b25 = pb['info'].get('Boost 25%')
                
                if pd.notnull(b100) and c_recue >= b100: boost = 100
                elif pd.notnull(b50) and c_recue >= b50: boost = 50
                elif pd.notnull(b25) and c_recue >= b25: boost = 25
        ...
        pd.DataFrame(list(totals.items()), columns=["Type Production", "Culture Cumulée"]).to_excel(writer, sheet_name="Synthese", index=False)
        ...
        summary = [
            ...
            ["Nombre de bâtiments non placés", len(full_queue) - len(planner.placed_buildings)],
            ["Statut Algorithme", "STOP: LIMITE JOURNAL" if planner.interrupted else "OK"]
        ]
...
# --- STREAMLIT ---
st.title("Optimiseur de Ville - Fix Culture 3208 🏗️")
...
if uploaded:
    ...
    # Tri et file d'attente
    neutres = b_df[b_df['Type'] == 'Neutre'].sort_values(['Longueur', 'Largeur'], ascending=False)
    culturels = b_df[b_df['Type'] == 'Culturel'].sort_values(['Longueur', 'Largeur'], ascending=False)
    producteurs = b_df[b_df['Type'] == 'Producteur'].sort_values(['Longueur', 'Largeur'], ascending=False)
    
    full_queue = []
    for _, r in neutres.iterrows():
        for _ in range(int(r['Quantite'])): full_queue.append(r.to_dict())
    for _, r in culturels.iterrows():
        for _ in range(int(r['Quantite'])): full_queue.append(r.to_dict())
    for _, r in producteurs.iterrows():
        for _ in range(int(r['Quantite'])): full_queue.append(r.to_dict())

    planner = CityPlanner(t_df.values)
    planner.solve(full_queue)
    st.download_button("Télécharger Resultat_Placement.xlsx", generate_excel(planner, full_queue), "Resultat_Placement.xlsx")
