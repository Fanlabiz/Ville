...
class PlacementBatiments:
    ...
    def verifier_pas_de_doublons(self):
        """
        Vérifie qu'il n'y a pas de bâtiments en double
        """
        compteurs = {}
        for bat in self.batiments_places:
            cle = (bat['nom'], bat['x'], bat['y'])
            if cle in compteurs:
                return False
            compteurs[cle] = 1
        
        # Vérifier aussi que les quantités ne dépassent pas la demande
        quantites = {}
        for bat in self.batiments_places:
            if bat['nom'] not in quantites:
                quantites[bat['nom']] = 0
            quantites[bat['nom']] += 1
            ...
            for dx in range(-3, 4):
                for dy in range(-3, 4):
                    ...
                    if self.peut_placer_batiment(x_new, y_new, longueur, largeur):
                        ...
                        # ✅ Vérifier qu'on n'a pas créé de doublons
                        if self.verifier_pas_de_doublons():
...
# Footer
st.markdown("---")
st.markdown("🚀 Application développée avec l'algorithme de recherche tabou (avec vérification des doublons)")