...
try:
    ...
    def calculate_total_score(placed_list, culture_map):
        score = 0.0
        for b in placed_list:
            if b['Type'] != 'Producteur' or not b.get('placed'):
                continue
    ...
    # ─── Phase 2 : Optimisation Guérison-First ──────────────────────────────────
    ...
    # 2.1 – Producteurs Guérison (très agressif)
    st.write("   → 2.1 : optimisation producteurs Guérison")
    ...
    for building in guerison_prods:
        ...
        for try_rot in [False, True]:
            ...
            candidates = find_positions(grid_work, hh, ww, False)[:600]
    ...
    # 2.2 – Culturels (pour mieux couvrir les Guérison)
    st.write("   → 2.2 : optimisation culturels (à développer)")
    for building in [b for b in to_optimize if b['Type'] == 'Culturel']:
        # Même logique que ci-dessus, mais avec valeur -3 au placement final
        # Pour l'instant on laisse vide pour éviter l'erreur d'indentation
        pass   # ← À implémenter quand tu veux activer cette partie

    # 2.3 – Autres producteurs (léger)
    st.write("   → 2.3 : autres producteurs (léger)")
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Terrain visuel avec fusion et couleurs
        wb = writer.book
    ...
    st.download_button(
        "Télécharger résultat.xlsx",
        output,
        "Ville_optimisee_v3.xlsx",
