...
def optimiser(placed_list, inside_mask, grid_shape, prio_order, n_passes=3, progress_cb=None):
    """
    Optimisation en 3 phases par passe :
    A) Déplacements individuels exhaustifs (meilleure position pour chaque bâtiment)
       → trié par priorité décroissante : producteurs prioritaires d'abord, puis culturels
    B) Swaps de paires (toutes tailles)
    C) Déplacements coordonnés culturel+producteur pour débloquer des paliers de boost
    """
    ...
    def prio_sort_key(bat):
        # Producteurs prioritaires d'abord (priorité 1 = la plus importante)
    ...
    def meilleure_position(idx, state):
        """Cherche la meilleure position pour le bâtiment idx dans state."""
        bat = state[idx]
        best_s = score_total(state, prio_order)
            ...
            for r in range(rows):
                for c in range(cols):
                    if r == best_r and c == best_c and h == best_h and w == best_w:
                        continue
                    if peut_placer(bat, r, c, orientation, state, inside_mask, grid_shape, exclude_id=idx):
                        cand = copy.deepcopy(state)
                        cand[idx]['row'] = r; cand[idx]['col'] = c
                        cand[idx]['h'] = h;   cand[idx]['w'] = w
                        cand[idx]['orientation'] = orientation
    ...
    for pass_num in range(1, n_passes + 1):
        ...
        for i in order:
            ...
            if found:
                best[i]['row'] = r; best[i]['col'] = c
                best[i]['h'] = h;   best[i]['w'] = w
                best[i]['orientation'] = ori
                calculer_culture(best, grid_shape)
                best_score = s
            ...
            for j in range(i + 1, n):
                ...
                for ori_i in ['H', 'V']:
                    ...
                    for ori_j in ['H', 'V']:
                        ...
                        # Vérifier que les deux emplacements cibles sont libres
                        # (en retirant les deux bâtiments)
                        cand = copy.deepcopy(best)
                        cand[i]['row'] = rj; cand[i]['col'] = cj
                        cand[i]['h'] = hi;   cand[i]['w'] = wi; cand[i]['orientation'] = ori_i
                        cand[j]['row'] = ri; cand[j]['col'] = ci
                        cand[j]['h'] = hj;   cand[j]['w'] = wj; cand[j]['orientation'] = ori_j

                        # Vérification de non-chevauchement après swap
                        occupied = np.zeros((rows, cols), dtype=bool)
                        valid = True
                        for k, b in enumerate(cand):
                            for dr in range(b['h']):
                                for dc in range(b['w']):
                                    r2, c2 = b['row']+dr, b['col']+dc
                                    if r2 >= rows or c2 >= cols or not inside_mask[r2, c2]:
                                        valid = False; break
                                    if occupied[r2, c2]:
                                        valid = False; break
                                    occupied[r2, c2] = True
        ...
        # ── Phase C : regroupement des culturels autour des producteurs prioritaires ──
        # Pour chaque producteur prioritaire, on déplace les culturels vers lui
        # même si un déplacement isolé n'améliore pas le score global immédiatement.
        # L'objectif est de débloquer un palier de boost (25→50→100%).
        prod_order = [i for i in range(n)
                      if best[i]['type'] == 'Producteur' and best[i].get('priorite', 0) > 0]
        prod_order.sort(key=lambda i: best[i].get('priorite', 99))
        ...
        for pi in prod_order:
            ...
            for seuil in [prod.get('boost25',0), prod.get('boost50',0), prod.get('boost100',0)]:
                if seuil and cr < seuil:
                    next_seuil = seuil
                    break
            if next_seuil is None:
                continue  # déjà au max

            # Tenter de placer chaque culturel à la position qui maximise
            # la culture reçue par ce producteur (indépendamment du score global)
            for ci_idx in cult_indices:
                cult = best[ci_idx]
                ray = cult['rayonnement']
                best_cult_score = None
                best_pos = None

                for orientation in ['H', 'V']:
                    h = cult['largeur'] if orientation == 'H' else cult['longueur']
                    w = cult['longueur'] if orientation == 'H' else cult['largeur']
                    # Zone utile : placer le culturel de sorte que son rayonnement touche le producteur
                    # Le culturel (r,c)..(r+h-1,c+w-1) touche le producteur si :
                    # r-ray <= r1p et r+h-1+ray >= r0p  ET idem pour colonnes
                    # → on balaye tout le terrain, la vérification de touching est implicite
                    for r in range(rows):
                        for c in range(cols):
                            if r == cult['row'] and c == cult['col'] and h == cult['h'] and w == cult['w']:
                                continue
                            if not peut_placer(cult, r, c, orientation, best, inside_mask, grid_shape, exclude_id=ci_idx):
                                continue
                            # Calculer la culture reçue par le producteur avec ce placement
                            # (sans modifier best)
                            zone_r0 = r - ray; zone_r1 = r + h - 1 + ray
                            zone_c0 = c - ray; zone_c1 = c + w - 1 + ray
                            touches = (r0p <= zone_r1 and r1p >= zone_r0 and
                                       c0p <= zone_c1 and c1p >= zone_c0)
                            if not touches:
                                continue
                            # Simuler culture reçue totale
                            cand = copy.deepcopy(best)
                            cand[ci_idx]['row'] = r; cand[ci_idx]['col'] = c
                            cand[ci_idx]['h'] = h;   cand[ci_idx]['w'] = w
                            cand[ci_idx]['orientation'] = orientation
                            calculer_culture(cand, grid_shape)
                            new_cr = cand[pi].get('culture_recue', 0)
                            gs = score_total(cand, prio_order)
                            # Accepter si ça améliore la culture reçue par ce producteur
                            # ET que ça ne dégrade pas trop le score global (≥ 95% du meilleur)
                            if new_cr > prod.get('culture_recue', 0) and gs >= best_score * 0.95:
                                if best_cult_score is None or new_cr > best_cult_score:
                                    best_cult_score = new_cr
                                    best_pos = (r, c, h, w, orientation, gs, cand)

                if best_pos is not None:
                    r, c, h, w, ori, gs, cand = best_pos
                    best = cand
                    best[ci_idx]['row'] = r; best[ci_idx]['col'] = c
                    best[ci_idx]['h'] = h;   best[ci_idx]['w'] = w
                    best[ci_idx]['orientation'] = ori
                    calculer_culture(best, grid_shape)
                    if gs > best_score:
                        best_score = gs
                    # Mettre à jour culture_recue du producteur pour la prochaine itération
                    prod = best[pi]
