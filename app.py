...
if 'resultats_disponibles' not in st.session_state:
    st.session_state.resultats_disponibles = False
...
def placer_batiments_avec_backtracking(terrain, batiments):
    ...
    try:
        ...
        # Si on atteint la limite dès le début, on remonte l'exception
        raise
...
if uploaded_file:
    # Lecture du fichier (en dehors du try pour que les erreurs soient visibles)
    try:
            ...
            with st.spinner("Optimisation en cours..."):
                ...
                try:
                    ...
                    if str(e) == "LIMITE_1000_ATTEINTE":
                        st.session_state.limite_atteinte = True
                        st.session_state.optimisation_terminee = True
                        # Ne pas afficher de warning ici, on le fera dans l'affichage
                    else:
                ...
                if terrain_optimise:
                    ...
                    st.session_state.resultats_disponibles = True
...
# AFFICHAGE DES RÉSULTATS - EN DEHORS DU TRY/EXCEPT
if st.session_state.resultats_disponibles and st.session_state.solution:
    solution = st.session_state.solution
    
    if solution['limite_atteinte']:
        st.warning("⚠️ Limite de 1000 entrées journal atteinte - Résultats PARTIELS")
    ...
    # BOUTON DE TÉLÉCHARGEMENT - TOUJOURS PRÉSENT QUAND RÉSULTATS DISPONIBLES
...
elif uploaded_file and not st.session_state.resultats_disponibles and 'df_terrain' in locals():
    # Message si le fichier est chargé mais pas encore optimisé
    st.info("👆 Cliquez sur 'Lancer l'optimisation' dans la barre latérale pour commencer")
...
st.markdown("---")
st.markdown("🔒 Optimiseur de placement v3.3 - Arrêt à 1000 entrées avec téléchargement garanti")