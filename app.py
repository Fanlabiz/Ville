...
class Terrain:
    ...
    def get_radiation_score(self, building, x, y, orientation):
        """
        Calcule un score pour maximiser la zone de rayonnement dans les limites du terrain.
        """
        l, L = building.get_dimensions(orientation)
        ...
        # Calculer la taille de la zone de rayonnement qui serait dans le terrain
        zone_size = 0
        for dx in range(max(0, x - rayon), min(self.width, x + l + rayon)):
            for dy in range(max(0, y - rayon), min(self.height, y + L + rayon)):
                if self.is_in_radiation_zone(temp_cultural, dx, dy):
                    zone_size += 1
        
        # Bonus si toute la zone est dans le terrain
        theoretical_max = (l + 2*rayon) * (L + 2*rayon) - (l * L)
        if zone_size == theoretical_max:
            zone_size *= 2  # Double bonus si la zone est entièrement dans le terrain
        
        return zone_size
    
    def find_all_contiguous_areas(self, temp_occupied=None):
        """
        Trouve toutes les zones contiguës de cases libres et les trie par taille.
        """
        ...
        for y in range(self.height):
            for x in range(self.width):
                if self.grid[y, x] == 1 and not temp_occupied[y, x] and not visited[y, x]:
                    ...
                    while queue:
                        ...
                        for dx, dy in [(0,1), (1,0), (0,-1), (-1,0)]:
                            ...
                            if (0 <= nx < self.width and 0 <= ny < self.height and 
                                self.grid[ny, nx] == 1 and not temp_occupied[ny, nx] and 
                                not visited[ny, nx]):
                                visited[ny, nx] = True
                                queue.append((nx, ny))
    ...
    def can_place_building_anywhere(self, building, ignore_occupied=None):
        """
        Vérifie si un bâtiment peut être placé quelque part sur le terrain.
        """
        if ignore_occupied is None:
            ignore_occupied = self.occupied
        ...
        for orientation in ['H', 'V']:
            ...
            for y in range(self.height - L + 1):
                for x in range(self.width - l + 1):
                    ...
                    for i in range(l):
                        for j in range(L):
                            if self.grid[y + j, x + i] == 0 or ignore_occupied[y + j, x + i]:
                                can = False
                                break
                        if not can:
                            break
    ...
    def get_space_preservation_score(self, x, y, longueur, largeur, remaining_buildings):
        """
        Calcule un score de préservation d'espace très strict.
        """
        ...
        # Trouver toutes les zones contiguës après placement
        areas = self.find_all_contiguous_areas(temp_occupied)
        
        if not remaining_buildings:
            return 10000  # Plus rien à placer, score maximal
        
        # Compter combien de bâtiments restants peuvent être placés dans chaque zone
        total_can_place = 0
        buildings_copy = remaining_buildings.copy()
        
        for area in areas:
            area_size = len(area)
            # Chercher le plus grand bâtiment qui peut tenir dans cette zone
            possible_buildings = [b for b in buildings_copy if b.get_area() <= area_size]
            if possible_buildings:
                # Prendre le plus grand
                biggest = max(possible_buildings, key=lambda b: b.get_area())
                buildings_copy.remove(biggest)
                total_can_place += 1
        
        # Score = nombre de bâtiments qui peuvent être placés
        preservation_score = total_can_place * 1000
        
        # Pénalité si on ne peut pas placer tous les bâtiments
        if total_can_place < len(remaining_buildings):
            preservation_score -= 10000
...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Préservation stricte des zones")
    ...
    if st.button("🚀 Lancer l'optimisation", type="primary"):
        with st.spinner("Placement en cours..."):
            ...
            # Étape 1: Placer les culturels en priorité
            # Trier les culturels par rayon décroissant pour maximiser l'impact
            ...
            for b in culturels:
                ...
                for x, y, o in positions:
                    ...
                    # Score basé sur la zone de rayonnement dans le terrain
                    rayon_score = terrain_final.get_radiation_score(b, x, y, o)
                    
                    # Bonus supplémentaire si le bâtiment est bien centré
                    centre_x, centre_y = terrain_final.width // 2, terrain_final.height // 2
                    dist_centre = abs(x + l//2 - centre_x) + abs(y + L//2 - centre_y)
                    centrage_bonus = -dist_centre * 10
                    
                    score = rayon_score * 100 + centrage_bonus
                ...
                if meilleure_pos:
                    ...
                    # Calculer la zone de rayonnement réelle
                    zone_size = terrain_final.get_radiation_score(b, x, y, o)
                    st.write(f"✅ {b.nom} placé à ({x},{y}) - zone rayonnement: {zone_size} cases")
                else:
                    b.failed = True
                    stats["non_places"] += 1
                    st.write(f"❌ Impossible de placer {b.nom}")
            
            # Étape 2: Placer les producteurs par ordre de taille décroissante
            producteurs.sort(key=lambda b: (-b.get_area(), b.production))
            ...
            for i, b in enumerate(producteurs):
                ...
                # Récupérer les bâtiments restants (ceux pas encore placés)
                remaining = [bb for bb in producteurs[i+1:] if not bb.failed]
                ...
                for x, y, o in positions:
                    ...
                    # Score de préservation d'espace (très strict)
                    space_score = terrain_final.get_space_preservation_score(x, y, l, L, remaining)
                    score += space_score * 1000  # Pondération extrêmement importante
                ...
                if meilleure_pos:
                    ...
                    # Afficher les zones restantes
                    areas = terrain_final.find_all_contiguous_areas()
                    st.write(f"✅ {b.nom} à ({x},{y}) - culture: {culture:.0f} {boost_text} - {len(areas)} zones libres, plus grande: {len(areas[0]) if areas else 0} cases")
                else:
                    b.failed = True
                    stats["non_places"] += 1
                    st.write(f"❌ Impossible de placer {b.nom}")
