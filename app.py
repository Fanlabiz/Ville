...
class Terrain:
    ...
    def find_move_for_unplaced(self, unplaced_building):
        """Cherche un déplacement qui permettrait de placer un bâtiment non placé"""
        best_move = None
        best_score = -1
        
        # Dimensions du bâtiment à placer
        height = unplaced_building.longueur
        width = unplaced_building.largeur
        
        # Essayer les deux orientations
        for orientation in ["horizontal", "vertical"]:
            h = unplaced_building.longueur if orientation == "horizontal" else unplaced_building.largeur
            w = unplaced_building.largeur if orientation == "horizontal" else unplaced_building.longueur
            
            # Parcourir toutes les positions potentielles
            for row in range(self.height - h + 1):
                for col in range(self.width - w + 1):
                    # Compter combien de cellules sont libres dans cette zone
                    free_cells = 0
                    blocked_by = set()
                    ...
                    for i in range(h):
                        for j in range(w):
                            ...
                            if self.is_cell_free(r, c):
                                ...
                                # Trouver quel bâtiment occupe cette cellule
                                for building in self.buildings:
                                    if (r, c) in building.get_all_cells_set():
                                        blocked_by.add(building)
                                        break
                    
                    # Si la zone est presque libre (bloquée par 1 seul bâtiment)
                    if len(blocked_by) == 1 and free_cells >= h * w - 10:
                        blocking_building = list(blocked_by)[0]
                        
                        # Chercher une nouvelle position pour le bâtiment bloquant
                        for new_orientation in ["horizontal", "vertical"]:
                            new_h = blocking_building.building.longueur if new_orientation == "horizontal" else blocking_building.building.largeur
                            new_w = blocking_building.building.largeur if new_orientation == "horizontal" else blocking_building.building.longueur
                            
                            for new_row in range(self.height - new_h + 1):
                                for new_col in range(self.width - new_w + 1):
                                    # Vérifier si le bâtiment peut être placé ici
                                    if self.can_place_building(blocking_building.building, new_row, new_col, new_orientation):
                                        # Vérifier que la nouvelle position ne bloque pas la zone cible
                                        new_cells = []
                                        for i in range(new_h):
                                            for j in range(new_w):
                                                new_cells.append((new_row + i, new_col + j))
                                        
                                        # Vérifier que la nouvelle position n'empiète pas sur la zone cible
                                        target_zone = set()
                                        for i in range(h):
                                            for j in range(w):
                                                target_zone.add((row + i, col + j))
                                        
                                        if not any(cell in target_zone for cell in new_cells):
                                            # Calculer le score (nombre de cellules libres dans la zone après déplacement)
                                            score = free_cells + 1
                                            if score > best_score:
                                                best_score = score
                                                best_move = {
                                                    'unplaced': unplaced_building,
                                                    'unplaced_pos': (row, col, orientation),
                                                    'blocking_building': blocking_building,
                                                    'blocking_new_pos': (new_row, new_col, new_orientation),
...
class BuildingPlacer:
    ...
    def try_place_with_move(self):
        """Tente de placer un bâtiment non placé en déplaçant un autre bâtiment"""
        if not self.unplaced:
            return False
        
        placed_any = False
        
        # Trier les bâtiments non placés par priorité de production
        production_priority = {"Guerison": 0, "Nourriture": 1, "Or": 2, "Autre": 3}
        self.unplaced.sort(key=lambda x: production_priority.get(x.production, 3))
        
        for unplaced in self.unplaced[:]:
            # Chercher un déplacement qui permettrait de placer ce bâtiment
            move = self.terrain.find_move_for_unplaced(unplaced)
            
            if move:
                # Appliquer le déplacement
                blocking = move['blocking_building']
                old_row, old_col = blocking.row, blocking.col
                old_orientation = blocking.orientation
                new_row, new_col, new_orientation = move['blocking_new_pos']
                
                # Déplacer le bâtiment bloquant
                self.terrain.remove_building(blocking)
                
                if self.terrain.can_place_building(blocking.building, new_row, new_col, new_orientation):
                    self.terrain.place_building(blocking.building, new_row, new_col, new_orientation)
                    
                    # Placer le bâtiment non placé
                    row, col, orientation = move['unplaced_pos']
                    self.terrain.place_building(unplaced, row, col, orientation)
                    self.unplaced.remove(unplaced)
                    placed_any = True
                    break
                else:
                    # Revenir à l'ancienne position
                    self.terrain.place_building(blocking.building, old_row, old_col, old_orientation)
    ...
    def place_all_buildings_complete(self):
        ...
        # Phase 2: Tentative de placement par déplacement ciblé
        st.info("🔍 Recherche de déplacements pour placer les bâtiments restants...")
        ...
        for iteration in range(max_iterations):
            ...
            if self.try_place_with_move():
                st.info(f"✅ Placement réussi par déplacement (itération {iteration + 1})")
            else:
                break
        
        # Phase 3: Placement standard des bâtiments restants
        remaining = []
...
def main():
    ...
    st.title("🏗️ Placeur de Bâtiments - Version Ciblée")
    st.markdown("Chargez un fichier Excel pour placer les bâtiments avec déplacement ciblé")
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("🚀 Lancer le placement ciblé", type="primary"):
                with st.spinner("Placement avec recherche de déplacements..."):
                    ...
                    st.download_button(
                        ...
                        data=output_file,
                        file_name="resultats_placement_cible.xlsx",
