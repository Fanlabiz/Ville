...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    def ffd_pure(nn_ordered, n_ordered):
        ...
        # Stratégie pour les boost100_required :
        #   2a. Placer TOUS les max ensemble (groupés) avant les autres
        #   2b. Placer les autres top_prods au centre
        #   2c-max. Distribuer les culturels pour couvrir simultanément tous les max
        #   2c-autres. Culturels pour les autres top_prods (logique d'origine)
        ...
        # ── Phase 2a : placer tous les max groupés ──
        _placed_max_prods = []
        ...
        # ── Phase 2c-max : culturels distribués pour couvrir tous les max simultanément ──
        if _placed_max_prods:
            _cults_grands_max = sorted(
                [c for c in cults_pool if id(c) not in cults_placed_ids and c["rows"]*c["cols"] > 2],
                key=lambda c: -(c["culture"] * max(c.get("rayonnement", 1), 1))
            )
            _cults_tous_max = sorted(
                [c for c in cults_pool if id(c) not in cults_placed_ids],
                key=lambda c: -(c["culture"] * max(c.get("rayonnement", 1), 1))
            )
            _total_max_seuil = sum(p.get("boost100", 9999) for p in _placed_max_prods)
            _cands_max = (_cults_grands_max
                          if sum(c["culture"] for c in _cults_grands_max) >= _total_max_seuil
                          else _cults_tous_max)
            ...
            while True:
                ...
                if not _max_not_done:
                    break
                _avail = [c for c in _cands_max if id(c) not in cults_placed_ids]
                if not _avail:
                    break
                occ = build_occ(result)
        ...
        # ── Phase 2c-autres : culturels pour les autres top_prods ──
        for placed_prod_ref in [b for b in placed_top_prods
            ...
            cults_disponibles = (cults_grands if _cult_deja + _cult_max_grands >= prod_boost100
                                 else cults_tous)
