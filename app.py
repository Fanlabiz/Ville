...
class Terrain:
    ...
    def get_radiation_zone_size(self, cultural, x, y, orientation):
        """Calcule le nombre de cases de rayonnement qui restent dans le terrain"""
        l, L = cultural.get_dimensions(orientation)
        rayon = cultural.rayonnement
        ...
        for dx in range(max(0, x - rayon), min(self.width, x + l + rayon)):
            for dy in range(max(0, y - rayon), min(self.height, y + L + rayon)):
                if self.is_in_radiation_zone(temp, dx, dy):
                    count += 1
    ...
    def find_all_contiguous_areas(self, temp_occupied=None):
        """Trouve toutes les zones contiguës de cases libres"""
    ...
    def can_place_all_remaining(self, remaining_buildings, temp_occupied):
        """Vérifie si tous les bâtiments restants peuvent être placés"""
        ...
        sorted_buildings = sorted(remaining_buildings, key=lambda b: -b.get_area())
        
        # Stratégie gloutonne
        used_areas = [False] * len(areas)
...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Toutes contraintes")
    ...
    if st.button("🚀 Lancer", type="primary"):
        with st.spinner("Placement..."):
            ...
            total_area = sum(b.get_area() * b.quantite for b in buildings)
            free_cases = np.sum(terrain.grid == 1)
            st.write(f"📊 Surface totale nécessaire: {total_area} cases")
            st.write(f"📊 Cases libres disponibles: {free_cases} cases")
            
            if total_area > free_cases:
                st.warning(f"⚠️ Surface insuffisante! Manque {total_area - free_cases} cases")
            
            # ÉTAPE 1: Placer les culturels
            st.write(f"🏛️ Placement de {len(cult)} culturels")
            
            # Pour les culturels, on maximise la zone de rayonnement dans le terrain
            for i, b in enumerate(cult):
                best_score = -1
                best_pos = None
                ...
                for x, y, o in positions:
                    # Score = taille de la zone de rayonnement qui reste dans le terrain
                    rayon_score = terrain.get_radiation_zone_size(b, x, y, o)
                    
                    # Bonus si on est près du centre
                    centre_x, centre_y = terrain.width // 2, terrain.height // 2
                    l, L = b.get_dimensions(o)
                    dist = abs(x + l//2 - centre_x) + abs(y + L//2 - centre_y)
                    
                    score = rayon_score * 1000 - dist * 10
                ...
                if best_pos:
                    ...
                    st.write(f"✅ {b.nom} à ({best_pos[0]},{best_pos[1]}) - zone rayonnement: {rayon} cases")
                else:
                    st.write(f"❌ Impossible de placer {b.nom}")
            
            # ÉTAPE 2: Trier les producteurs par taille décroissante
            prod.sort(key=lambda b: -b.get_area())
            
            # ÉTAPE 3: Placer chaque producteur
            for i, b in enumerate(prod):
                ...
                for x, y, o in positions:
                    ...
                    temp_occupied = terrain.occupied.copy()
                    for ii in range(l):
                        for jj in range(L):
                            if 0 <= y+jj < terrain.height and 0 <= x+ii < terrain.width:
                                temp_occupied[y+jj, x+ii] = True
                    
                    # Vérifier si on peut encore placer tous les bâtiments restants
                    can_place_all = terrain.can_place_all_remaining(remaining, temp_occupied)
                    
                    # Calculer la culture
                    culture = terrain.get_culture_for_position(x, y, l, L)
                    
                    # Score = priorité absolue à la préservation d'espace
                    if can_place_all:
                        score = 1000000 + culture
                    else:
                        score = -1000000
            ...
            with col1:
                st.metric("Bâtiments placés", f"{sum(b.placed for b in buildings)}/{sum(b.quantite for b in buildings)}")
            with col2:
