...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    if not real_moves:
        ...
        # Simuler l'etat du terrain pas a pas pour generer une sequence reellement executable.
        #
        # current_state[nom][i] = [r, c, rows, cols] de l'etat courant de chaque batiment.
        # On commence avec les positions initiales ET les dimensions initiales.
        # Quand un batiment est deplace, on met a jour r,c ET rows,cols vers
        # les dimensions finales (l'orientation peut changer apres optimisation).
        #
        # final_state[nom][i] = [r, c, rows, cols] de l'etat final vise.
        # L'appariement initial<->final se fait par ordre de declaration.
        ...
        # en preservant les dimensions initiales (orientation d'origine)
        current_state = {}
        for b in original_placed:
            current_state.setdefault(b["nom"], []).append(
                [b["r"], b["c"], b["rows"], b["cols"]])

        def current_grid(state, exclude_nom=None, exclude_idx=None):
            """Retourne dict (r,c)->nom pour l'etat courant."""
            g = {}
            for nom, positions in state.items():
                for i, (r, c, rows, cols) in enumerate(positions):
                    if nom == exclude_nom and i == exclude_idx:
                        continue
        ...
        for mv in real_moves:
            nom = mv["nom"]
            new_r, new_c = mv["new_r"], mv["new_c"]

            # Trouver l'index de cette instance dans current_state
            # (celle qui est a old_r, old_c)
            mv_idx = None
            for i, (r, c, rows, cols) in enumerate(current_state.get(nom, [])):
                if r == mv["old_r"] and c == mv["old_c"]:
                    mv_idx = i
                    break
            if mv_idx is None:
                # Le batiment n'est plus a sa position initiale (deja deplace)
                # Chercher sa position courante la plus proche de old_r, old_c
                best_i, best_d = 0, float("inf")
                for i, (r, c, rows, cols) in enumerate(current_state.get(nom, [])):
                    d = abs(r - mv["old_r"]) + abs(c - mv["old_c"])
                    if d < best_d:
                        best_d, best_i = d, i
                mv_idx = best_i

            pos = current_state[nom][mv_idx]
            cur_r, cur_c, rows, cols = pos[0], pos[1], pos[2], pos[3]
            cur_str = f"L{cur_r+1} C{cur_c+1}"
            new_str = f"L{new_r+1} C{new_c+1}"

            # Utiliser les dimensions FINALES du batiment pour calculer la zone de destination
            # (l'orientation peut avoir change lors de l'optimisation)
            fin_dims = final_state.get(nom, [])
            fin_rows = fin_dims[mv_idx][2] if mv_idx < len(fin_dims) else rows
            fin_cols = fin_dims[mv_idx][3] if mv_idx < len(fin_dims) else cols

            # Trouver les batiments qui occupent la destination dans l'etat COURANT
            grid_now = current_grid(current_state, exclude_nom=nom, exclude_idx=mv_idx)
            dest_cells = cells_of_pos(new_r, new_c, fin_rows, fin_cols)
            blockers_now = {}  # nom -> idx
            for cell in dest_cells:
                if cell in grid_now:
                    bnom = grid_now[cell]
                    # Trouver l'index
                    for i, (br, bc, brows, bcols) in enumerate(current_state.get(bnom, [])):
                        if cell in cells_of_pos(br, bc, brows, bcols):
                            key = (bnom, i)
                            if key not in blockers_now:
                                blockers_now[key] = (br, bc, brows, bcols)
                            break

            if blockers_now:
                # Construire la liste des bloqueurs avec leur position courante et finale
                blocker_lines = []
                for (bnom, bidx), (br, bc, brows, bcols) in blockers_now.items():
                    # Position finale de ce bloqueur
                    final_positions = final_state.get(bnom, [])
                    if bidx < len(final_positions):
                        fr, fc = final_positions[bidx][0], final_positions[bidx][1]
                        final_str = f"L{fr+1} C{fc+1}"
                    else:
                        final_str = "position finale inconnue"
                    blocker_lines.append(f"{bnom} (actuellement en L{br+1} C{bc+1}, aller en {final_str})")

                action = (
                    f"1) Deplacer d'abord vers leur position finale : {'; '.join(blocker_lines)}. "
                    f"2) Deplacer '{nom}' de {cur_str} vers {new_str}."
                )
            else:
                action = f"Deplacer '{nom}' de {cur_str} vers {new_str}."

            # Mettre a jour l'etat courant: position ET dimensions finales
            current_state[nom][mv_idx][0] = new_r
            current_state[nom][mv_idx][1] = new_c
            current_state[nom][mv_idx][2] = fin_rows
            current_state[nom][mv_idx][3] = fin_cols

            old_str = f"L{mv['old_r']+1} C{mv['old_c']+1}"
            ri = step + 1
            ws3.cell(ri, 1, step)
            ws3.cell(ri, 2, nom)
            ws3.cell(ri, 3, old_str)   # position initiale (fichier input)
            ws3.cell(ri, 4, new_str)   # position finale
