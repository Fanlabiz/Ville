...
try:
    ...
    # PHASE 1 – Placement initial (inchangé)
    ...
    # 2.1 – Optimisation des casernes Guérison (priorité 1)
    ...
    for building in guerison_prods:
        ...
        for try_rot in [False, True]:
            ...
            cands = find_positions(grid_work, hh, ww, False)[:800]  # beaucoup plus de candidats
            for nr, nc, _ in cands:
                ...
                if score > best_score + 5:  # gain minimal de 5 culture totale
                    best_score = score
    ...
    # 2.2 – Optimisation des culturels puissants (priorité 2)
    st.write("   → 2.2 Culturels puissants (≥1000 culture ou rayon ≥2)")
    ...
    for building in strong_cult:
        ...
        for try_rot in [False, True]:
            ...
            for nr, nc, _ in cands:
                ...
                temp_map = compute_culture_map(grid_work, placed)
                score = guerison_culture_score(placed, temp_map)
                if score > best_score + 10:  # gain minimal plus élevé pour gros bâtiments
                    best_score = score
                    best_r, best_c, best_rot = nr, nc, try_rot
                place(grid_work, nr, nc, hh, ww, 1, try_rot)
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        # (le bloc d'export est identique à la version précédente – je ne le répète pas ici pour brevité)
        # ... copier-coller la partie export depuis ton script v3.1 ...
        # Assure-toi juste d'avoir le même style (fusion cellules, couleurs orange/vert/gris)
