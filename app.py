...
st.set_page_config(page_title="Optimiseur v3.1 - Guérison First", layout="wide")
st.title("Optimiseur v3.1 – Priorité absolue Guérison")
...
try:
    ...
    # ====================== PHASE 2 ======================
    st.write("Phase 2 – Optimisation Guérison-First...")

    to_optimize = [b for b in placed if b.get('placed') and b['Type'] != 'Neutre']

    # 2.1 Producteurs Guérison
    st.write("   → 2.1 Producteurs Guérison (agressif)")
    ...
    # 2.2 Culturels
    st.write("   → 2.2 Culturels (pour couvrir les Guérison)")
    for building in [b for b in to_optimize if b['Type'] == 'Culturel']:
        ...
        for try_rot in [False, True]:
            hh = building['Largeur'] if try_rot else building['Longueur']
            ww = building['Longueur'] if try_rot else building['Largeur']
            candidates = find_positions(grid_work, hh, ww, False)[:400]  # moins pour les gros
            for nr, nc, _ in candidates:
                if not can_place(grid_work, nr, nc, hh, ww, False): continue
                place(grid_work, nr, nc, hh, ww, -5, try_rot)
                temp_map = compute_culture_map(grid_work, placed)
    ...
    # Stats + export (code identique aux versions précédentes)
    ...
    st.download_button("Télécharger résultat.xlsx", output, "Ville_optimisee_v3_1.xlsx",
