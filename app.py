...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        Version corrigée pour détecter correctement tous les bâtiments
        """
        ...
        for r in range(min(len(actuel_data), self.rows)):
            for c in range(min(len(actuel_data[0]), self.cols)):
                ...
                # Chercher le plus grand rectangle de même nom à partir de (r,c)
                
                # Largeur maximale
                max_w = 1
                for dc in range(1, self.cols - c):
                    if c + dc < self.cols:
                        next_val = str(actuel_data[r][c + dc]).strip()
                        if next_val == cell_value and not visited[r, c + dc]:
                            max_w = dc + 1
                        else:
                            break
                ...
                for dr in range(1, self.rows - r):
                    if r + dr < self.rows:
                        next_val = str(actuel_data[r + dr][c]).strip()
                        if next_val == cell_value and not visited[r + dr, c]:
                            max_h = dr + 1
                        else:
                            break
                ...
                for dr in range(h):
                    for dc in range(w):
                        if r + dr >= self.rows or c + dc >= self.cols:
                            valid = False
                            break
                        check_val = str(actuel_data[r + dr][c + dc]).strip()
                        if check_val != cell_value or visited[r + dr, c + dc]:
                            valid = False
                            break
                    if not valid:
                        break
                ...
                if not valid:
                    # Réduire progressivement si le rectangle maximal n'est pas valide
                    found = False
    ...
    def match_buildings_with_info(self, buildings_info):
        ...
        for temp in temp_buildings:
            ...
            if nom in building_pool and building_pool[nom]:
                ...
                self.log(f"Bâtiment associé: {nom} à ({r},{c}) dimensions {w}x{h}")
            else:
...
- **Détection corrigée** des bâtiments existants (ne fusionne plus les bâtiments adjacents)
