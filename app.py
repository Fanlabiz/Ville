...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    def ffd_pure(nn_ordered, n_ordered):
        ...
        cults_pool = [b for b in nn_ordered if b["type"] == "Culturel"]
        ...
        for prod in top_prods_all:

            # 2a. Placer le producteur au barycentre mobile
            occ = build_occ(result)
            if placed_top_prods:
                _tr = sum(p["r"]+p["rows"]/2 for p in placed_top_prods)/len(placed_top_prods)
                _tc = sum(p["c"]+p["cols"]/2 for p in placed_top_prods)/len(placed_top_prods)
            else:
            ...
            # 2b. Placer immédiatement les culturels pour amener ce producteur à 100%
            # Trier les culturels disponibles par valeur décroissante
            # Utiliser seulement les culturels de surface >= 4 en Phase 2.
            # Les culturels 1x1 et 1x2 (Sites réduits/compacts) créent de la
            # fragmentation et empêchent les Petites maisons 2x2 de se placer.
            # Ils seront traités en Phase 3 où ils remplissent les interstices.
            cults_disponibles = sorted(
                [c for c in cults_pool
                 if id(c) not in cults_placed_ids
                 and c["rows"] * c["cols"] >= 4],
                key=lambda c: -c["culture"]
            )
            for cult in cults_disponibles:
                # Recalculer la culture reçue par ce producteur
                cults_actuels = [b for b in result if b["type"] == "Culturel"]
                cult_recue = culture_received(placed_prod_ref, cults_actuels)
                if cult_recue >= placed_prod_ref.get("boost100", 9999):
                    break  # déjà à 100%, pas besoin de plus

                occ = build_occ(result)
                ray = cult.get("rayonnement", 1)
                # Chercher une position valide qui couvre ce producteur
                best_pos = None; first_fit = None
                for rows, cols in [(cult["rows"], cult["cols"]),
                    ...
                    for r in range(max_r):
                        if best_pos: break
                        for c in range(max_c):
                            if not can_place(r, c, rows, cols, x_grid, occ,
                                             max_r, max_c):
                                continue
                            if first_fit is None:
                                first_fit = (r, c, rows, cols)
                            ...
                            if cells_of(placed_prod_ref) & zone:
                                best_pos = (r, c, rows, cols); break
        ...
        # Phase 3 : Petits producteurs (2x2) en tête, puis le reste par taille desc.
        # Les Petites maisons 2x2 doivent trouver des blocs 2x2 AVANT que les
        # grands bâtiments les fragmentent. Il y a ~680 positions 2x2 libres
        # après Phase 2, largement assez pour les 26 Petites maisons.
        _tout_reste = ([c for c in cults_pool if id(c) not in cults_placed_ids]
                       + autres)
        # Phase 3 : Best Fit Decreasing — grands bâtiments en premier.
        # Les grands occupent les grands espaces. Les petits (2x2, 2x3) se
        # logent dans les interstices restants.
        # À taille égale : Culturels avant Producteurs.
        reste = sorted(
            _tout_reste,
            key=lambda b: (-(b["rows"] * b["cols"]),
