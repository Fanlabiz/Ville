...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    # non_neutres_ffd : ordre optimisé pour grouper casernes + culturels
    # 1. Casernes (haute priorité) en tête par taille desc -> se groupent en haut-gauche
    # 2. Culturels forts (grande culture) juste après -> couvrent les casernes
    # 3. Reste par taille desc
    def _nn_sort_key(b):
        if b["type"] == "Producteur" and b["priorite"] >= prio_threshold:
            return (0, -(b["rows"] * b["cols"]))
        elif b["type"] == "Culturel":
            return (1, -(b.get("culture", 0) * b.get("rayonnement", 1)))
        else:
            return (2, -(b["rows"] * b["cols"]))
    ...
    def ffd_pure(nn_ordered, n_ordered):
        """
        FFD avec biais de couverture prioritaire pour les Culturels :
        - Producteurs : first-fit raster.
        - Culturels   : cherche d'abord une position couvrant un producteur
                        HAUTE PRIORITE (casernes prio>=threshold). Si impossible,
                        se rabat sur n'importe quel producteur place. Fallback first-fit.
        - Neutres     : scan depuis les bords (regle 1).
        Garantit 0 echecs si le terrain peut accueillir tous les batiments.
        """
        result = [dict(b) for b in placed]
        n_ok = n_fail = 0
        placed_prods = []       # tous les producteurs places
        placed_top_prods = []   # producteurs haute priorite places

        for b in nn_ordered:
            occ = build_occ(result)
            first_fit = None
            chosen_top = None   # position couvrant un prod haute prio
            chosen_any = None   # position couvrant n'importe quel prod
            ...
            # Priorité : couvrir caserne > couvrir n'importe quel prod > first-fit
            ...
            if chosen:
                ...
                if b["type"] == "Producteur":
                    ...
                    if b["priorite"] >= prio_threshold:
                        placed_top_prods.append(result[-1])
                n_ok += 1
            else:
                n_fail += 1
    ...
    while swap_improved and n_swap_passes < max_swap_passes:
        ...
        for i, c1 in enumerate(culturels_list):
            for c2 in culturels_list[i+1:]:
                ...
                # Tester le swap (et les variantes avec pivot)
                ...
                for new1, new2 in [
                    (old2, old1),
                    ((old2[0], old2[1], old2[3], old2[2]), old1),
                    (old2, (old1[0], old1[1], old1[3], old1[2])),
                ]:
                    # Verifier que les nouvelles tailles tiennent dans les cases
                    # (les cases sont les memes, juste les dimensions changent)
                    r1, c1r, rows1, cols1 = new1
                    r2, c2r, rows2, cols2 = new2
                    if (r1 + rows1 > max_r or c1r + cols1 > max_c or
                            r2 + rows2 > max_r or c2r + cols2 > max_c):
                        continue
