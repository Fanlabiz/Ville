...
class Terrain:
    def __init__(self, grid):
        ...
        self.coverage_count = np.zeros((self.height, self.width), dtype=int)  # Nombre de culturels couvrant chaque case
    ...
    def update_coverage(self, building, x, y, orientation, add=True):
        ...
        for dx in range(max(0, cx - building.rayonnement), min(self.width, cx + building.rayonnement + 1)):
            for dy in range(max(0, cy - building.rayonnement), min(self.height, cy + building.rayonnement + 1)):
                distance = max(abs(dx - cx), abs(dy - cy))
                if distance <= building.rayonnement:
                    self.coverage_count[dy, dx] += delta
    ...
    def place_building(self, building, x, y, orientation):
        ...
        if building.type == "culturel" and building.culture > 0:
            ...
            self.update_coverage(building, x, y, orientation, add=True)
    
    def get_culture_for_position(self, x, y, longueur, largeur):
        if not self.cultural_buildings:
            return 0
        
        # Utiliser la carte de couverture pour déterminer combien de culturels touchent ce bâtiment
        max_coverage = 0
        for i in range(longueur):
            for j in range(largeur):
                px, py = x + i, y + j
                if 0 <= px < self.width and 0 <= py < self.height:
                    max_coverage = max(max_coverage, self.coverage_count[py, px])
        
        # Estimer la culture (approximation - on prend les plus gros culturels)
        if max_coverage > 0:
            cultures = sorted([cb['culture'] for cb in self.cultural_buildings], reverse=True)
            return sum(cultures[:max_coverage])
        return 0
    ...
    def get_overlap_score(self, building, x, y, orientation):
        """Calcule un score basé sur les chevauchements potentiels avec d'autres culturels"""
        ...
        for dx in range(max(0, cx - building.rayonnement), min(self.width, cx + building.rayonnement + 1)):
            for dy in range(max(0, cy - building.rayonnement), min(self.height, cy + building.rayonnement + 1)):
                ...
                if distance <= building.rayonnement:
                    ...
                    if self.coverage_count[dy, dx] > 0:
                        score += 100 * (self.coverage_count[dy, dx] + 1)
                    # Bonus pour les cases libres
                    if self.grid[dy, dx] == 1 and not self.occupied[dy, dx]:
                        score += 10
        return score
...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Optimisation des chevauchements")
    ...
    if st.button("🚀 Lancer l'optimisation", type="primary"):
        with st.spinner("Placement en cours..."):
            ...
            st.subheader("🏛️ Placement des culturels (optimisation des chevauchements)")
            
            # Placer les culturels en maximisant les chevauchements
            ...
            for i, b in enumerate(culturels):
                ...
                for x, y, o in positions:
                    score = terrain_final.get_overlap_score(b, x, y, o)
                ...
                if meilleure_pos:
                    ...
                    st.write(f"✅ {b.nom} à ({x},{y}) - score chevauchement: {meilleur_score}")
                else:
                    st.write(f"❌ Impossible de placer {b.nom}")
                
                progress.progress((i + 1) / len(tous))
            
            # Afficher la carte de chevauchement
            st.subheader("🗺️ Carte des chevauchements culturels")
            overlap_df = pd.DataFrame(terrain_final.coverage_count)
            ...
            for i, b in enumerate(producteurs):
                ...
                for x, y, o in positions:
                    ...
                    # Vérifier le nombre de culturels qui couvrent cette position
                    max_coverage = 0
                    for i2 in range(l):
                        for j2 in range(L):
                            px, py = x + i2, y + j2
                            if 0 <= px < terrain_final.width and 0 <= py < terrain_final.height:
                                max_coverage = max(max_coverage, terrain_final.coverage_count[py, px])
                    
                    score = culture
                    if max_coverage >= 3:
                        score += 10000  # Bonus énorme pour une couverture des 3 culturels
                    elif max_coverage >= 2:
                        score += 5000   # Bonus pour 2 culturels
                ...
                if meilleure_pos:
                    ...
                    for i2 in range(l):
                        for j2 in range(L):
                            px, py = x + i2, y + j2
                            if 0 <= px < terrain_final.width and 0 <= py < terrain_final.height:
                                max_cov = max(max_cov, terrain_final.coverage_count[py, px])
                    ...
                    st.write(f"✅ {b.nom} à ({x},{y}) - couverture: {max_cov} culturels, culture: {culture:.0f} {boost_text}")
                else:
                    st.write(f"❌ Impossible de placer {b.nom}")
