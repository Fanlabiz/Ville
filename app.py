...
def parse_terrain(df_terrain):
    """Parse terrain sheet -> grid of cell values, boundary mask"""
    grid = df_terrain.values.tolist()
    rows = len(grid)
    ...
    for r in grid:
        while len(r) < cols:
            r.append(None)
    # Build playable mask (inside X borders)
    playable = [[False]*cols for _ in range(rows)]
    for r in range(rows):
        for c in range(cols):
            val = grid[r][c]
            is_x = isinstance(val, str) and val.strip().upper() == 'X'
            if not is_x and val is not None and not (isinstance(val, float) and np.isnan(val)):
                playable[r][c] = True
            elif not is_x and (val is None or (isinstance(val, float) and np.isnan(val))):
                # Could still be inside terrain - check later
                playable[r][c] = None  # unknown
    # Flood-fill from outside to mark truly outside cells
        ...
        for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
            ...
            if 0 <= nr < rows and 0 <= nc < cols and not outside[nr][nc]:
                ...
                if is_x or is_empty:
                    outside[nr][nc] = True
                    q.append((nr, nc))
    # Playable = inside terrain (not X, not outside)
    for r in range(rows):
        for c in range(cols):
            val = grid[r][c]
            is_x = isinstance(val, str) and val.strip().upper() == 'X'
            if is_x or outside[r][c]:
                playable[r][c] = False
            else:
                playable[r][c] = True
...
def parse_buildings_from_terrain(grid, playable, rows, cols, bldg_info):
    """Extract placed buildings from terrain grid"""
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if val is None or (isinstance(val, float) and np.isnan(val)):
                continue
            name = str(val).strip()
            # Find all cells with same name connected
            cells = []
            from collections import deque
            q = deque([(r, c)])
            visited[r][c] = True
            ...
            placed.append({
                ...
                'h': h,
                'w': w,
                'type': info.get('type', 'Neutre'),
                'culture': info.get('culture', 0),
                'rayonnement': info.get('rayonnement', 0),
                'boost25': info.get('boost25', None),
                'boost50': info.get('boost50', None),
                'boost100': info.get('boost100', None),
                'production': info.get('production', 'Rien'),
...
def try_swap_or_reloc(placed, playable, rows, cols):
    """One pass of swap/relocation attempts to improve score"""
    best_score = score_placement(placed)
    improved = False
    occ = build_occupancy(placed, rows, cols)

    for i, b in enumerate(placed):
        if b['type'] == 'Neutre':
            continue
        orig_r, orig_c = b['row'], b['col']
        # Try each orientation
        for (h, w) in [(b['h'], b['w']), (b['w'], b['h'])]:
            for r in range(rows - h + 1):
                for c in range(cols - w + 1):
                    if r == orig_r and c == orig_c and h == b['h'] and w == b['w']:
                        continue
                    # Temporarily remove b from occ
                    for dr in range(b['h']):
                        for dc in range(b['w']):
                            occ[orig_r+dr][orig_c+dc] = None
                    if can_place(b, r, c, h, w, playable, occ, rows, cols):
                        # Test move
                        old_h, old_w = b['h'], b['w']
                        b['row'], b['col'], b['h'], b['w'] = r, c, h, w
                        new_score = score_placement(placed)
                        if new_score > best_score:
                            ...
                            b['row'], b['col'], b['h'], b['w'] = orig_r, orig_c, old_h, old_w
                    # Restore occ
                    for dr in range(b['h']):
                        for dc in range(b['w']):
                            occ[orig_r+dr][orig_c+dc] = b
                if improved:
                    break
            if improved:
                break
...
def optimize(placed, playable, rows, cols, max_iter=30, progress_cb=None):
    """Iterative swap/relocation optimization"""
    best = copy.deepcopy(placed)
    best_score = score_placement(best)
    for i in range(max_iter):
        if progress_cb:
            progress_cb(i / max_iter)
        improved, new_score = try_swap_or_reloc(best, playable, rows, cols)
        if not improved:
            break
        best_score = new_score
    if progress_cb:
...
def write_sheet_liste(ws, placed, culture_map, original_placed):
    ...
    headers = ['Nom', 'Type', 'Production', 'Ligne', 'Colonne', 'Hauteur', 'Largeur',
               'Culture reçue', 'Boost atteint (%)', 'Qté/h brute', 'Qté/h avec boost']
    ...
    for ri, b in enumerate(placed, 2):
        ...
        boost = get_boost(b, cr)
        qty_brut = b['quantite']
        qty_boost = qty_brut * (1 + boost/100) if b['type'] == 'Producteur' else 0
        row_data = [b['name'], b['type'], b['production'],
                    b['row']+1, b['col']+1, b['h'], b['w'],
                    cr, boost, qty_brut, round(qty_boost)]
        fill = make_fill(col_color(b['type']))
...
def write_sheet_synthese(ws, placed, culture_map, original_culture_map, original_placed):
    ...
    headers = ['Type de production', 'Culture totale reçue',
               ...
               'Qté/h brute', 'Qté/h optimisée', 'Qté/h avant optim', 'Gain/Perte']
    for ci, h in enumerate(headers, 1):
        cell = ws.cell(row=1, column=ci, value=h)
        cell.font = Font(bold=True, color=WHITE)
        cell.fill = make_fill("444444")
    ...
    for ri, prod in enumerate(sorted_prods, 2):
        ...
        row_data = [prod, p['culture_total'],
                    ...
                    round(p['qty_brut']), round(p['qty_boost']), round(orig_qty), round(gain)]
        fill_color = LIGHT_GREEN if gain >= 0 else "FFAAAA"
        ...
        for ci, val in enumerate(row_data, 1):
            ...
            cell.border = make_border()
            if ci == len(headers):
                cell.font = Font(bold=True, color="006600" if gain >= 0 else "880000")
    for ci in range(1, len(headers)+1):
...
def write_sheet_deplacements(ws, original_placed, optimized_placed):
    ...
    ws.cell(row=2, column=1, value="SÉQUENCE DE DÉPLACEMENT RECOMMANDÉE").font = Font(bold=True, color="AA0000")
...
def write_sheet_terrain(ws, placed, playable, rows, cols, culture_map, label="Terrain optimisé"):
    ...
    max_c = cols
    for r in range(rows):
        ws.row_dimensions[r + offset_r + 1].height = 30
        for c in range(max_c):
            cell = ws.cell(row=r + offset_r + 1, column=c + offset_c + 1)
            cell.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            cell.border = make_border()
            if not playable[r][c]:
                cell.fill = make_fill("222222")
                cell.value = 'X'
                cell.font = Font(color=WHITE, size=7)
            elif (r, c) in cell_to_bldg:
                b = cell_to_bldg[(r, c)]
                if b['type'] == 'Culturel':
                    fill_hex = ORANGE
                ...
                # Only write name + boost in top-left cell
                if b['row'] == r and b['col'] == c:
                    cr = culture_map.get(id(b), {}).get('culture', 0)
                    boost = get_boost(b, cr)
                    boost_str = f"+{boost}%" if (b['type']=='Producteur' and boost > 0) else ""
                    cell.value = f"{b['name']}\n{boost_str}" if boost_str else b['name']
                    cell.font = Font(bold=True, size=7, color=WHITE)
            else:
                cell.fill = make_fill(WHITE)

    for c in range(max_c + 1):
        ws.column_dimensions[get_column_letter(c + offset_c + 1)].width = 12
...
if uploaded:
    with st.spinner("Lecture du fichier..."):
        ...
        df_terrain = pd.read_excel(xl, sheet_name=xl.sheet_names[0], header=None)
        df_bat     = pd.read_excel(xl, sheet_name=xl.sheet_names[1], header=None)

        bldg_info = parse_bldg_info(df_bat)
        grid, playable, rows, cols = parse_terrain(df_terrain)
        original_placed = parse_buildings_from_terrain(grid, playable, rows, cols, bldg_info)
