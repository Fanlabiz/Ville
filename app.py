...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        Version qui gère correctement les bâtiments multiples
        """
        ...
        for r in range(len(actuel_str)):
            for c in range(len(actuel_str[0])):
                ...
                if cell_value in self.possible_dimensions:
                    ...
                    # Trier par surface décroissante pour prendre les plus grands d'abord
                    # (pour éviter de fragmenter inutilement)
                    possible_dims.sort(key=lambda x: x[0] * x[1], reverse=True)
                    
                    # Essayer de trouver un placement avec les dimensions connues
                    found = False
                    for (w, h) in possible_dims:
                        # Vérifier que le rectangle de dimensions (w, h) est valide
                    ...
                    if not found:
                        # Si aucune dimension connue ne correspond, on prend la plus petite dimension
                        # (pour les cas où plusieurs bâtiments sont côte à côte)
                        
                        # Prendre la plus petite dimension (en surface)
                        min_dims = min(possible_dims, key=lambda x: x[0] * x[1])
                        w, h = min_dims
                        
                        # Vérifier qu'on peut prendre cette dimension
                        if r + h <= self.rows and c + w <= self.cols:
                            valid = True
                            for dr in range(h):
                                for dc in range(w):
    ...
    def match_buildings_with_info(self, buildings_info):
        ...
        Version améliorée : vérifie d'abord les dimensions avant de consommer
        """
        ...
        for temp in temp_buildings:
            ...
            if nom in building_pool and building_pool[nom]:
                ...
                if found:
                    ...
                    info = building_pool[nom].pop(best_match_idx)
                    ...
                    # Aucune correspondance de dimensions, mais on garde le bâtiment dans le pool
                    unmatched.append(f"{nom} à ({r},{c}) dimensions {w}x{h} (pas de correspondance de dimensions)")
                    self.log(f"⚠️ Attention: bâtiment '{nom}' dimensions {w}x{h} ne correspond à aucune orientation")
            else:
                unmatched.append(f"{nom} à ({r},{c}) (plus disponible)")
...
- **Détection guidée par les dimensions** des bâtiments
- **Gère les orientations différentes** des bâtiments
- **Gère les bâtiments multiples** (détecte chaque bâtiment individuellement)
...
if uploaded:
    with st.spinner("Analyse du fichier en cours..."):
        try:
            ...
            # Initialiser les dimensions possibles
            planner.set_building_dimensions(all_buildings)
            
            # Recharger les bâtiments avec les dimensions connues
            if a_df is not None:
                planner.placed_buildings = []
                planner.grid = planner.base_grid.copy()
                planner.load_existing_buildings(a_df.values)
