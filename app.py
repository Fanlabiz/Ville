...
Version finale - Avec toutes les méthodes nécessaires
"""
...
class Terrain:
    ...
    def remove_building(self, instance):
        """Supprime un bâtiment spécifique du terrain"""
...
class AdvancedOptimizer:
    ...
    def optimize(self):
        ...
        # Nettoyer le terrain (enlever tous les bâtiments existants)
        ...
        # Sauvegarder les positions originales pour les déplacements
        original_positions = [(p.x, p.y, p.orientation) for p in current_producteurs]
        
        # Enlever tous les producteurs
        for producteur in current_producteurs[:]:  # Copie pour éviter les problèmes d'itération
            self.terrain.remove_building(producteur)
        
        # Replacer les producteurs de manière optimale
        new_producteurs = []
        for building in producteur_buildings:
            for _ in range(building.nombre):
                best_score = -1
        ...
        for i, (old_pos, new_inst) in enumerate(zip(original_positions, new_producteurs)):
            old_x, old_y, old_ori = old_pos
            if (old_x, old_y, old_ori) != (new_inst.x, new_inst.y, new_inst.orientation):
                self.moves.append({
                    'nom': new_inst.building.nom,
                    'avant_x': old_x,
                    'avant_y': old_y,
                    'avant_orientation': 'Horizontal' if old_ori == 'h' else 'Vertical',
                    'apres_x': new_inst.x,
                    'apres_y': new_inst.y,
                    'apres_orientation': 'Horizontal' if new_inst.orientation == 'h' else 'Vertical'
                })
...
class ExcelExporter:
    ...
    def _create_terrain_sheet(ws: Worksheet, terrain: Terrain):
        ...
        COLOR_BORDURE = "A9A9A9"  # Gris foncé pour les bordures
        ...
        # Créer un dictionnaire pour regrouper les bâtiments optimisés par leur première cellule
        ...
        # Deuxième passage: Appliquer les fusions et le contenu pour les bâtiments optimisés
...
def main():
    ...
    **Stratégie d'optimisation :**
    1. 📍 Nettoyage complet du terrain
    2. 🎯 Optimisation du placement des culturels (bonus pour les positions centrales)
    3. 🔄 Optimisation du placement des producteurs autour des culturels
