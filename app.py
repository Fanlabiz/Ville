...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    # Tri : par type fonctionnel (alphabétique FR) puis par nom affiché (alphabétique)
    _cat_order = {"Barracks": 0, "Farm": 1, "Home": 2, "Workshop": 3}
    _cat_fr    = {"Barracks": "Casernes", "Farm": "Fermes", "Home": "Maisons", "Workshop": "Ateliers"}

    def _sort_key_prod(nom):
        cat = _csv_building_category(nom)
        cat_rank = _cat_order.get(cat, 4)  # inconnu en dernier
        cat_label = _cat_fr.get(cat, "Autres") if cat else "Autres"
        display = _dn(nom).lower()
        return (cat_rank, cat_label, display)
