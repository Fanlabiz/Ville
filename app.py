...
class PlacementBatiments:
    ...
    def retirer_batiment(self, index):
        ...
        # Recalculer la carte de rayonnement (simplifié - on pourrait optimiser)
        self.carte_rayonnement = np.zeros((self.hauteur, self.largeur))
        for b in self.batiments_places:
            if b['type'] == 'culturel':
                self.calculer_zone_rayonnement(b['x'], b['y'], b['longueur'], b['largeur'],
                                              self.trouver_rayonnement_original(b['nom']),
                                              self.trouver_culture_originale(b['nom']))
        
        return batiment
    
    def trouver_rayonnement_original(self, nom):
        """Trouve le rayonnement original d'un bâtiment"""
        for b in self.batiments:
            if b['nom'] == nom:
                return b['rayonnement']
        return 0
    
    def trouver_culture_originale(self, nom):
        """Trouve la culture originale d'un bâtiment"""
        for b in self.batiments:
            if b['nom'] == nom:
                return b['culture']
        return 0
    ...
    def calculer_score_configuration(self):
        ...
        # Pénalité pour les bandes de largeur 1 (optionnel)
        for i in range(self.hauteur):
            for j in range(self.largeur):
                if self.est_case_libre(i, j):
                    # Vérifier les bandes horizontales
                    if j < self.largeur - 1 and self.est_case_libre(i, j+1):
                        largeur_bande = 1
                        while j + largeur_bande < self.largeur and self.est_case_libre(i, j + largeur_bande):
                            largeur_bande += 1
                        if largeur_bande == 2:  # Bande de largeur 2
                            pass  # Pas de pénalité
                        elif largeur_bande == 1:  # Bande de largeur 1
                            score -= 10
    ...
    def generer_voisinage(self, liste_tabou):
        ...
        ou en échangeant deux bâtiments
        """
        voisins = []
            ...
            for dx in range(-3, 4):
                for dy in range(-3, 4):
                    ...
                    if self.peut_placer_batiment(x_new, y_new, longueur, largeur):
                        ...
                        self.placer_batiment(batiment, x_new, y_new, longueur, largeur, culture)
        ...
        return voisins[:10]  # Garder les 10 meilleurs voisins
...
# Titre
st.title("🏗️ Optimiseur de Placement de Bâtiments (Recherche Tabou)")
st.markdown("---")
...
# Footer
st.markdown("---")
st.markdown("🚀 Application développée avec l'algorithme de recherche tabou")