...
class Batiment:
    ...
    def __post_init__(self):
        if self.type not in ['culturel', 'producteur', 'neutre']:
            if self.type == '' or pd.isna(self.type):
                self.type = 'neutre'
...
class PlacementBatiments:
    ...
    def _creer_batiments(self, data: List[Dict]) -> List[Batiment]:
        ...
        for row in data:
            try:
                ...
                if pd.isna(type_bat) or type_bat == '':
                    type_bat = 'neutre'
                ...
                batiment = Batiment(
                    ...
                    type=type_bat,
                    culture=float(row['Culture']) if not pd.isna(row.get('Culture')) else 0,
                    rayonnement=int(row['Rayonnement']) if not pd.isna(row.get('Rayonnement')) else 0,
                    boost_25=float(row['Boost 25%']) if not pd.isna(row.get('Boost 25%')) else 0,
                    boost_50=float(row['Boost 50%']) if not pd.isna(row.get('Boost 50%')) else 0,
                    boost_100=float(row['Boost 100%']) if not pd.isna(row.get('Boost 100%')) else 0,
    ...
    def _verifier_espace_suffisant(self) -> bool:
        ...
        cases_libres = np.sum(self.terrain.matrice * (self.terrain.occupation == 0))
    ...
    def placer_batiment(self, batiment: Batiment) -> bool:
        ...
        for score, x, y, orientation in emplacements_avec_score:
            ...
            if self._verifier_espace_suffisant():
                ...
                self.logger.ajouter(f"✗ Placement de {batiment.nom} en ({x}, {y}) annulé - pas assez d'espace pour les bâtiments restants")
    ...
    def executer_placement(self):
        ...
        while (culturels or producteurs) and iteration < max_iterations and not self.logger.arret:
            ...
            # Si aucun placement n'a été fait, on a un problème
            if not culturels and not producteurs:
                break
    ...
    def calculer_statistiques(self) -> Dict:
        ...
        stats = {
            ...
            'culture_par_type': {},
            'cases_non_utilisees': int(np.sum(self.terrain.matrice * (self.terrain.occupation == 0))),
        ...
        try:
            ...
            if bouton_lancer:
                with st.spinner("Placement en cours... (cela peut prendre quelques instants)"):
                    ...
                    mask_libre = (placement.terrain.matrice == 1) & (placement.terrain.occupation == 0)
                    color_map[mask_libre] = [1, 1, 1]
                    
                    # Cases occupées (noir)
                    color_map[placement.terrain.matrice == 0] = [0, 0, 0]
        ...
        with st.expander("📝 Structure du fichier attendue", expanded=True):
            ...
            **Onglet 2: Bâtiments** (avec en-têtes)
            ...
            with col1:
                ...
                st.dataframe(example_terrain, use_container_width=True)
            
            with col2:
                st.markdown("**Bâtiments:**")
                example_batiments = pd.DataFrame([
                    ['Maison', 2, 2, 2, 'culturel', 10, 1, 5, 10, 20, ''],
                    ['Ferme', 3, 2, 1, 'producteur', 0, 0, 10, 20, 30, 'nourriture'],
                    ['Route', 1, 3, 3, 'neutre', 0, 0, 0, 0, 0, '']
                ], columns=['Nom', 'Longueur', 'Largeur', 'Quantite', 'Type', 'Culture', 
                           'Rayonnement', 'Boost 25%', 'Boost 50%', 'Boost 100%', 'Production'])
                st.dataframe(example_batiments, use_container_width=True)
