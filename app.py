...
def optimiser_placement(grid_base, batiments_list, terrain, max_time=60, progress_cb=None):
    ...
    # ── PASSE 5C : relocation des culturels orphelins (best-improvement) ─────────
    # Un culturel orphelin = aucun producteur dans sa zone de rayonnement.
    # Contrairement à la passe 5 (first-improvement), cette passe évalue TOUS les
    # orphelins puis exécute le mouvement global le plus bénéfique (best-improvement).
    # Cela évite que de petits gains locaux bloquent de grands gains globaux.
    if progress_cb:
        progress_cb(0.97, "Passe 5C : relocation des culturels orphelins...")

    def est_orphelin_5c(b):
        zone = zone_rayonnement(b['row'], b['col'], b['h'], b['w'], b['rayonnement'])
        return not any(
            {(p['row'] + dr, p['col'] + dc)
             for dr in range(p['h']) for dc in range(p['w'])} & zone
    ...
    t5c = time.time()
    MAX_P5C = min(remaining() * 0.7, 20)
    ameliorations_p5c = 0
    s_obj_5c = objectif_enrichi()

    while time.time() - t5c < MAX_P5C:
        # Identifier tous les orphelins
        orphelins_idx = [
            (i, b) for i, b in enumerate(places)
            if b['type'] == 'Culturel' and b['rayonnement'] > 0 and est_orphelin_5c(b)
        ]
        if not orphelins_idx:
            break

        # Calculer le meilleur mouvement pour CHAQUE orphelin
        best_global_gain = 0
        best_global_move = None  # (ci, r, c, h, w)

        for ci, b in orphelins_idx:
            h, w = b['h'], b['w']
            old_r, old_c = b['row'], b['col']
            liberer_g(grid, old_r, old_c, h, w)
            best_obj_ci = s_obj_5c
            best_pos_ci = None
            ...
            # B) Orientations alternatives (si bâtiment non carré)
            if h != w:
                for r in range(rows):
                    for c in range(cols):
                        if peut_placer(grid, r, c, w, h):
                            places[ci]['row'], places[ci]['col'] = r, c
                            places[ci]['h'], places[ci]['w'] = w, h
                            obj = objectif_enrichi()
                            if obj > best_obj_ci:
                                best_obj_ci = obj
                                best_pos_ci = (r, c, w, h)
                            places[ci]['row'], places[ci]['col'] = old_r, old_c
                            places[ci]['h'], places[ci]['w'] = h, w

            placer_g(grid, old_r, old_c, h, w)

            gain = best_obj_ci - s_obj_5c
            if gain > best_global_gain:
                best_global_gain = gain
                best_global_move = (ci, best_pos_ci)

        if best_global_move is None:
            break

        ci, best_pos = best_global_move
        b = places[ci]
        old_r, old_c, old_h, old_w = b['row'], b['col'], b['h'], b['w']
        r, c, h, w = best_pos
        liberer_g(grid, old_r, old_c, old_h, old_w)
        placer_g(grid, r, c, h, w)
        places[ci]['row'], places[ci]['col'] = r, c
        places[ci]['h'], places[ci]['w'] = h, w
        places[ci]['orientation'] = 'H' if h == places[ci]['largeur'] else 'V'
        s_obj_5c = objectif_enrichi()
        score_p1 = recalc_all(places)
        ameliorations_p5c += 1
    ...
    recalc_all(places)
    if progress_cb:
        progress_cb(0.97, f"Passe 5C : {ameliorations_p5c} orphelins relocalisés. Score={score_p1}")

    # ── PASSE 5D : re-relocation des culturels apres les passes 5B et 5C ──────────
    # Les passes 5B (producteurs) et 5C (orphelins) ont change l'etat du terrain.
    # Cette passe relance la logique de la passe 5 sur le nouvel etat pour capturer
    # les ameliorations qui n'etaient pas visibles avant ces deplacements.
    if progress_cb:
        progress_cb(0.975, "Passe 5D : relocalisation complementaire des culturels...")

    t5d = time.time()
    MAX_P5D = min(remaining() * 0.6, 15)
    ameliorations_p5d = 0
    s_obj_5d = objectif_enrichi()

    while time.time() - t5d < MAX_P5D:
        improved = False
        cinfo_5d = sorted(
            [(i, culture_delivree(b), est_orphelin_5c(b))
             for i, b in enumerate(places)
             if b['type'] == 'Culturel' and b['rayonnement'] > 0],
            key=lambda x: (0 if x[2] else 1, x[1])
        )
        for ci, _, _ in cinfo_5d:
            b = places[ci]
            h, w = b['h'], b['w']
            liberer_g(grid, b['row'], b['col'], h, w)
            best_obj_5d, best_pos_5d = s_obj_5d, None
            old_r, old_c = b['row'], b['col']
            for r in range(rows):
                for c in range(cols):
                    if peut_placer(grid, r, c, h, w) and (r, c) != (old_r, old_c):
    ...
    recalc_all(places)
    if progress_cb:
        progress_cb(0.98, f"Passe 5D : {ameliorations_p5d} relocalisations complementaires. Score={score_p1}")
