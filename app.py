...
class Building:
    ...
    def __init__(self, nom, longueur, largeur, nombre, type_bat, culture, rayonnement,
        ...
        self.type = type_bat
        self.culture = int(culture) if culture != "Rien" else 0
        self.rayonnement = int(rayonnement)
        self.boost_25 = int(boost_25) if boost_25 != "" else 0
        self.boost_50 = int(boost_50) if boost_50 != "" else 0
        self.boost_100 = int(boost_100) if boost_100 != "" else 0
        self.production = production
        self.quantite = int(quantite) if quantite != "" else 0
...
class BuildingInstance:
    ...
    def _update_boost(self):
        ...
        if self.culture_recue >= thresholds[2]:
            self.boost_percentage = 100
        elif self.culture_recue >= thresholds[1]:
            self.boost_percentage = 50
        elif self.culture_recue >= thresholds[0]:
...
class Terrain:
    ...
    def remove_building(self, instance):
        ...
        cells = instance.get_occupied_cells()
        for x, y in cells:
            if self.grid[x, y] == instance.building.nom:
                self.grid[x, y] = ''
...
class Optimizer:
    ...
    def get_all_positions(self, building):
        ...
        for x in range(self.terrain.height - l + 1):
            for y in range(self.terrain.width - L + 1):
                ...
                cells = instance.get_occupied_cells()
                if self.terrain.is_within_bounds(cells) and not self.terrain.is_occupied(cells):
    ...
    def evaluate_position(self, building_instance, cultural_instances):
        ...
        if building_instance.building.type == 'Producteur':
            ...
            if total_culture >= thresholds[2]:
                score = 3  # Boost 100%
            elif total_culture >= thresholds[1]:
                score = 2  # Boost 50%
            elif total_culture >= thresholds[0]:
                score = 1  # Boost 25%
            else:
                score = 0
    ...
    def optimize(self):
        ...
        producteur_buildings = [b for b in self.buildings if b.type == 'Producteur']
...
class ExcelProcessor:
    ...
    def export_results(terrain, optimizer, original_terrain):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            operations = []
            for move in optimizer.moves:
                operations.append({
                    'Étape': f"Déplacer {move['nom']} de ({move['avant_x']}, {move['avant_y']}) vers ({move['apres_x']}, {move['apres_y']})"
                })
            if operations:
...
def main():
    ...
    if uploaded_file is not None:
        try:
            ...
            st.info("Vérifiez que votre fichier Excel contient bien les onglets 'Terrain' et 'Batiments' avec le format attendu.")
