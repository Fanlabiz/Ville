    ...
    def calculate_culture(self):
        ...
        for pb in self.placed_buildings:
            if pb['info']['Type'] == 'Culturel':
                ...
                if rayon > 0 and culture_value > 0:
                    # Calculer la zone de rayonnement (inclut le bâtiment lui-même)
                    r_start = max(0, pb['r'] - rayon)
                    r_end = min(self.rows, pb['r'] + pb['h'] + rayon)
                    c_start = max(0, pb['c'] - rayon)
                    c_end = min(self.cols, pb['c'] + pb['w'] + rayon)
                    
                    # Ajouter la culture à chaque case de la zone de rayonnement
                    # Chaque case reçoit la valeur totale du bâtiment culturel
                    culture_map[r_start:r_end, c_start:c_end] += culture_value
        ...
        for pb in self.placed_buildings:
            if pb['info']['Type'] == 'Producteur':
                # Récupérer la valeur maximale de culture dans l'empreinte du bâtiment
                # (car le bâtiment reçoit la culture si une de ses cases est dans la zone)
        ...
        for r in range(planner.rows):
            for c in range(planner.cols):
                if planner.border_mask[r, c]:
                    cell = ws.cell(row=r+1, column=c+1, value="X")
                    cell.fill = PatternFill(start_color='FF000000', fill_type='solid')  # Noir pour les bords
        
        # Marquer les bâtiments placés
        for pb in planner.placed_buildings:
            fill = PatternFill(start_color=COLORS.get(pb['info']['Type'], 'FFFFFFFF'), fill_type='solid')
            for r in range(pb['r'], pb['r']+pb['h']):
                for c in range(pb['c'], pb['c']+pb['w']):
                    cell = ws.cell(row=r+1, column=c+1, value=pb['info']['Nom'][:5])  # Nom tronqué
                    cell.fill = fill
