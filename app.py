...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c):
    """
    Place sur le terrain les batiments declares dans buildings_def
    qui ne sont pas encore presents sur le terrain (ou en nombre insuffisant).

    Strategie de placement initial :
    - Les Producteurs sont places en priorite au centre du terrain
      (pour maximiser les chances de couverture culturelle).
    - Les Culturels sont places juste apres, pres des producteurs.
    - Les Neutres en dernier, dans les cases restantes.
    - Les deux orientations sont testees ; on choisit celle qui laisse
      le plus de cases libres adjacentes (heuristique de compacite).

    Retourne (new_placed, n_placed, n_failed) :
      new_placed  : liste complete (anciens + nouveaux)
      n_placed    : nombre de batiments effectivement places
      n_failed    : nombre de batiments n'ayant pas pu etre places (terrain plein)
    """
    from collections import Counter

    # Compter les instances deja presentes par nom
    placed_counts = Counter(b["nom"].strip() for b in placed)

    # Construire la liste des batiments a placer, dans l'ordre de priorite
    to_place = []
    for b_def in buildings_def:
        nom = b_def["nom"].strip()
        already = placed_counts.get(nom, 0)
        needed  = b_def["nombre"] - already
        for _ in range(needed):
            to_place.append(dict(b_def, nom=nom,
                                 rows=b_def["largeur"],
                                 cols=b_def["longueur"]))

    # Ordre : Producteurs -> Culturels -> Neutres
    # (pour que les culturels se placent pres des producteurs)
    priority = {"Producteur": 0, "Culturel": 1, "Neutre": 2}
    to_place.sort(key=lambda b: priority.get(b["type"], 9))

    # Calcul du centre utile du terrain (hors bords X)
    internal = [(r, c) for r in range(max_r) for c in range(max_c)
                if terrain_grid[r][c] != "X"]
    if internal:
        cr = sum(r for r, c in internal) / len(internal)
        cc = sum(c for r, c in internal) / len(internal)
    else:
        cr, cc = max_r / 2, max_c / 2

    x_grid = [[terrain_grid[r][c] == "X" for c in range(max_c)] for r in range(max_r)]
    new_placed = [dict(b) for b in placed]
    n_placed = 0
    n_failed = 0

    def occ_grid_fast():
        """Grille d'occupation courante."""
        occ = [[False] * max_c for _ in range(max_r)]
        for b in new_placed:
            for dr in range(b["rows"]):
                for dc in range(b["cols"]):
                    rr, cc2 = b["r"] + dr, b["c"] + dc
                    if 0 <= rr < max_r and 0 <= cc2 < max_c:
                        occ[rr][cc2] = True
        return occ

    def can_place_here(r, c, rows, cols, occ):
        if r < 0 or c < 0 or r + rows > max_r or c + cols > max_c:
            return False
        return not any(x_grid[r+dr][c+dc] or occ[r+dr][c+dc]
                       for dr in range(rows) for dc in range(cols))

    def free_neighbors(r, c, rows, cols, occ):
        """Nombre de cases libres adjacentes (heuristique de compacite)."""
        count = 0
        for dr in range(-1, rows + 1):
            for dc in range(-1, cols + 1):
                rr, cc2 = r + dr, c + dc
                if 0 <= rr < max_r and 0 <= cc2 < max_c:
                    if not x_grid[rr][cc2] and not occ[rr][cc2]:
                        count += 1
        return count
    ...
    for b in to_place:
        occ = occ_grid_fast()
        best_pos   = None
        best_score = None  # (distance_au_centre, -free_neighbors)

        # Tester les deux orientations
        orientations = [(b["rows"], b["cols"])]
        if b["rows"] != b["cols"]:
            orientations.append((b["cols"], b["rows"]))

        for rows, cols in orientations:
            for r in range(max_r):
                for c in range(max_c):
                    if can_place_here(r, c, rows, cols, occ):
                        # Centre du batiment
                        br = r + rows / 2
                        bc = c + cols / 2
                        dist = abs(br - cr) + abs(bc - cc)
                        fn   = free_neighbors(r, c, rows, cols, occ)
                        score = (dist, -fn)
        ...
        if best_pos:
            ...
            new_placed.append({**b, "r": r, "c": c, "rows": rows, "cols": cols})
            n_placed += 1
        else:
            n_failed += 1

    return new_placed, n_placed, n_failed
...
if uploaded:
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if do_place_missing and missing_list:
            status.info("Placement des batiments manquants...")
            placed_for_optim, n_placed_new, n_failed_new = place_missing_buildings(
                placed, buildings_def, terrain_grid, max_r, max_c
            )
            if n_failed_new > 0:
        ...
        # Le "original_placed" pour le fichier = etat apres placement manquants, avant optimisation
        with st.spinner("Generation du fichier Excel..."):
            output_buf = build_excel_output(
                optimized, placed_for_optim, terrain_grid, max_r, max_c, buildings_def
            )
