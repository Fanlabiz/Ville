...
class CityPlanner:
    ...
    def calculate_culture(self):
        ...
        Un producteur reçoit la SOMME des cultures de TOUS les bâtiments culturels
        dont la zone de rayonnement l'atteint (même partiellement).
        """
        # Liste de tous les bâtiments culturels placés
        ...
        # Pour chaque bâtiment producteur
        for pb in self.placed_buildings:
            if pb['info']['Type'] == 'Producteur':
                ...
                # Liste des cultures reçues de chaque bâtiment culturel
                cultures_recues = []
                
                # Vérifier chaque bâtiment culturel
                for cb in cultural_buildings:
                    ...
                    if rayon > 0 and culture_value > 0:
                        # Coordonnées du culturel
                        cult_r_start = cb['r']
                        cult_r_end = cb['r'] + cb['h']
                        cult_c_start = cb['c']
                        cult_c_end = cb['c'] + cb['w']
                        
                        # Vérifier si UNE case du producteur est dans la zone de rayonnement du culturel
                        est_dans_zone = False
                        
                        # Pour chaque case du producteur
                        for pr in range(prod_r_start, prod_r_end):
                            for pc in range(prod_c_start, prod_c_end):
                                # Calculer la distance de Manhattan à chaque case du culturel
                        ...
                        if est_dans_zone:
                            ...
                            self.log(f"  DEBUG - {pb['info']['Nom']} reçoit {culture_value} de {cb['info']['Nom']}")
                
                # SOMME de toutes les cultures reçues
        ...
        # Log spécifique pour la caserne de siège
        for stat in prod_stats:
            if stat['Nom'] == 'Caserne de siège':
                self.log(f"  RÉSULTAT - Caserne de siège reçoit {stat['Culture reçue']} de culture")
                break
        ...
        return {
            ...
            'culture_map': np.zeros((self.rows, self.cols))  # Non utilisé mais conservé pour compatibilité
        }
