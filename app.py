...
class CityPlanner:
    ...
    def can_place(self, r, c, w, h, remaining_queue):
        ...
        # RÈGLE DU POINT 2 : Vérifier qu'il reste de la place pour le plus grand restant
        if remaining_queue:
    ...
    def solve(self, buildings):
        ...
        # PHASE 1 : Priorité Bords (pour Neutres)
        for w, h in dims:
        ...
        # PHASE 2 : Secours (si Neutre pas placé sur bord)
        if b['Type'] == 'Neutre':
            for w, h in dims:
                for r in range(self.rows - h + 1):
                    for c in range(self.cols - w + 1):
...
def generate_excel(planner, full_queue):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # 2. CARTE DE CULTURE (SOMME CUMULATIVE)
        ...
        for pb in planner.placed_buildings:
            if pb['info']['Type'] == 'Producteur':
                # On lit la valeur cumulée sur la case d'origine du bâtiment
        ...
        summary = [
            ...
            ["Nombre de bâtiments non placés", len(not_placed)],
            ["Volume total non placé", sum(b['Longueur']*b['Largeur'] for b in not_placed)],
...
# --- INTERFACE STREAMLIT ---
st.title("Optimiseur de Ville - Calcul Somme Culture 🏗️")
