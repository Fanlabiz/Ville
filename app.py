...
def _detect_building_type(row):
    ...
    - Culture > 0 et Seuil100 == 0 → Culturel
    - Sinon → Neutre
    """
    if row["Seuil100"] > 0:
        return "Producteur"
...
def _camel_to_words(s):
    """Insère un espace avant chaque majuscule suivant une minuscule. Ex: CultureSite → Culture Site."""
    import re
    return re.sub(r'(?<=[a-z])(?=[A-Z])', ' ', s)


def _short_building_name(nom_complet):
    """
    Extrait le nom court d'un bâtiment RoC depuis son nom technique complet.

    Structure standard (5 parties) :
      Building _ [Era] _ [Categorie] _ [SousType] _ [Niveau]
      garde uniquement [SousType] (partie index 3)

    Structure Harbor (4 parties) :
      Building _ Harbor _ [Nom] _ [Niveau]
      garde [Nom] (partie index 2)
    """
    if nom_complet == "PURCHASED_BLOCK":
        return "PURCHASED_BLOCK"
    parts = nom_complet.split("_")
    if len(parts) >= 5:
        return _camel_to_words(parts[3])
    elif len(parts) == 4:
        return _camel_to_words(parts[2])
    elif len(parts) == 3:
        return _camel_to_words(parts[2])
    return nom_complet
...
def read_from_csv(file_obj, city_name=None):
    ...
    for _, row in df_real.iterrows():
        ...
        if key not in catalog_rows:
            ...
            short = _short_building_name(key)
            catalog_rows[key] = {
                "nom":          short,          # nom court affiché dans l'UI et l'Excel
                "nom_complet":  key,            # identifiant technique RoC (conservé pour référence)
                "longueur":    int(row["Largeur"]),
                "largeur":     int(row["Hauteur"]),
                "nombre":      0,       # compté ci-dessous
                "type":        btype,
                "culture":     float(row["Culture"]),
                "rayonnement": int(row["Rayonnement"]),
                "boost25":     float(row["Seuil25"]),
                "boost50":     float(row["Seuil50"]),
                "boost100":    float(row["Seuil100"]),
                "production":  "",
                "quantite":    0.0,
                "priorite":    0.0,
...
if uploaded_csv is not None:
    ...
    try:
        ...
        if len(_csv_cities) > 1:
            _selected_city = st.selectbox(
                "Plusieurs villes détectées dans le CSV — choisissez celle à importer :",
                _csv_cities
            )
        else:
            _selected_city = _csv_cities[0]
            st.info(f"Ville détectée : **{_selected_city}**")
...
if uploaded_csv is not None and _csv_data_ready:
    ...
    try:
        ...
        terrain_grid, max_r, max_c, placed, buildings_def, _ = read_from_csv(
            uploaded_csv, city_name=_selected_city
        )
...
# ── Tableau de configuration priorités/placement (mode CSV uniquement) ──
if uploaded_csv is not None and _csv_data_ready and 'buildings_def' in dir():
    import pandas as _pd_cfg

    # Initialiser ou récupérer les configs sauvegardées en session
    if "csv_priorities" not in st.session_state or st.session_state.get("csv_city") != _selected_city:
        # Première fois ou changement de ville : initialiser depuis buildings_def
        # Détecter les noms courts en doublon pour les disambiguiser
        _short_counts = {}
        for _bd in buildings_def:
            _sn = _bd["nom"]
            _short_counts[_sn] = _short_counts.get(_sn, 0) + 1

        _init_rows = []
        for _bd in sorted(buildings_def, key=lambda b: (b["type"], b["nom"])):
            _sn = _bd["nom"]
            # Si doublon, ajouter l'ère (partie 2 du nom complet) pour disambiguiser
            if _short_counts.get(_sn, 1) > 1:
                _parts = _bd.get("nom_complet", _sn).split("_")
                _era = _parts[1] if len(_parts) > 1 else ""
                _display_name = f"{_sn} ({_era})"
            else:
                _display_name = _sn
            _init_rows.append({
                "nom_complet": _bd.get("nom_complet", _bd["nom"]),   # clé technique (cachée)
                "Nom":         _display_name,
                "Type":        _bd["type"],
                "Qté":         _bd["nombre"],
                "Culture":     int(_bd["culture"]) if _bd["culture"] else "",
                "Ray.":        int(_bd["rayonnement"]) if _bd["rayonnement"] else "",
                "Seuil 100%":  int(_bd["boost100"]) if _bd["boost100"] else "",
                "Priorité":    int(_bd["priorite"]),
                "Placement":   _bd["placement"],
            })
        st.session_state.csv_priorities = _init_rows
        st.session_state.csv_city = _selected_city

    st.divider()
    st.subheader("⚙️ Configuration des bâtiments")
    st.caption(
        "Définissez la **priorité** (0 = ignoré, 1–10 = crescendo d'importance) "
        "et le **placement** (Obligatoire / Optionnel) pour chaque bâtiment. "
        "Ces réglages guident l'optimiseur : les producteurs à haute priorité "
        "seront préférentiellement entourés de bâtiments culturels."
    )

    _col_cfg, _col_help = st.columns([3, 1])
    with _col_help:
        with st.expander("💡 Aide priorités"):
            st.markdown("""
**Priorité**
- `0` : bâtiment ignoré par l'optimiseur (fermes, maisons…)
- `1–4` : importance faible
- `5–7` : importance moyenne
- `8–10` : haute priorité (casernes, ateliers rares…)

**Placement**
- `Obligatoire` : doit être placé (par défaut)
- `Optionnel` : placé seulement si de la place reste
            """)

    with _col_cfg:
        _df_editor = _pd_cfg.DataFrame(st.session_state.csv_priorities)
        # Colonnes à afficher (nom_complet est masqué — on le garde dans le df mais on ne l'expose pas)
        _display_cols = ["Nom", "Type", "Qté", "Culture", "Ray.", "Seuil 100%", "Priorité", "Placement"]
        _edited = st.data_editor(
            _df_editor[_display_cols + ["nom_complet"]],
            column_config={
                "nom_complet": None,   # colonne cachée
                ...
                "Culture":    st.column_config.TextColumn("Culture", disabled=True, width="small"),
                "Ray.":       st.column_config.TextColumn("Ray.", disabled=True, width="small"),
                "Seuil 100%": st.column_config.TextColumn("Seuil 100%", disabled=True, width="small"),
                "Priorité":   st.column_config.NumberColumn(
                    "Priorité", min_value=0, max_value=10, step=1,
                    help="0 = ignoré, 10 = très haute priorité", width="small"
                ),
                "Placement":  st.column_config.SelectboxColumn(
                    "Placement",
                    options=["Obligatoire", "Optionnel"],
                    width="medium"
                ),
            },
            hide_index=True,
            ...
            num_rows="fixed",
            key="editor_csv_priorities",
        )

    # Appliquer la configuration (immédiate + sur bouton)
    def _apply_csv_priorities(_edited_df, _buildings_def, _placed):
        """Répercute les réglages du tableau dans buildings_def et placed."""
        _prio_map = {
            r["nom_complet"]: (int(r["Priorité"]), r["Placement"])
            for r in _edited_df.to_dict("records")
        }
        for _bd in _buildings_def:
            _key = _bd.get("nom_complet", _bd["nom"])
            if _key in _prio_map:
                _bd["priorite"]  = float(_prio_map[_key][0])
                _bd["placement"] = _prio_map[_key][1]
        for _b in _placed:
            _key = _b.get("nom_complet", _b["nom"])
            if _key in _prio_map:
                _b["priorite"]  = float(_prio_map[_key][0])
                _b["placement"] = _prio_map[_key][1]

    if st.button("✅ Appliquer la configuration", type="secondary"):
        _records = _edited.to_dict("records")
        # Mettre à jour la session avec les nouvelles valeurs
        _saved = st.session_state.csv_priorities
        _nc_to_saved = {r["nom_complet"]: r for r in _saved}
        for _rec in _records:
            _nc = _rec["nom_complet"]
            if _nc in _nc_to_saved:
                _nc_to_saved[_nc]["Priorité"]  = _rec["Priorité"]
                _nc_to_saved[_nc]["Placement"] = _rec["Placement"]
        st.session_state.csv_priorities = list(_nc_to_saved.values())
        _apply_csv_priorities(_edited, buildings_def, placed)
        original_placed = [dict(b) for b in placed]
        st.success("Configuration appliquée ! Vous pouvez maintenant lancer l'optimisation.")
    else:
        # Appliquer silencieusement les valeurs courantes de la session
        _apply_csv_priorities(
            _pd_cfg.DataFrame(st.session_state.csv_priorities),
            buildings_def, placed
        )
        original_placed = [dict(b) for b in placed]
