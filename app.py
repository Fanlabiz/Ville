...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        Version corrigée qui détecte TOUS les bâtiments individuellement
        """
        ...
        for r in range(len(actuel_str)):
            for c in range(len(actuel_str[0])):
                ...
                if cell_value in self.possible_dimensions:
                    ...
                    # Essayer chaque dimension dans l'ordre croissant
                    found = False
                    ...
                    if not found:
                        # Si aucune dimension ne correspond, on prend la plus petite dimension
                        # pour les bâtiments qui peuvent être multiples
                        min_w, min_h = min(possible_dims, key=lambda x: x[0] * x[1])
                        
                        # Prendre un seul bâtiment de taille minimale
                    ...
                    # Si le type de bâtiment n'est pas dans possible_dimensions, on le traite comme inconnu
                    # mais on prend quand même la plus petite dimension si possible
        ...
        for temp in temp_buildings:
            ...
            if nom in building_pool and building_pool[nom]:
                ...
                if found:
                    ...
                    # Aucune correspondance de dimensions
...
- **Détection systématique** des bâtiments (prend la plus petite dimension possible)
