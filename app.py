...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        """
        # Dictionnaire pour identifier les bâtiments par leur nom
        # (on aura besoin de leurs caractéristiques plus tard)
        
        for r in range(min(len(actuel_data), self.rows)):
            for c in range(min(len(actuel_data[0]), self.cols)):
                cell_value = str(actuel_data[r][c]).strip()
                
                # Ignorer les cases vides, les X et les 1/0
                if cell_value in ['', 'X', '1', '0', 'nan']:
                    continue
                
                # Vérifier si ce bâtiment n'a pas déjà été enregistré à cette position
                ...
                if not already_placed:
                    # C'est un nouveau bâtiment - on va devoir déterminer ses dimensions
                    # Pour l'instant, on enregistre juste la position et on marquera
                    # la grille comme occupée
                    self.grid[r, c] = 0
                    
                    # Note: les vraies dimensions seront déterminées quand on aura
    ...
    def match_buildings_with_info(self, buildings_info):
        ...
        et détermine leurs dimensions réelles
        """
        # Créer un dictionnaire des bâtiments par nom
        building_dict = {}
        for b in buildings_info:
            nom = b['Nom']
            if nom not in building_dict:
                building_dict[nom] = []
            building_dict[nom].append(b)
        ...
        for temp in temp_buildings:
            ...
            if nom in building_dict and building_dict[nom]:
                # Prendre la première occurrence disponible
                info = building_dict[nom].pop(0)
                
                # Déterminer les dimensions réelles
                w = info['Largeur']
                h = info['Longueur']
                
                # Vérifier l'orientation (si le nom est répété sur plusieurs cases)
                # On regarde si le nom s'étend horizontalement ou verticalement
                h_extend = 1
                w_extend = 1
                
                # Chercher l'extension horizontale
                for dc in range(1, w):
                    if c + dc < self.cols:
                        cell_val = str(temp.get('ligne_' + str(dc), ''))  # Approximation
                        # Dans un vrai scénario, il faudrait analyser le dataframe
                
                # Pour l'instant, on utilise les dimensions du dictionnaire
                ...
                self.log(f"Bâtiment existant: {nom} à ({r},{c}) dimensions {w}x{h}")
            else:
                self.log(f"Attention: bâtiment '{nom}' non trouvé dans la liste")
    ...
    def optimize_placement(self, iterations=20):
        """
...
if uploaded:
    with st.spinner("Analyse du fichier en cours..."):
        try:
            ...
            # Essayer de lire l'onglet 3 (peut ne pas exister)
            try:
            ...
            planner.match_buildings_with_info(all_buildings)
