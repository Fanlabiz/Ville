...
Version finale - Placement correct à l'intérieur des bordures
"""
...
class Terrain:
    ...
    def __init__(self, df_terrain):
        ...
        # Appeler _parse_terrain après initialisation
    ...
    def _parse_terrain(self, df):
        ...
        if x_rows:
            ...
            # Sauvegarder les positions originales des bâtiments
            for i in range(interior.shape[0]):
                for j in range(interior.shape[1]):
                    val = interior[i, j]
                    if val and str(val).strip() != '' and str(val).strip() != ' ':
                        self.original_buildings[(i, j)] = str(val)
            return interior, interior.shape[0], interior.shape[1], (min_row, min_col)
        
        return arr, arr.shape[0], arr.shape[1], (0, 0)
    
    def get_original_building_position(self, building_name, index):
        """Récupère la position originale d'un bâtiment"""
        positions = []
        for (x, y), name in self.original_buildings.items():
    ...
    def clear_buildings(self):
        """Supprime tous les bâtiments du terrain"""
        for instance in self.building_instances:
    ...
    def remove_building(self, instance):
        if instance not in self.building_instances:
            return False
...
class AdvancedOptimizer:
    ...
    def optimize(self):
        ...
        # Créer la liste des positions originales des producteurs
        original_producteur_positions = []
        idx_counter = {b.nom: 0 for b in producteur_buildings}
        for building in producteur_buildings:
            for i in range(building.nombre):
                pos = self.terrain.get_original_building_position(building.nom, idx_counter[building.nom])
                idx_counter[building.nom] += 1
                if pos:
                    original_producteur_positions.append((building, pos[0], pos[1], 'h'))
                else:
                    original_producteur_positions.append((building, 0, 0, 'h'))
        ...
        # Enregistrer les déplacements avec les positions originales
        for i, (orig_building, orig_x, orig_y, orig_ori) in enumerate(original_producteur_positions):
            if i < len(new_producteurs):
                new = new_producteurs[i]
                if (orig_x, orig_y) != (new.x, new.y) or orig_ori != new.orientation:
                    self.moves.append({
                        'nom': new.building.nom,
                        'avant_x': orig_x,
                        'avant_y': orig_y,
                        'avant_orientation': 'Horizontal' if orig_ori == 'h' else 'Vertical',
                        'apres_x': new.x,
                        'apres_y': new.y,
...
class ExcelExporter:
    ...
    def _create_terrain_sheet(ws: Worksheet, terrain: Terrain):
        """Crée la feuille du terrain avec les bordures exactes comme dans l'input"""
        ...
        # Récupérer les dimensions avec les bordures
        border_offset = terrain.border_offset
        original_grid = terrain.original_grid
        
        total_height = original_grid.shape[0]
        total_width = original_grid.shape[1]
        
        # Créer une grille de travail
        work_grid = [['' for _ in range(total_width)] for _ in range(total_height)]
        
        # Remplir avec les X
        for i in range(total_height):
            for j in range(total_width):
                cell_value = str(original_grid[i, j]).strip() if original_grid[i, j] != '' else ''
                if cell_value == 'X':
                    work_grid[i][j] = 'X'
        
        # Ajouter les bâtiments optimisés
        for instance in terrain.building_instances:
            cells = instance.get_occupied_cells()
            if cells:
                # La première case intérieure est à (border_offset[0]+1, border_offset[1]+1)
                start_i = border_offset[0] + 1
                start_j = border_offset[1] + 1
                
                for (x, y) in cells:
                    real_i = start_i + x
                    real_j = start_j + y
                    if 0 <= real_i < total_height and 0 <= real_j < total_width:
                        work_grid[real_i][real_j] = instance.building.nom
        
        # Créer un dictionnaire des cellules de départ des bâtiments
        building_start_cells = {}
        for instance in terrain.building_instances:
            cells = instance.get_occupied_cells()
            if cells:
                min_cell = min(cells, key=lambda c: (c[0], c[1]))
                start_i = border_offset[0] + 1 + min_cell[0]
                start_j = border_offset[1] + 1 + min_cell[1]
                # Vérifier que le bâtiment ne dépasse pas les bordures
                ...
                if (start_i > 0 and start_j > 0 and 
                    start_i + height - 1 < total_height - 1 and 
                    start_j + width - 1 < total_width - 1):
                    building_start_cells[(start_i, start_j)] = instance
        ...
        for i in range(total_height):
            for j in range(total_width):
                ...
                cell_value = work_grid[i][j]
                
                if cell_value == 'X':
                    cell.value = "X"
                    cell.fill = PatternFill(start_color=COLOR_BORDURE, end_color=COLOR_BORDURE, fill_type="solid")
                    cell.font = Font(bold=True, color="FFFFFF")
                elif (i, j) in building_start_cells:
                    # C'est une cellule de départ de bâtiment
                    instance = building_start_cells[(i, j)]
                    l, L = instance.building.longueur, instance.building.largeur
                    if instance.orientation == 'h':
                        height, width = l, L
                    else:
                    ...
                    color = COLOR_CULTUREL if instance.building.type == 'Culturel' else COLOR_PRODUCTEUR
                    ...
                    # Bâtiment existant non optimisé
                    cell.value = cell_value
                    cell.fill = PatternFill(start_color="D3D3D3", end_color="D3D3D3", fill_type="solid")
                    cell.font = Font(bold=True, size=10)
                else:
                    cell.value = ""
                    cell.fill = PatternFill(start_color=COLOR_VIDE, end_color=COLOR_VIDE, fill_type="solid")
...
def main():
    ...
    **Stratégie d'optimisation :**
    1. 📍 Lecture des positions originales des bâtiments
    2. 🎯 Optimisation du placement des culturels
    3. 🔄 Optimisation du placement des producteurs
        ...
        try:
            ...
            with st.spinner("Optimisation avancée en cours..."):
                ...
                2. Consultez l'onglet **Terrain_final** pour voir le placement optimisé
                   - Les bordures sont exactement comme dans l'input
                   - Les bâtiments optimisés sont colorés (orange pour culturels, vert pour producteurs)
