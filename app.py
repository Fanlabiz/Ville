...
class FastPlacement:
    ...
    def best_emitter_position(self, template, sample=150, compact_weight=1.0):
        """
        Place un emetteur de culture en maximisant la culture qu'il apportera
        aux batiments boostables DEJA places sur le terrain.
        """
        orientations = get_orientations(template)
        best = None  # (score, r, c, label, h, w)

        # Identifier les cases occupées par des boostables
        boostable_cells = set()
        for inst in self.instances:
            t = inst["template"]
            if t["boost_25"] > 0 or t["boost_50"] > 0 or t["boost_100"] > 0:
                for dr in range(inst["h"]):
                    for dc in range(inst["w"]):
                        boostable_cells.add((inst["row"]+dr, inst["col"]+dc))
        ...
        for (label, h, w) in orientations:
            ...
            for (r, c) in candidates:
                # Calculer combien de cases boostables seraient dans le rayon
                R = template["rayonnement"]
                r0 = max(0, r - R);  r1 = min(self.shape[0], r + h + R)
                c0 = max(0, c - R);  c1 = min(self.shape[1], c + w + R)

                # Compter les cases boostables couvertes par ce rayon
                covered = sum(1 for (br, bc) in boostable_cells
                              if r0 <= br < r1 and c0 <= bc < c1)

                # Score : maximiser la couverture de boostables, minimiser fragmentation
                compact = corner_score(self.grid, r, c, h, w)
                score = covered * 100 - compact * compact_weight
