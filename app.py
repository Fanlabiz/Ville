...
def write_sheet_liste(ws, placed, culture_map, original_placed):
    ...
    # Build original lookup by name for before/after comparison
    orig_by_name = {}
    ...
    headers = ['Nom', 'Type', 'Production', 'Ligne', 'Colonne', 'Hauteur', 'Largeur',
               'Culture reçue', 'Boost atteint (%)', 'Qté/h avant optim', 'Qté/h après optim', 'Gain/Perte']
    ...
    for ri, b in enumerate(placed, 2):
        ...
        row_data = [b['name'], b['type'], b['production'],
                    ...
                    cr, f"{boost}%" if b['type'] == 'Producteur' else '',
        ...
        for ci, val in enumerate(row_data, 1):
            ...
            if ci == len(headers) and isinstance(val, (int, float)) and val != '':
                cell.font = Font(bold=True, color="006600" if val >= 0 else "880000")
...
def write_sheet_terrain(ws, placed, playable, rows, cols, culture_map, label="Terrain optimisé"):
    ...
    TYPE_FILL = {
        'Culturel':   PatternFill("solid", fgColor="FF8C00"),   # orange
        'Producteur': PatternFill("solid", fgColor="228B22"),   # green
        'Neutre':     PatternFill("solid", fgColor="808080"),   # gray
    }
    X_FILL   = PatternFill("solid", fgColor="000000")
    ...
    # Pass 1: set X cells and empty cells
    for r in range(rows):
        for c in range(cols):
            excel_row = r + 2
            excel_col = c + 1
            ...
            if (r, c) in x_cells:
                ...
                # Both playable-empty and outside-empty: no fill, no value
                cell.fill = NO_FILL
    ...
    for b in placed:
        ...
        top_cell.font = Font(bold=True, size=9, color='FFFFFF')
