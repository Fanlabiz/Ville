...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        Version qui gère correctement les bâtiments adjacents
        """
        ...
        for r in range(len(actuel_str)):
            for c in range(len(actuel_str[0])):
                ...
                if cell_value in self.possible_dimensions:
                    possible_dims = self.possible_dimensions[cell_value]
                    ...
                    if not found:
                        # Si aucune dimension connue ne correspond, on va chercher des bâtiments individuels
                        # en prenant la plus petite dimension possible
                        
                        # Trier les dimensions par surface croissante pour prendre les plus petits d'abord
                        sorted_dims = sorted(possible_dims, key=lambda x: x[0] * x[1])
                        
                        for (w, h) in sorted_dims:
                            # Vérifier si on peut placer un bâtiment de cette dimension
                            if r + h <= self.rows and c + w <= self.cols:
                                # Vérifier que toutes les cellules du rectangle sont du bon type
                                valid = True
                                for dr in range(h):
                                    for dc in range(w):
                                        if r + dr >= len(actuel_str) or c + dc >= len(actuel_str[0]):
...
- **Gère les bâtiments adjacents** (ne les fusionne pas)
