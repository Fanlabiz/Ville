...
if uploaded:
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if optionnels_a_placer:
            ...
            def _best_pos_optionnel(opt_b, current):
                """Meilleure position pour un optionnel Culturel ou Neutre.
                Pour les Culturels: maximise gain de score, puis couverture
                des producteurs sous leur seuil 100%.
                Pour les Neutres: première case libre (pas d'impact sur score)."""
                _occ = make_occ_grid(current, max_r, max_c)
                _score_now = score_placement(current)

                if opt_b["type"] != "Culturel":
                    # Neutre: placer à la première case libre
                    for _rows, _cols in [(opt_b["rows"], opt_b["cols"]),
                                          (opt_b["cols"], opt_b["rows"])]:
                        for _r in range(max_r):
                            for _c in range(max_c):
                                if can_place(_r, _c, _rows, _cols, _xg_opt, _occ, max_r, max_c):
                                    return (_r, _c, _rows, _cols), 0
                    return None, 0

                # Culturel: chercher la position optimale
                _prods = [b for b in current if b["type"] == "Producteur"]
                _cults = [b for b in current if b["type"] == "Culturel"]
                _best_pos = None; _best_gain = 0; _best_cov = -1

                for _rows, _cols in [(opt_b["rows"], opt_b["cols"]),
                                      (opt_b["cols"], opt_b["rows"])]:
                    for _r in range(max_r - _rows + 1):
                        for _c in range(max_c - _cols + 1):
                            if not can_place(_r, _c, _rows, _cols, _xg_opt, _occ, max_r, max_c):
                                continue
                            _bat = {**opt_b, "r": _r, "c": _c, "rows": _rows, "cols": _cols}
                            _zone = radiation_zone(_bat)
                            # Gain de score
                            _gain = score_placement(current + [_bat]) - _score_now
                            # Couverture utile: culture vers producteurs sous 100%
                            _cov = sum(
                                opt_b["culture"] * max(0, 1 - culture_received(p, _cults) / p["boost100"])
                                for p in _prods
                                if cells_of(p) & _zone and p.get("boost100", 0) > 0
                            )
                            if _gain > _best_gain or (_gain == _best_gain and _cov > _best_cov):
                                _best_gain = _gain; _best_cov = _cov; _best_pos = (_r, _c, _rows, _cols)

                return _best_pos, _best_gain

            # Trier les optionnels: Culturels en premier (impactent le score),
            # puis Neutres; dans chaque groupe, par taille décroissante
            _opts_sorted = sorted(
                optionnels_a_placer,
                key=lambda b: (0 if b["type"] == "Culturel" else 1, -(b["rows"] * b["cols"]))
            )

            # Placer les optionnels tant qu'ils ont une couverture utile
            # (couvrent des producteurs sous leur seuil 100%).
            # Le score peut rester stable plusieurs placements avant de sauter
            # (ex: il faut 3 HdV pour franchir un palier).
            # On s'arrête seulement quand la couverture utile est nulle.
            _no_coverage_streak = 0
            _MAX_NO_COVERAGE = 2  # arrêter après 2 optionnels sans couverture utile
            for _opt_b in _opts_sorted:
                _pos, _gain = _best_pos_optionnel(_opt_b, optimized)
                if _pos is not None:
                    # Vérifier si ce bâtiment a une couverture utile
                    # (extraire la couverture calculée dans _best_pos_optionnel)
                    _has_coverage = (_gain > 0)
                    if not _has_coverage and _opt_b["type"] == "Culturel":
                        # Recalculer la couverture à la position choisie
                        _prods_chk = [b for b in optimized if b["type"] == "Producteur"]
                        _cults_chk = [b for b in optimized if b["type"] == "Culturel"]
                        _bat_chk = {**_opt_b, "r": _pos[0], "c": _pos[1],
                                    "rows": _pos[2], "cols": _pos[3]}
                        _zone_chk = radiation_zone(_bat_chk)
                        _cov_chk = sum(
                            _opt_b["culture"] * max(0, 1 - culture_received(p, _cults_chk) / p["boost100"])
                            for p in _prods_chk
                            if cells_of(p) & _zone_chk and p.get("boost100", 0) > 0
                        )
                        _has_coverage = _cov_chk > 0
                    optimized.append({**_opt_b, "r": _pos[0], "c": _pos[1],
                                      "rows": _pos[2], "cols": _pos[3]})
                    if _has_coverage:
                        _no_coverage_streak = 0
                    else:
                        _no_coverage_streak += 1
                        if _no_coverage_streak >= _MAX_NO_COVERAGE:
                            break
                else:
                    _no_coverage_streak += 1
                    if _no_coverage_streak >= _MAX_NO_COVERAGE:
            ...
            # Passe greedy finale pour profiter des nouveaux culturels
            if any(b["type"] == "Culturel" for b in optionnels_a_placer):
                _opt_greedy, _ = optimize(
                    optimized, terrain_grid, max_r, max_c, n_passes=1
                )
