...
try:
    ...
    strong_cult = [
        b for b in placed
        if b['Type'] == 'Culturel' and b.get('placed') and 'row' in b
    ]
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Terrain visuel – fusion corrigée
        wb = writer.book
        ...
        for b in placed:
            ...
            # Application du style (fill) sur TOUTES les cellules de la zone
            for ri in range(r, r + h):
                for ci in range(c, c + w):
                    ws.cell(row=ri + 1, column=ci + 1).fill = fill
