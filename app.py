...
##############################
# CLASSE DE PLACEMENT AVEC RECHERCHE TABOU ET RÉORGANISATION
##############################
class PlacementBatiments:
    ...
    def trouver_maximum_empty_rectangles(self):
        """
        Identifie les plus grands rectangles vides sur le terrain
        """
        mer_list = []
        ...
        for i in range(self.hauteur):
            for j in range(self.largeur):
                if self.est_case_libre(i, j):
                    # Initialiser la largeur maximale pour cette ligne
                    max_width = self.largeur - j
                    ...
                    for k in range(i, self.hauteur):
                        ...
                        # Calculer la largeur maximale pour cette ligne
                        width = 0
                        while (j + width < self.largeur and 
                               self.est_case_libre(k, j + width) and 
                               width < max_width):
                            width += 1
                        ...
                        hauteur = k - i + 1
                        if hauteur > 0 and max_width > 0:
                            mer_list.append({
                                'x': i,
                                'y': j,
                                'hauteur': hauteur,
                                'largeur': max_width,
                                'surface': hauteur * max_width
                            })
        
        # Filtrer pour ne garder que les rectangles maximaux
        mer_list_filtree = []
        for mer in mer_list:
            est_maximal = True
            for autre in mer_list:
                if (autre != mer and 
                    autre['x'] <= mer['x'] and 
                    autre['y'] <= mer['y'] and 
                    autre['x'] + autre['hauteur'] >= mer['x'] + mer['hauteur'] and 
                    autre['y'] + autre['largeur'] >= mer['y'] + mer['largeur']):
                    est_maximal = False
                    break
            if est_maximal:
                mer_list_filtree.append(mer)
        
        # Trier par surface décroissante
        mer_list_filtree.sort(key=lambda x: -x['surface'])
        
        return mer_list_filtree
    
    def phase_reorganisation(self):
        """
        Phase de réorganisation pour créer de grands espaces libres
        """
        st.write("### Phase 3: Réorganisation pour créer des espaces")
        
        # Identifier les bâtiments non placés restants
        non_places = []
        for bat in self.batiments:
            ...
            if quantite_placee < quantite_demandee:
                for _ in range(quantite_demandee - quantite_placee):
        ...
        # Identifier les espaces libres actuels
        mer_list = self.trouver_maximum_empty_rectangles()
        
        st.write(f"Espaces libres disponibles: {len(mer_list)}")
        for mer in mer_list[:5]:
            st.write(f"  - {mer['hauteur']}x{mer['largeur']} ({mer['surface']} cases) à ({mer['x']},{mer['y']})")
        
        # Vérifier si le plus grand espace peut accueillir le plus grand bâtiment non placé
        if mer_list and non_places:
            plus_grand_batiment = non_places[0]
            taille_necessaire = plus_grand_batiment['longueur'] * plus_grand_batiment['largeur']
            
            if mer_list[0]['surface'] >= taille_necessaire:
                st.write(f"✅ Le plus grand espace ({mer_list[0]['surface']} cases) peut accueillir {plus_grand_batiment['nom']} ({taille_necessaire} cases)")
                return True
            else:
                st.write(f"❌ Espace insuffisant pour {plus_grand_batiment['nom']} ({taille_necessaire} cases)")
                
                # Chercher des bâtiments à déplacer pour agrandir l'espace
                return self.tenter_reorganisation_ciblee(mer_list[0], plus_grand_batiment)
        
        return True
    
    def tenter_reorganisation_ciblee(self, espace_cible, batiment_a_placer):
        """
        Tente de déplacer des bâtiments autour d'un espace cible pour l'agrandir
        """
        st.write(f"Tentative de réorganisation autour de l'espace ({espace_cible['x']},{espace_cible['y']}) de taille {espace_cible['hauteur']}x{espace_cible['largeur']}")
        
        # Chercher des bâtiments à déplacer dans un rayon de 5 cases autour de l'espace
        zone_x_min = max(0, espace_cible['x'] - 5)
        zone_x_max = min(self.hauteur, espace_cible['x'] + espace_cible['hauteur'] + 5)
        zone_y_min = max(0, espace_cible['y'] - 5)
        zone_y_max = min(self.largeur, espace_cible['y'] + espace_cible['largeur'] + 5)
        
        batiments_dans_zone = []
        for i, bat in enumerate(self.batiments_places):
            # Vérifier si le bâtiment est dans la zone
            if (bat['x'] + bat['longueur'] > zone_x_min and bat['x'] < zone_x_max and
                bat['y'] + bat['largeur'] > zone_y_min and bat['y'] < zone_y_max):
                batiments_dans_zone.append((i, bat))
        
        st.write(f"  {len(batiments_dans_zone)} bâtiments dans la zone")
        
        # Trier par taille (déplacer d'abord les plus petits)
        batiments_dans_zone.sort(key=lambda x: x[1]['longueur'] * x[1]['largeur'])
        
        # Essayer de déplacer chaque bâtiment
        for idx, (i, bat) in enumerate(batiments_dans_zone[:10]):  # Limiter à 10 tentatives
            # Sauvegarder l'état
            x_old, y_old = bat['x'], bat['y']
            longueur, largeur = bat['longueur'], bat['largeur']
            
            # Retirer le bâtiment
            self.retirer_batiment(i)
            
            # Chercher une nouvelle position loin de la zone
            for dx in [-10, -5, 5, 10]:
                for dy in [-10, -5, 5, 10]:
                    x_new = x_old + dx
                    y_new = y_old + dy
                    ...
                    # Vérifier que la nouvelle position est loin de la zone cible
                    if (x_new < zone_x_min - 2 or x_new > zone_x_max + 2 or
                        y_new < zone_y_min - 2 or y_new > zone_y_max + 2):
                        ...
                        if self.peut_placer_batiment(x_new, y_new, longueur, largeur):
                            ...
                            self.placer_batiment(bat, x_new, y_new, longueur, largeur, culture)
                            self.stats_placement['reorganisations'] += 1
                            st.write(f"  ✅ {bat['nom']} déplacé de ({x_old},{y_old}) à ({x_new},{y_new})")
                            
                            # Recalculer les espaces libres
                            nouveaux_mer = self.trouver_maximum_empty_rectangles()
                            if nouveaux_mer and nouveaux_mer[0]['surface'] > espace_cible['surface']:
                                st.write(f"  ✅ Nouvel espace créé: {nouveaux_mer[0]['surface']} cases")
                                return True
                            
                            # Revenir à l'état précédent si pas d'amélioration
                            self.retirer_batiment(-1)
                            self.placer_batiment(bat, x_old, y_old, longueur, largeur, bat['culture_recue'])
                            break
            
            # Remettre le bâtiment à sa place si aucun déplacement n'a fonctionné
            if i < len(self.batiments_places) and self.batiments_places[i]['nom'] != bat['nom']:
                self.placer_batiment(bat, x_old, y_old, longueur, largeur, bat['culture_recue'])
    ...
    def calculer_score_configuration(self):
        ...
        # Bonus pour les grands espaces libres (favorise la compacité)
        mer_list = self.trouver_maximum_empty_rectangles()
        if mer_list:
            # Pénalité si le plus grand espace est trop petit
            if mer_list[0]['surface'] < 20:
                score -= 500
    ...
    def executer_placement(self):
        """
        Exécute l'algorithme complet avec recherche tabou et réorganisation
        """
        ...
        # Afficher les statistiques après recherche tabou
        non_places_tabou = self.compter_batiments_non_places()
        st.write(f"Bâtiments non placés après recherche tabou: {non_places_tabou}")
        
        # Phase 3: Réorganisation si nécessaire
        if non_places_tabou > 0:
            self.phase_reorganisation()
        
        # Statistiques finales
        non_places_final = self.compter_batiments_non_places()
        st.write(f"Bâtiments non placés après réorganisation: {non_places_final}")
        ...
        st.write(f"Réorganisations: {self.stats_placement['reorganisations']}")
...
# Titre
st.title("🏗️ Optimiseur de Placement de Bâtiments (avec Réorganisation)")
st.markdown("---")
...
# Footer
st.markdown("---")
st.markdown("🚀 Application développée avec algorithme de recherche tabou et réorganisation globale")