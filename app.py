...
def main():
    ...
    if st.button("🚀 Lancer l'optimisation", type="primary"):
        with st.spinner("Placement en cours..."):
            ...
            # IMPORTANT: Mettre à jour les quantités placées dans les bâtiments originaux
            for original_b in buildings:
                original_b.placed = 0
                original_b.positions = []
                for placed_b in tous:
                    if placed_b.nom == original_b.nom and not hasattr(placed_b, 'failed') or not placed_b.failed:
                        original_b.placed += 1
