...
class Terrain:
    def __init__(self, grid):
        ...
        self.cultural_zones = np.zeros_like(self.grid, dtype=object)  # Pour stocker quels culturels affectent chaque case
        self.building_type_map = np.full_like(self.grid, '', dtype=object)
        self.building_objects = []  # Liste des objets Building placés
        
    def can_place(self, x, y, longueur, largeur, ignore_cultural=False):
    ...
    def calculate_cultural_effect(self):
        """Calcule l'effet culturel de tous les bâtiments - chaque culturel compte une fois par producteur"""
        # Initialiser une matrice pour stocker les ensembles de culturels
        ...
        for building, x, y, orientation, longueur, largeur in self.cultural_buildings:
            if building.culture > 0:
                ...
                # Marquer toutes les cases dans le rayon
                for i in range(max(0, center_x - building.rayonnement), 
                    ...
                    for j in range(max(0, center_y - building.rayonnement), 
                        ...
                        # Distance de Manhattan ou distance max? On utilise la distance max
    ...
    def get_culture_for_building(self, building, x, y, orientation):
        ...
        # Collecter tous les IDs uniques des culturels qui affectent ce bâtiment
        ...
        for cultural_id in affecting_cultural_ids:
            # Trouver le bâtiment culturel correspondant
    ...
    def get_production_boosts(self):
        ...
        for building, x, y, orientation, longueur, largeur in self.buildings:
            if building.type == "producteur" and building.production:
                ...
                # Calculer la culture reçue (chaque culturel compte une fois)
...
class BuildingPlacer:
    ...
    def get_priority_score(self, building):
        ...
        if building.type == "producteur" and building.production:
            ...
            # Les culturels avec grand rayonnement en premier
            return 10 - (building.rayonnement / 10) - (building.culture / 1000)
        return 100
    
    def find_optimal_position(self, building):
        """Trouve la meilleure position pour un bâtiment"""
        best_score = -1
        best_position = None
        
        # Pour les grands bâtiments, vérifier d'abord si la plus grande zone libre est assez grande
        if building.get_area() > 9:  # Bâtiments de taille > 3x3
            free_area = self.terrain.find_largest_free_area()
            if len(free_area) < building.get_area():
                return None
    ...
    def evaluate_position(self, building, x, y, orientation):
        ...
        if building.type == "producteur" and building.production:
            # Pour les producteurs, maximiser la culture reçue (chaque culturel compte une fois)
            total_culture = self.terrain.get_culture_for_building(building, x, y, orientation)
            
            # Bonus important si on atteint un seuil
            if building.boost_100 > 0 and total_culture >= building.boost_100:
                return 10000 + total_culture
            elif building.boost_50 > 0 and total_culture >= building.boost_50:
                return 5000 + total_culture
            elif building.boost_25 > 0 and total_culture >= building.boost_25:
                return 2500 + total_culture
            else:
                return total_culture
        
        elif building.type == "culturel":
            # Pour les culturels, maximiser l'impact futur
            # Calculer combien de cases libres seront dans le rayon
            future_impact = 0
            center_x = x + longueur // 2
            center_y = y + largeur // 2
            
            for i in range(max(0, center_x - building.rayonnement), 
                         min(self.terrain.width, center_x + building.rayonnement + 1)):
                for j in range(max(0, center_y - building.rayonnement), 
                             min(self.terrain.height, center_y + building.rayonnement + 1)):
                    if self.terrain.grid[j, i] == 1 and not self.terrain.occupied[j, i]:
                        future_impact += 1
            
            # Bonus pour les positions centrales (pour éviter les bords)
            center_dist = abs(x + longueur/2 - self.terrain.width/2) + abs(y + largeur/2 - self.terrain.height/2)
            
            return future_impact * 100 - center_dist
    ...
    def place_all(self):
        ...
        st.write(f"Total à placer: {total_a_placer} bâtiments")
        st.write(f"  - Culturels: {len(cultural_buildings)} types")
        st.write(f"  - Producteurs: {len(producer_buildings)} types")
        st.write(f"  - Autres: {len(other_buildings)} types")
        
        # 1. D'abord, placer les plus grands culturels pour maximiser la couverture
        cultural_buildings.sort(key=lambda b: (-b.rayonnement, -b.culture, -b.get_area()))
        
        st.subheader("🏛️ Placement des bâtiments culturels (par rayon décroissant)")
        
        for building in cultural_buildings:
            if building.quantite == 0:
                continue
            
            st.write(f"\n📦 {building.nom} (rayon={building.rayonnement}, culture={building.culture})")
            ...
            for i in range(building.quantite):
                ...
                best_pos = self.find_optimal_position(building)
                
                if best_pos:
                    x, y, orientation = best_pos
                    self.terrain.place_building(building, x, y, orientation)
                    placed_count += 1
                    st.write(f"  ✅ Exemplaire {i+1} placé à ({x},{y})")
                else:
                    reason = "Aucun emplacement assez grand disponible"
                    building.failed_attempts.append(f"Exemplaire {i+1}: {reason}")
                    st.write(f"  ❌ Exemplaire {i+1} non placé - {reason}")
            
            if placed_count > 0:
                st.write(f"  → {placed_count}/{building.quantite} placés")
        
        # Recalculer l'effet culturel
        self.terrain.calculate_cultural_effect()
        
        # 2. Placer les producteurs par ordre de priorité
        ...
        for building in producer_buildings:
            ...
            st.write(f"\n📦 {building.nom} (priorité={self.get_priority_score(building)})")
            ...
            for i in range(building.quantite):
                ...
                if best_pos:
                    ...
                    culture = self.terrain.get_culture_for_building(building, x, y, orientation)
                    
                    st.write(f"  ✅ Exemplaire {i+1} placé à ({x},{y}) - reçoit {culture:.0f} culture")
                else:
                    reason = "Aucun emplacement disponible"
                    building.failed_attempts.append(f"Exemplaire {i+1}: {reason}")
                    st.write(f"  ❌ Exemplaire {i+1} non placé - {reason}")
        ...
        # Résumé final
        total_placed = sum(b.placed for b in valid_buildings)
        st.subheader("📊 Résumé final")
        st.write(f"✅ Total placé: {total_placed}/{total_a_placer}")
        
        if total_placed < total_a_placer:
            st.warning(f"❌ {total_a_placer - total_placed} bâtiments non placés")
            
            # Analyse des raisons
            by_reason = defaultdict(int)
            for b in valid_buildings:
                if b.placed < b.quantite:
                    for attempt in b.failed_attempts:
                        reason = attempt.split(': ')[-1]
                        by_reason[reason] += 1
...
def read_input_file(file):
    ...
    try:
        ...
        # Lire les bâtiments (prendre le 2ème onglet)
...
def main():
    ...
    **Stratégie**:
    - Placement des culturels par rayon décroissant pour maximiser la couverture
    - Regroupement des espaces libres pour permettre les grands bâtiments
    - **Calcul précis** : Chaque bâtiment culturel ne compte qu'une fois par producteur
    """)
    ...
    if uploaded_file:
        ...
        if terrain_grid is not None and buildings_df is not None:
            ...
            if st.button("🚀 Lancer l'optimisation", type="primary"):
                with st.spinner("Optimisation en cours..."):
                    ...
                    # Créer une visualisation avec couleurs par type
                    ...
                    if building_symbols:
                        ...
                        legend_items = list(building_symbols.items())[:20]  # Limiter à 20 pour la lisibilité
                        legend_cols = st.columns(4)
                        for i, (building_name, symbol) in enumerate(legend_items):
                            with legend_cols[i % 4]:
                                st.markdown(f"{symbol} {building_name[:20]}...")
                        
                        if len(building_symbols) > 20:
                            st.caption(f"... et {len(building_symbols) - 20} autres (voir fichier Excel)")
        ...
        with st.expander("ℹ️ Comment ça marche"):
            ...
            **Stratégie de placement**:
            ...
            1. **Bâtiments culturels** : Placés en premier, par rayon décroissant
               - Maximise la couverture culturelle
               - Priorité aux grands rayons pour couvrir plus de zone
            
            2. **Bâtiments producteurs** : Placés ensuite par priorité
               - Guérison > Nourriture > Or
               - Positionnés pour maximiser la culture reçue
            
            3. **Calcul de la culture** : 
               - Chaque bâtiment culturel ne compte qu'une seule fois par producteur
               - Peu importe la taille du producteur, il ne reçoit la culture qu'une fois
               - Exemple: Un producteur de 2x2 cases dans le rayon d'un culturel de 615 reçoit 615, pas 2460
            
            4. **Regroupement** : L'algorithme cherche à garder les cases libres groupées
               - Permet de placer les grands bâtiments
