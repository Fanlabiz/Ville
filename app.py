...
class Terrain:
    ...
    def find_largest_contiguous_area(self, temp_occupied=None):
        """
    ...
    def get_space_preservation_score(self, x, y, longueur, largeur, remaining_buildings):
        """
        Calcule un score de préservation d'espace basé sur la taille de la plus grande zone contiguë.
        """
        ...
        # Trouver la plus grande zone contiguë après placement
        largest_area = self.find_largest_contiguous_area(temp_occupied)
        largest_size = len(largest_area)
        
        if not remaining_buildings:
            return 10000  # Plus rien à placer
        
        # Vérifier si le plus grand bâtiment restant peut tenir dans la plus grande zone
        biggest_remaining = max(remaining_buildings, key=lambda b: b.get_area())
        biggest_area = biggest_remaining.get_area()
        
        if largest_size < biggest_area:
            return -10000  # Pénalité énorme si on ne peut pas placer le plus grand
        
        # Score basé sur la taille de la plus grande zone
        return largest_size * 10
...
def main():
    ...
    st.title("🏗️ Placeur de bâtiments - Préservation des zones contiguës")
    ...
    if st.button("🚀 Lancer l'optimisation", type="primary"):
        with st.spinner("Placement en cours..."):
            ...
            # (les plus grands d'abord car plus contraignants)
            ...
            for i, b in enumerate(producteurs):
                ...
                for x, y, o in positions:
                    ...
                    # Score de préservation d'espace (basé sur la taille de la plus grande zone contiguë)
                    space_score = terrain_final.get_space_preservation_score(x, y, l, L, remaining)
                    score += space_score * 100  # Pondération très importante
                ...
                if meilleure_pos:
                    ...
                    # Afficher la taille de la plus grande zone restante
                    largest_area = terrain_final.find_largest_contiguous_area()
                    st.write(f"✅ {b.nom} à ({x},{y}) - culture: {culture:.0f} {boost_text} - plus grande zone: {len(largest_area)} cases")
                else:
                    b.failed = True
                    stats["non_places"] += 1
                    st.write(f"❌ Impossible de placer {b.nom}")
