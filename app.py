...
class PlaceurBatiments:
    ...
    def _charger_batiments(self, df):
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(row.get('Culture')) and row['Culture'] > 0:
                type_bat = 'culturel'
            elif pd.notna(row.get('Production')) and str(row.get('Production', '')).strip() != '':
                type_bat = 'producteur'
            ...
            batiment = Batiment(
                ...
                production=str(row.get('Production', '')) if pd.notna(row.get('Production')) else ''
            )
    ...
    def placer_batiments(self):
        ...
        while (index_culturel < len(culturels) or index_producteur < len(producteurs)) and essais < max_essais:
            ...
            if index_culturel < len(culturels):
                ...
                if batiment not in positions_essayees:
                    positions_essayees[batiment] = set()
                
                self._journaliser(f"Évaluation du bâtiment culturel: {batiment.nom}")
                
                # Trouver un emplacement différent de ceux déjà essayés
                emplacement = self._trouver_emplacement_avec_exclusion(batiment, positions_essayees[batiment])
                ...
                if emplacement:
                    ...
                    if self._verifier_place_disponible(batiment, culturels_restants + producteurs_restants):
                        ...
                        positions_essayees[batiment].add((x, y, orientation))
                    ...
                    if pile_placement:
                        ...
                        if dernier[0] not in positions_essayees:
                            positions_essayees[dernier[0]] = set()
                        positions_essayees[dernier[0]].add((dernier[1], dernier[2], dernier[3]))
    ...
    def generer_resultats(self, batiments_non_places):
        ...
        cases_batiments_non_places = sum(b.longueur * b.largeur for b in batiments_non_places if b in batiments_non_places)
            ...
            if st.button("🚀 Lancer le placement", type="primary", use_container_width=True):
                ...
                if resultats['batiments_non_places']:
                    ...
                    for b in resultats['batiments_non_places']:
                        if b in resultats['batiments_non_places']:
                ...
                with st.expander("📋 Journal des opérations"):
                    # Compter les occurrences pour détecter les boucles potentielles
                    dernieres_entrees = resultats['journal'][-10:] if len(resultats['journal']) > 10 else resultats['journal']
                    for i, entry in enumerate(dernieres_entrees, max(1, len(resultats['journal']) - 9)):
                        st.text(f"{i:3d}. {entry}")
                    
                    if len(resultats['journal']) > 10:
                        st.text("...")
