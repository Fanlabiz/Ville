...
class CityPlanner:
    ...
    def load_existing_buildings(self, actuel_data):
        ...
        # Afficher la liste complète dans Streamlit
        st.session_state['buildings_detected'] = buildings_found
...
st.title("🏗️ Optimiseur de Cité avec Diagnostic")
st.markdown("""
...
- Lit la configuration actuelle (onglet 3)
- Affiche la liste détaillée des bâtiments détectés
...
if uploaded:
    with st.spinner("Analyse du fichier en cours..."):
        try:
            ...
            # Afficher les bâtiments détectés
            if 'buildings_detected' in st.session_state:
                st.subheader("📋 Bâtiments détectés dans l'onglet Actuel")
                detected = st.session_state['buildings_detected']
                st.info(f"Nombre de bâtiments détectés: {len(detected)}")
                
                # Afficher dans un expander pour ne pas surcharger
                with st.expander("Voir la liste complète"):
                    for b in detected:
                        st.text(b)
            
            # Construire la liste de tous les bâtiments à placer
            all_buildings = []
