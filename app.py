...
class Batiment:
    ...
    def __post_init__(self):
        if self.type not in ['culturel', 'producteur', 'neutre']:
            if self.type == '' or pd.isna(self.type) or self.type is None:
                self.type = 'neutre'
...
class PlacementBatiments:
    ...
    def __init__(self, terrain_data: List[List[int]], batiments_data: List[Dict]):
        ...
        # Debug
        st.write(f"DEBUG - Nombre de bâtiments créés: {len(self.batiments)}")
        for b in self.batiments[:5]:  # Afficher les 5 premiers
            st.write(f"  - {b.nom}: type={b.type}, dimensions={b.longueur}x{b.largeur}, quantité initiale={b.quantite}")
    ...
    def _creer_batiments(self, data: List[Dict]) -> List[Batiment]:
        ...
        batiments = []
        st.write("DEBUG - Début de création des bâtiments")
        
        for i, row in enumerate(data):
            try:
                # Afficher les colonnes disponibles pour debug
                if i == 0:
                    st.write(f"Colonnes disponibles: {list(row.keys())}")
                ...
                if i < 5:  # Afficher les 5 premiers pour debug
                    st.write(f"  ✓ Créé: {batiment.nom}, type={batiment.type}, dim={batiment.longueur}x{batiment.largeur}, qté={batiment.quantite}")
                    
            except Exception as e:
                st.error(f"Erreur lors de la création du bâtiment à la ligne {i}: {str(e)}")
                st.write(f"Données de la ligne: {row}")
                continue
                
        st.write(f"DEBUG - Total bâtiments créés: {len(batiments)}")
        return batiments
    ...
    def executer_placement(self):
        ...
        st.write(f"DEBUG - Début placement avec {len(self.batiments_non_places)} bâtiments")
        
        # Afficher la répartition par type
        types_count = {}
        for b in self.batiments_non_places:
            types_count[b.type] = types_count.get(b.type, 0) + 1
        st.write(f"DEBUG - Répartition: {types_count}")
        ...
        st.write(f"DEBUG - Après phase 1: {len(self.batiments_places)} placés, {len(self.batiments_non_places)} restants")
        ...
        st.write(f"DEBUG - FINAL: {len(self.batiments_places)} placés, {len(self.batiments_non_places)} non placés")
...
def main():
    ...
    if uploaded_file:
        try:
            ...
            st.write(f"DEBUG - Onglets trouvés: {excel_file.sheet_names}")
