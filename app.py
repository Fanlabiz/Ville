...
class PlacedBuilding:
    ...
    def get_bounding_box(self):
        """Retourne le rectangle englobant du bâtiment"""
...
class Terrain:
    ...
    def find_potential_space_creations(self, target_rect_size):
        """Trouve des déplacements qui pourraient créer un espace de taille target_rect_size"""
        potential_moves = []
        
        # Pour chaque bâtiment, essayer de le décaler légèrement
        for building in self.buildings:
            old_row, old_col = building.row, building.col
            old_orientation = building.orientation
            
            # Tester les petits déplacements (1-3 cases dans chaque direction)
            for dr in range(-3, 4):
                for dc in range(-3, 4):
                    if dr == 0 and dc == 0:
                        continue
                    ...
                    # Vérifier si le bâtiment peut être placé à cette nouvelle position
                    if self.can_place_building(building.building, new_row, new_col, old_orientation):
                        # Simuler le déplacement
                        self.remove_building(building)
                        
                        # Vérifier si cela crée un grand espace libre
                        self.place_building(building.building, new_row, new_col, old_orientation)
                        largest_rect = self.find_largest_free_rectangle()
                        
                        if largest_rect:
                            rect_row, rect_col, rect_height, rect_width = largest_rect
                            rect_area = rect_height * rect_width
                            if rect_area >= target_rect_size:
                        ...
                        self.remove_building(building)
                        self.place_building(building.building, old_row, old_col, old_orientation)
...
class BuildingPlacer:
    ...
    def smart_space_creation(self):
        """Tente de créer de grands espaces en déplaçant intelligemment des bâtiments"""
        if not self.unplaced:
            return
        
        # Calculer la taille minimale nécessaire pour les bâtiments non placés
        min_needed_area = min([b.get_area() for b in self.unplaced]) if self.unplaced else 0
        
        # Chercher des déplacements qui pourraient créer un espace de cette taille
        potential_moves = self.terrain.find_potential_space_creations(min_needed_area)
        
        # Trier par taille d'espace créé (décroissant)
        potential_moves.sort(key=lambda x: -x['rect_area'])
        
        # Essayer les meilleurs déplacements
        moves_applied = 0
        for move in potential_moves[:20]:  # Limiter le nombre de tentatives
            building = move['building']
            old_row, old_col, old_orientation = move['old_row'], move['old_col'], move['old_orientation']
            new_row, new_col, new_orientation = move['new_row'], move['new_col'], move['new_orientation']
            ...
            if self.terrain.can_place_building(building.building, new_row, new_col, new_orientation):
                ...
                # Essayer de placer un bâtiment non placé dans le nouvel espace
                rect = move['rect']
                for unplaced in self.unplaced[:]:
                    pos = self.find_position_in_rectangle(unplaced, rect)
                    if pos:
    ...
    def consolidate_free_space(self):
        ...
        for building in all_buildings[:50]:
    ...
    def try_place_remaining_with_consolidation(self):
        ...
        # Phase 1: Essayer de créer des espaces par déplacements intelligents
        st.info("🔧 Recherche d'opportunités de déplacement...")
        moves_applied = self.smart_space_creation()
        if moves_applied > 0:
            st.info(f"✅ {moves_applied} déplacements effectués pour créer des espaces")
        
        # Phase 2: Consolidation standard
...
def main():
    ...
    st.markdown("Chargez un fichier Excel pour placer les bâtiments avec optimisation intelligente des espaces")
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("🚀 Lancer le placement intelligent", type="primary"):
                with st.spinner("Placement et optimisation intelligente..."):
                    ...
                    st.download_button(
                        ...
                        data=output_file,
                        file_name="resultats_placement_intelligent.xlsx",
