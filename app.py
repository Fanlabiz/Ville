...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    # - non_neutres_ffd : Producteurs + Culturels triés par taille décroissante ENSEMBLE
    #   (mélange naturel qui place un Culturel à côté d'un Producteur de même taille)
    ...
    def ffd_pure(nn_ordered, n_ordered):
        """
        FFD avec biais de couverture pour les Culturels :
        - Producteurs : first-fit raster.
        - Culturels   : premiere position qui couvre au moins un Producteur
                        deja place (biais d'intrication). Fallback first-fit.
                ...
                for r in range(max_r):
                    ...
                    for c in range(max_c):
                        if can_place(r, c, rows, cols, x_grid, occ, max_r, max_c):
                            if first_fit is None:
                                first_fit = (r, c, rows, cols)
                            if b["type"] == "Culturel" and placed_prods:
                                # Biais : chercher une position qui couvre un Producteur
                                ray = b.get("rayonnement", 1)
                                ...
                                if any(cells_of(p) & zone for p in placed_prods):
                                    chosen = (r, c, rows, cols); break
                            else:
                                chosen = (r, c, rows, cols); break

            if chosen is None:
                chosen = first_fit  # fallback garanti
