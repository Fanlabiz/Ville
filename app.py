    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if optionnels_a_placer:
            ...
            def _best_pos_optionnel(opt_b, current):
                ...
                Stratégie en deux passes pour éviter d'appeler score_placement
                à chaque position (trop lent avec 99 bâtiments x 736 positions):
                1. Utiliser la couverture comme critère rapide (O(n_prod) par pos)
                2. Vérifier score_placement uniquement sur la meilleure position trouvée
                """
                _occ = make_occ_grid(current, max_r, max_c)

                if opt_b["type"] != "Culturel":
                    # Neutre: première case libre
                ...
                # Culturel: chercher par couverture (rapide), puis vérifier le gain
                ...
                # Pré-calculer les zones des producteurs
                _prod_cells = {id(p): cells_of(p) for p in _prods}

                _best_pos = None; _best_cov = -1
                    ...
                    for _r in range(max_r - _rows + 1):
                        for _c in range(max_c - _cols + 1):
                            ...
                            _cov = sum(
                                ...
                                for p in _prods
                                if _prod_cells[id(p)] & _zone and p.get("boost100", 0) > 0
                            )
                            if _cov > _best_cov:
                                _best_cov = _cov; _best_pos = (_r, _c, _rows, _cols)

                if _best_pos is None:
                    # Aucune position ne couvre de producteurs: prendre la première case libre
                    for _rows, _cols in [(opt_b["rows"], opt_b["cols"]),
                                          (opt_b["cols"], opt_b["rows"])]:
                        for _r in range(max_r - _rows + 1):
                            for _c in range(max_c - _cols + 1):
                                if can_place(_r, _c, _rows, _cols, _xg_opt, _occ, max_r, max_c):
                                    _best_pos = (_r, _c, _rows, _cols)
                                    break
                            if _best_pos: break
                        if _best_pos: break
                ...
                # Vérifier que le gain est >= 0 (ne diminue pas le score)
                _bat = {**opt_b, "r": _best_pos[0], "c": _best_pos[1],
                        "rows": _best_pos[2], "cols": _best_pos[3]}
                _gain = score_placement(current + [_bat]) - _score_now
                return _best_pos, _gain
            ...
            # Règle: placer le maximum d'optionnels tant que le score ne diminue pas.
            # On place même si gain=0 (neutre) — seul gain<0 interdit le placement.
            # Pour les Culturels: chercher la meilleure position (max gain, puis max couverture).
            # Pour les Neutres: première case libre disponible.
