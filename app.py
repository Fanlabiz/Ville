...
def _coord(r0, c0):
    """Convertit (r, c) 0-based interne en reference cellule Excel apres transposition.
    La transposition echange lignes et colonnes du terrain:
      new_col_excel = r0 + 1 - 41 = r0 - 40
      new_row_excel = c0 + 1 - 35 = c0 - 34
    Ex: Grand HdV (r0=42, c0=36) -> col=2(B), row=2 -> B2
    """
    new_col = r0 - 40
    new_row = c0 - 34
...
def _csv_building_category(nom_complet):
    ...
    des priorités : 'Barracks', 'Farm', 'Home', ou None.
    La détection se base sur les mots-clés du nom technique.
    """
    n = nom_complet.upper()
    if "BARRACKS" in n:
        return "Barracks"
    if "FARM" in n or "CAMELFARMED" in n or "WORKSHOP" in n or "MERCHANT" in n \
...
def read_from_csv(file_obj, city_name=None,
                  prio_barracks=0, prio_farm=0, prio_home=0):
    """
...
def build_excel_output(optimized, original_placed, terrain_grid, max_r, max_c, buildings_def):
    ...
    if not real_moves:
        ...
        while pending and iterations < len(real_moves) * 6:
            ...
            for mv in pending:
                ...
                if not blockers:
                    ...
                    from_str = (_coord(mv['old_r'], mv['old_c'])
                                if mv["_uid"] not in _in_reserve else "Réserve")
                    new_str = _coord(mv['new_r'], mv['new_c'])
                    ...
                    for blocker in blockers:
                        ...
                        if buid not in _in_reserve and buid not in _done_uids:
                            old_b = _coord(blocker['r'], blocker['c'])
        ...
        for buid, blocker in list(_in_reserve.items()):
            if buid not in _done_uids:
                ...
                if mv_final:
                    new_str = _coord(mv_final['new_r'], mv_final['new_c'])
                else:
                    # Pas de déplacement prévu: replacer à la position initiale
                    new_str = _coord(blocker['r'], blocker['c'])
                same_name_count = sum(1 for b in original_placed
                                       if b["nom"] == blocker["nom"])
                from_str = (_coord(blocker['r'], blocker['c'])
    ...
    col_w = 14
    row_h = 20
    # Terrain transposé : rows et cols échangés, avec offset
    # new_col = r+1-41 = r-40,  new_row = c+1-35 = c-34
    # => dimensions transposées : max_c colonnes x max_r lignes
    ...
    for r in range(max_r):
        for c in range(max_c):
            ...
            # Coordonnées transposées dans l'onglet Excel
            t_col = r - 40   # new Excel col (1-based)
            t_row = c - 34   # new Excel row (1-based)
            if t_col < 1 or t_row < 1:
                continue
            ...
            if terrain_grid[r][c] == "X":
                ...
                if b["r"] == r and b["c"] == c:
                    ...
                    # Fusion transposée : rows et cols inversés
                    if b["rows"] > 1 or b["cols"] > 1:
                        try:
                            ws4.merge_cells(
                                start_row=t_row, start_column=t_col,
                                end_row=t_row + b["cols"] - 1,
                                end_column=t_col + b["rows"] - 1
                            )
                        except Exception:
                            pass
    ...
    if uploaded_csv is not None:
        ...
        with _col_p2:
            _prio_farm = st.number_input(
                "🌾 Fermes (Farm / Workshop)",
                min_value=0, max_value=10, value=3, step=1,
                help="Bâtiments contenant 'Farm', 'Workshop', 'Merchant', 'Irrigation' ou 'Camel'"
            )
        with _col_p3:
        ...
        _prio_barracks, _prio_farm, _prio_home = 8, 3, 2
                ...
                if idx < len(orig_map[nom]):
                    ...
                    if op[0] != b["r"] or op[1] != b["c"]:
                        ...
                        line = (
                            ...
                            f"{_coord(op[0], op[1])} → {_coord(b['r'], b['c'])}"
