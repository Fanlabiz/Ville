...
def place_missing_buildings(placed, buildings_def, terrain_grid, max_r, max_c,
    ...
    def ffd_pure(nn_ordered, n_ordered):
        ...
        # ── Zone réservée aux culturels des casernes max ──
        # Les producteurs bas-prio ne doivent PAS occuper la zone centrale,
        # afin de laisser de la place aux culturels qui couvriront les casernes max.
        # Zone réservée = disque de rayon (ray_max_culturel + demi-taille_max_caserne)
        # centré sur le terrain.
        _max_prods_list = [b for b in nn_ordered
                           if b["type"] == "Producteur" and b.get("boost100_required", False)]
        if _max_prods_list:
            _ray_max_cult = max(
                (b.get("rayonnement", 0) for b in nn_ordered if b["type"] == "Culturel"),
                default=3
            )
            _max_cas_half = max(
                ((b["rows"] + b["cols"]) for b in _max_prods_list), default=0
            ) // 2
            _reserve_radius = _ray_max_cult + _max_cas_half + 2
            _center_r, _center_c = max_r / 2.0, max_c / 2.0
            _reserved_zone = {
                (r, c)
                for r in range(max_r) for c in range(max_c)
                if (abs(r - _center_r) + abs(c - _center_c)) <= _reserve_radius
            }
        else:
            _reserved_zone = set()
        ...
        for b in _border_obligatoires + _border_optionnels:
            occ = build_occ(result)
            ok = False
            _is_bas_prio_prod = (b["type"] == "Producteur" and id(b) in prods_bas_prio_ids)
            # Tentative 1 : placement en évitant la zone réservée (pour les prod bas-prio)
            for r, c in cells_border_first:
                for rows, cols in [(b["rows"], b["cols"]), (b["cols"], b["rows"])]:
                    if not can_place(r, c, rows, cols, x_grid, occ, max_r, max_c):
                        continue
                    if _is_bas_prio_prod and _reserved_zone:
                        bat_cells = {(r+dr, c+dc) for dr in range(rows) for dc in range(cols)}
                        if bat_cells & _reserved_zone:
                            continue
                    result.append({**b, "r": r, "c": c, "rows": rows, "cols": cols})
                    n_ok += 1; ok = True; break
                if ok: break
            # Tentative 2 (fallback) : si impossible d'éviter, placer sans contrainte
            if not ok:
                for r, c in cells_border_first:
                    for rows, cols in [(b["rows"], b["cols"]), (b["cols"], b["rows"])]:
                        if can_place(r, c, rows, cols, x_grid, occ, max_r, max_c):
        ...
        # Stratégie pour les boost100_required (max) :
        #   Logique SÉRIE : pour chaque max, placer la caserne PUIS ses culturels.
        #   Correction 1 : choisir la position de la caserne qui maximise l'espace libre
        #                  dans son rayon (éviter coins/bords peu entourés de cases libres).
        #   Correction 2 : pour chaque culturel, choisir la position dans le rayon de la
        #                  caserne courante qui couvre aussi le plus de casernes déjà placées.
        ...
        def _free_cells_in_radius(r, c, rows, cols, ray, occ_set):
            """Nombre de cases libres (non dans occ) dans le rayon autour d'un bâtiment."""
            ...
            for rz in range(r0-ray, r1+ray+1):
                for cz in range(c0-ray, c1+ray+1):
                    if r0 <= rz <= r1 and c0 <= cz <= c1:
                        continue
                    if 0 <= rz < max_r and 0 <= cz < max_c:
                        if (rz, cz) not in occ_set:
                            count += 1
            return count

        _placed_max_prods = []

        # ── Phase 2a : séries max (caserne + culturels) ──
        for prod in _max_prods:
            # 1. Placer la caserne à la position qui maximise les cases libres dans son rayon
            occ = build_occ(result)
            ray_max = max(c.get("rayonnement", 1) for c in cults_pool) if cults_pool else 3
            best_pos = None; best_free = -1; best_center_d = 9999
            ...
            for r in range(max_r):
                for c in range(max_c):
                    for rows, cols in [(prod["rows"], prod["cols"]), (prod["cols"], prod["rows"])]:
                        if not can_place(r, c, rows, cols, x_grid, occ, max_r, max_c):
                            continue
                        free = _free_cells_in_radius(r, c, rows, cols, ray_max, occ)
                        d = abs(r+rows/2-_tr) + abs(c+cols/2-_tc)
                        # Critère : d'abord maximiser l'espace libre, puis minimiser dist centre
                        if free > best_free or (free == best_free and d < best_center_d):
                            best_free = free; best_center_d = d
                            best_pos = (r, c, rows, cols)
            if best_pos:
                r, c, rows, cols = best_pos
            ...
            # 2. Placer les culturels pour cette caserne
            #    Ordre : grands d'abord (surface > 2), puis petits si nécessaire
            prod_boost100 = _ref.get("boost100", 9999)
            cults_grands = sorted(
            ...
            _cult_deja = culture_received(_ref, [b for b in result if b["type"] == "Culturel"])
            ...
            for cult in cults_dispo:
                ...
                if culture_received(_ref, _cults_act) >= prod_boost100:
                    break
                occ = build_occ(result)
                ray = cult.get("rayonnement", 1)
                prod_cells = cells_of(_ref)
                best_cult_pos = None; best_cult_gain = -1; best_cult_d = float("inf")
                ...
                for rows, cols in [(cult["rows"], cult["cols"]), (cult["cols"], cult["rows"])]:
                    for r in range(max_r):
                        for c in range(max_c):
                            ...
                            # Doit couvrir la caserne courante
                            if not (prod_cells & zone):
                                continue
                            # Gain = culture apportée aux casernes MAX déjà placées
                            # (dont la caserne courante)
                            gain = sum(cult["culture"] for p in _placed_max_prods
                                       if cells_of(p) & zone)
                            d = abs(r+rows/2-(_ref["r"]+_ref["rows"]/2)) + \
                                abs(c+cols/2-(_ref["c"]+_ref["cols"]/2))
                            # Critère : maximiser le gain sur les max déjà placés,
                            # tie-break : minimiser la distance à la caserne courante
                            if gain > best_cult_gain or \
                               (gain == best_cult_gain and d < best_cult_d):
                                best_cult_gain = gain; best_cult_d = d
                                best_cult_pos = (r, c, rows, cols)

                if best_cult_pos:
                    r, c, rows, cols = best_cult_pos
                    result.append({**cult, "r": r, "c": c, "rows": rows, "cols": cols})
                    cults_placed_ids.add(id(cult)); n_ok += 1

        # ── Phase 2b : placer les autres top_prods au centre ──
        ...
        # ── Phase 2c : culturels pour les autres top_prods (logique d'origine) ──
        for placed_prod_ref in [b for b in placed_top_prods
            ...
            cults_grands_loc = sorted(
                [c for c in cults_pool if id(c) not in cults_placed_ids
                 and c["rows"] * c["cols"] > 2],
                key=lambda c: -(c["culture"] * max(c.get("rayonnement", 1), 1))
            )
            cults_tous_loc = sorted(
                [c for c in cults_pool if id(c) not in cults_placed_ids],
                key=lambda c: -(c["culture"] * max(c.get("rayonnement", 1), 1))
            )
            _cult_deja = culture_received(placed_prod_ref,
                                          [b for b in result if b["type"] == "Culturel"])
            cults_dispo = (cults_grands_loc
                           if _cult_deja + sum(c["culture"] for c in cults_grands_loc) >= prod_boost100
                           else cults_tous_loc)
            ...
            for cult in cults_dispo:
                ...
                best_pos = None; best_dist_cult = float("inf")
                for rows, cols in [(cult["rows"], cult["cols"]), (cult["cols"], cult["rows"])]:
                    for r in range(max_r):
                        for c in range(max_c):
                            if not can_place(r, c, rows, cols, x_grid, occ, max_r, max_c):
                            ...
                            if prod_cells_ref & zone:
                                ...
                                if d < best_dist_cult:
                                    best_dist_cult = d; best_pos = (r, c, rows, cols)
                if best_pos:
                    r, c, rows, cols = best_pos
                    result.append({**cult, "r": r, "c": c, "rows": rows, "cols": cols})
                    cults_placed_ids.add(id(cult)); n_ok += 1
...
if 'placed' in dir() and 'original_placed' in dir():
    ...
    if st.button("Lancer l'optimisation", type="primary"):
        ...
        if _boost100_failures:
            st.warning(
                "⚠️ **Boost 100% obligatoire non atteint** pour :\n\n"
                + "\n\n".join(f"- {f}" for f in _boost100_failures)
                + "\n\nLe fichier Excel est quand même généré pour vous permettre "
